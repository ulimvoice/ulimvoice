(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73111__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73111__ = true;

  const VERSION = '2026-07-31.731.11';
  const CACHE_PREFIX = 'ulim_staff_fsop_73111_';
  const inflight = new Map();
  let runtimePromise = null;
  let revisionEnsurePromise73111 = null;
      ...
      if (kind === 'attendance') {
        ...
        if (select && text(select.value) && typeof global.adminLoadAttendanceSnapshot === 'function') {
          global.adminLoadAttendanceSnapshot(false, false).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.11 attendance realtime reload]', error);
          });
        }
      } else {
        ...
        if (select && text(select.value) && typeof global.adminLoadDailyEvalStudents === 'function') {
          global.adminLoadDailyEvalStudents(false).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.11 daily realtime reload]', error);
          });
        }
      }
    }, 180);
  ...
  async function bindRevisionListener7318(kind, date) {
    ...
    try {
      ...
      state.unsubscribe = rt.sdk.onSnapshot(ref, function (snapshot) {
        ...
        state.lastRevision = revision;
        safeConsole('info', '[ULIM 7.31.11 ' + kind + ' revision]', targetDate, revision);
        ...
        if (code.indexOf('permission-denied') >= 0) {
          ...
          safeConsole('info', '[ULIM 7.31.11 ' + kind + ' revision waits for Firebase claims]');
          return;
        }
        safeConsole('warn', '[ULIM 7.31.11 ' + kind + ' revision listener]', error);
      });
    } catch (error) {
      state.blockedUntil = Date.now() + 1500;
      safeConsole('warn', '[ULIM 7.31.11 revision bind]', kind, error);
    }
  }

  function ensureRevisionListeners7318() {
    if (revisionEnsurePromise73111) return revisionEnsurePromise73111;
    revisionEnsurePromise73111 = (async function () {
      ...
      return true;
    })().finally(function () { revisionEnsurePromise73111 = null; });
    return revisionEnsurePromise73111;
  }
    ...
    try {
      ...
      if (room && typeof room.ensureAuthenticated === 'function') {
        room.ensureAuthenticated().catch(function (error) {
          safeConsole('warn', '[ULIM 7.31.11 auth prewarm]', error);
        });
      }
    } catch (ignore) {}
  }
  ...
  safeConsole('info', '[ULIM staff Firestore operational auth-ready gated]', VERSION);
