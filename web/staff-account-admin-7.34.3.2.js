(function (global) {
  'use strict';
  if (global.__ULIM_STAFF_ACCOUNT_ADMIN_73432__) return;
  global.__ULIM_STAFF_ACCOUNT_ADMIN_73432__ = true;

  const VERSION = '2026-08-01.734.03.2';
  let accounts = [];
  ...
  async function runtime(timeoutMs) {
    return withTimeout(async function () {
      ...
      if (typeof room.getStableIdToken === 'function') await room.getStableIdToken(rt, false, 'staff-account-admin-73432');
  ...
  function install() {
    ...
    try {
      ...
      console.info('[ULIM 7.34.3.2 CLEAN START] bounded Firebase runtime + staff sheet refresh');
    } catch (_ignore) {}
  }
