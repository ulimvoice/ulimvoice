(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735430__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-10.735.04.30-attendance-operations-convergence';

  var VERSION = '2026-08-10.735.04.30-attendance-operations-convergence';
  var loadSequence = 0;
  ...
  function specialCss7355033(value) {
    var kind = specialKind7355033(value);
    return kind ? ' ulim-special-' + kind.replace('_','-') + '-735430' : '';
  }
  ...
  async function submitAttendanceAdd735423() {
    ...
    var selectedDates = mode === 'existing' ? [] : selectedAttendanceAddDates735430();
    if (mode !== 'existing' && !selectedDates.length) return alert('적용할 수업일을 한 개 이상 체크해주세요.');
    var studentName = student ? student.name : directName;
    var dateSummary = mode === 'existing' ? '현재 수강반 정보 변경' : selectedDates.map(dateLabel7355033).join(', ');
  ...
  function cellStatusHtml735423(group, student, session, cell) {
    ...
    var showDateSpecial = !!special && !(text(cell.specialDisplayScope) === 'name' && (kind === 'new' || kind === 'class_move'));
    return '<div class="ulim-ledger-cell-actions735423' + (cell && cell.__saving735425 ? ' saving' : '') + '"><button type="button" data-ledger-status="출석" class="ulim-ledger-ox735423 ' + (status === '출석' ? 'on-o' : '') + '">O</button><button type="button" data-ledger-status="결석" class="ulim-ledger-ox735423 ' + (status === '결석' ? 'on-x' : '') + '">X</button><button type="button" data-ledger-detail="1" class="ulim-ledger-more735423" title="당일 메모">⋯</button></div>' + (showDateSpecial ? '<div class="ulim-ledger-special735423' + specialCss7355033(special) + '">' + escapeHtml(specialLabel7355033(special)) + '</div>' : '');
  }
  ...
  function ledgerStudentIdentityCells735427(group, student, rowIndex, monthKey, nameSpecial) {
    ...
    return '<td class="ulim-ledger-no735427">' + selector + '<span>' + (rowIndex + 1) + '</span></td>'
      + '<td class="ulim-ledger-name735427' + nameClass + '" data-month-roster="' + escapeHtml(monthKey) + '">' + drag + '<b>' + escapeHtml(student.studentName) + '</b>' + gear + '</td>';
  }
  ...
  function ledgerMonthBlockHtml735427(group, monthKey, sessions, kind) {
    ...
    var rows = students.map(function (student, rowIndex) {
      ...
      row += ledgerStudentIdentityCells735427(group, student, rowIndex, monthKey, nameSpecial);
      if (safeSessions.length) {
  ...
  function renderAllClassesBoardLocal735410() {
    ...
    var groups = (ledger.groups || []).filter(function (group) { return teacher === 'all' || normalize(group.instructorName) === teacher; }).sort(function (a, b) { var ta = text(a.startTime), tb = text(b.startTime); return ta.localeCompare(tb) || text(a.className).localeCompare(text(b.className), 'ko'); });
    if (!groups.length) {
  ...
  function bindLedgerEvents735423() {
    ...
    board.addEventListener('click', function (event) {
      ...
      if (target.hasAttribute('data-ledger-add-class')) {
        ...
        if (addGroup && addSession) openAttendanceAddModal735410({ date: addSession.date, month: text(target.dataset.month) || text(addSession.date).slice(0, 7), classId: addGroup.classId, className: addGroup.className });
        return;
      }
  ...
  async function applyAllClassesDrop735410(drag,targetClassId,targetDate) {
    ...
    try{
      ...
      if(mode==='makeup'||mode==='daily_special'){
        await call('addTemporaryAttendanceAdmin7355014',{studentUid:student.studentUid,studentName:student.name,kind:mode,date:targetDate,classId:target.classId,className:target.className,requestId:requestId('ledger-drop-temp-735430')});
      }else{
        await updateStudentClass735410(student,target,mode,drag.sourceClassId||'',(mode==='new'||mode==='class_move')?targetDate:'');
        if(mode==='new'||mode==='class_move') await call('addAttendanceSessionStudentsAdmin73550',{date:targetDate,classId:target.classId,studentUids:[student.studentUid],requestId:requestId('ledger-drop-session-735430')});
      }
  ...
  function install() {
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355016__ = 'attendance-admin-integrated-7.35.4.30';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355015__ = 'attendance-admin-integrated-7.35.4.30';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355014__ = 'attendance-admin-integrated-7.35.4.30';
