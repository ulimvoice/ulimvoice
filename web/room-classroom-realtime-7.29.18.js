(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72917__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72917__ = true;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72916__ = true;

  const VERSION = "2026-08-09.729.18-firestore-immediate-classroom";
  ...
  function resetStableTokenGuard72917(uid) {
    tokenGuard72917.uid = String(uid || "");
    tokenGuard72917.promise = null;
    tokenGuard72917.lastError = null;
    tokenGuard72917.lastErrorAt = 0;
    tokenGuard72917.lastForceAt = 0;
    tokenGuard72917.invalidEventUid = "";
  }

  function terminalTokenError72917(error) {
  ...
  async function getStableIdToken72917(runtime, forceRefresh, reason) {
    ...
    if (tokenGuard72917.uid !== uid) resetStableTokenGuard72917(uid);

    const now = Date.now();
    const cooldownMs = terminalTokenError72917(tokenGuard72917.lastError) ? 60000 : 12000;
    if (tokenGuard72917.lastError && now - tokenGuard72917.lastErrorAt < cooldownMs) {
      throw tokenGuard72917.lastError;
    }
    if (tokenGuard72917.promise) return tokenGuard72917.promise;

    let force = forceRefresh === true;
    if (force && now - tokenGuard72917.lastForceAt < 15000) force = false;
    if (force) tokenGuard72917.lastForceAt = now;

    tokenGuard72917.promise = Promise.resolve()
      ...
      .catch(function (error) {
        tokenGuard72917.lastError = error;
        tokenGuard72917.lastErrorAt = Date.now();
        if (terminalTokenError72917(error) && tokenGuard72917.invalidEventUid !== uid) {
          tokenGuard72917.invalidEventUid = uid;
          try {
    ...
    return tokenGuard72917.promise;
  }

  async function getStableIdTokenResult72917(runtime, forceRefresh, reason) {
    await getStableIdToken72917(runtime, forceRefresh === true, reason || "token-result");
  ...
  function resetRealtimeForStaffLogout72913() {
    state.ready = false;
    resetStableTokenGuard72917("");
    state.preissuedProof = null;
    state.sessionFingerprint = "";
    state.lastSheetCompatibilityToken = "";
  ...
  async function signInLegacySessionFresh(reason) {
    ...
    await getStableIdToken72917(runtime, false, "custom-token-signin");
  ...
  async function acceptExistingFirebaseSession72912(runtime, reason, forceRefresh) {
    ...
    const user = runtime.auth.currentUser;
    const tokenResult = await getStableIdTokenResult72917(runtime, forceRefresh === true, reason || "accept-existing");
    ...
    resetStableTokenGuard72917(user.uid);
    state.ready = true;
  ...
  async function ensureAuthenticated() {
    ...
    state.signingIn = (async function () {
      ...
      if (runtime.auth.currentUser && storedFingerprint === fingerprint) {
        try {
          await getStableIdToken72917(runtime, false, "legacy-existing");
          state.ready = true;
          state.sessionFingerprint = fingerprint;
          setStatus("ready", "실시간 연결됨");
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (lastRuntime && lastRuntime.auth && lastRuntime.auth.currentUser) {
          await getStableIdToken72917(lastRuntime, false, "wait-until-authenticated");
          return lastRuntime;
        }
      } catch (_ignore) {}
  ...
  async function subscribeClassroom(date) {
    ...
    state.classroomUnsubscribe = runtime.sdk.onSnapshot(
      ...
      function (error) {
        ...
        setStatus("error", "실시간 수신 실패 · 다시 연결 중");
  ...
  async function fastInitialClassroomLoad7297(date, options) {
    ...
    if (options.forceSheet === true && currentAdminToken727()) {
    ...
    const promise = (async function () {
      ...
      try {
        ...
        if (!currentAdminToken727()) {
          return { source: "firestore-primary", displayed: hasClassroomDisplayData7297(date), compatibilityPending: true };
        }
  ...
  async function saveClassroomFirestoreFirst727(silentConfirm, forceOverride) {
    ...
    if (!silentConfirm) {
      ...
      if (!confirm([
        ...
        "",
        actionText + " 내용을 실시간 저장할까요?"
      ].join("\n"))) return;
    }

    state.classroomPrimarySaving = true;
    try {
      if (typeof showLoading === "function") showLoading(forceOverride ? "강의실 배정 수정 중..." : "강의실 사용 저장 중...");
      const runtime = await ensureAuthenticated();
      if (!runtime || !runtime.auth || !runtime.auth.currentUser) {
        throw new Error("교직원 로그인 상태를 확인하지 못했습니다. 다시 로그인해주세요.");
      }

      if (forceOverride) {
        if (request.groups.length !== 1 || Number(group.endHour) !== Number(group.startHour) + 1) {
          throw new Error("강의실 배정 수정은 한 시간 칸씩 처리해주세요.");
        }
        const response = await runtime.updateClassroom({
          ...
          memo: request.memo,
          requestId: makeRequestId("CLASSROOM-UPDATE-72917")
        });
        ...
        const summary = document.getElementById("adminClassroomUsageSummary");
        if (summary) summary.textContent = "선택한 한 시간 칸을 수정했습니다.";
        scheduleKnownClassroomSheetJobs7294(runtime, result.jobId ? [{
          jobId: result.jobId,
          ...
          payload: {
            date: request.date, room: group.room, startHour: group.startHour, endHour: group.endHour,
            assignedInstructor: request.assignedInstructor, className: request.className, memo: request.memo, override: "1"
          }
        }] : []);
        return result;
      }
      ...
      const response = await runtime.commitClassroom({
        ...
        memo: request.memo,
        forceOverride: false,
        requestId: makeRequestId("CLASSROOM-PRIMARY-72917")
      });
      const result = callableData727(response) || {};
      if (!result.ok) throw new Error(result.message || "강의실 사용 저장에 실패했습니다.");
      applyClassroomCommitResult727(request.date, request, result);
      setStatus("ready", "실시간 연결됨");
      const accepted = Array.isArray(result.accepted) ? result.accepted : [];
      scheduleKnownClassroomSheetJobs7294(runtime, accepted.map(function (item) {
        const matched = request.groups.find(function (candidate) {
          return String(candidate.room || "").replace(/\s+/g, "") === String(item && item.room || "").replace(/\s+/g, "") &&
            Number(candidate.startHour) === Number(item && item.startHour);
        });
        if (!item || !item.jobId || !matched) return null;
        return {
          jobId: item.jobId, requestId: item.jobId, operation: "save", date: request.date, slotKey: item.slotKey || "",
          payload: {
            date: request.date, room: matched.room, startHour: matched.startHour, endHour: matched.endHour,
            assignedInstructor: request.assignedInstructor, className: request.className, memo: request.memo, override: "0"
          }
        };
      }).filter(Boolean));
      return result;
      ...
      const summary = document.getElementById("adminClassroomUsageSummary");
      if (summary) summary.textContent = "강의실 저장에 실패했습니다. 최신 현황을 다시 확인해주세요.";
      alert(callableMessage727(error) || "강의실 사용 저장에 실패했습니다.");
      setTimeout(function () { subscribeClassroom(request.date).catch(function () {}); }, 0);
      return null;
    } finally {
  ...
  async function commitSingleClassroomSlot729(room, hour) {
    ...
    try {
      if (typeof adminFindClassroomUsageAt_ === "function" && adminFindClassroomUsageAt_(normalizedRoom, numericHour)) {
        if (typeof adminRenderClassroomUsageTable === "function") adminRenderClassroomUsageTable();
        return alert("이미 사용 중인 강의실입니다.");
      }
    } catch (_ignore) {}
    ...
    const assignedInstructor = String(
      document.getElementById("adminClassroomUsageInstructor")?.value || currentAdminDisplayName727() || ""
    ).trim();
    const className = String(document.getElementById("adminClassroomUsageClass")?.value || "").trim();
    const memo = String(document.getElementById("adminClassroomUsageMemo")?.value || "").trim();
    const request = {
      ...
      className: className,
      memo: memo,
      selectedSlots: [{ date: date, room: normalizedRoom, hour: numericHour }],
      groups: [{ room: normalizedRoom, startHour: numericHour, endHour: numericHour + 1 }]
    };

    state.savingClassroomSlots.add(savingKey);
    if (typeof adminRenderClassroomUsageTable === "function") adminRenderClassroomUsageTable();
    const summary = document.getElementById("adminClassroomUsageSummary");
    if (summary) summary.textContent = normalizedRoom + " " + numericHour + ":00~" + (numericHour + 1) + ":00 · 저장 중";

    try {
      const runtime = await ensureAuthenticated();
      if (!runtime || !runtime.auth || !runtime.auth.currentUser) throw new Error("교직원 로그인 상태를 확인하지 못했습니다. 다시 로그인해주세요.");
      await ensureClassroomBaseline727(date);
      const response = await runtime.commitClassroom({
        date: date,
        groups: request.groups,
        assignedInstructor: assignedInstructor,
        ...
        memo: memo,
        forceOverride: false,
        requestId: makeRequestId("CLASSROOM-SLOT-72917")
      });
      const result = callableData727(response) || {};
      if (!result.ok) throw new Error(result.message || "강의실 사용 저장에 실패했습니다.");
      applyClassroomCommitResult727(date, request, result);
      const conflicts = Array.isArray(result.conflicts) ? result.conflicts : [];
      if (summary) summary.textContent = conflicts.length ? "다른 사용자가 먼저 저장한 시간칸입니다. 최신 현황을 반영했습니다." : "강의실 사용 저장 완료";
      setStatus("ready", "실시간 연결됨");
      const accepted = Array.isArray(result.accepted) ? result.accepted : [];
      const acceptedJob = accepted.find(function (item) {
        return item && String(item.room || "").replace(/\s+/g, "") === normalizedRoom.replace(/\s+/g, "") && Number(item.startHour) === numericHour;
      });
      scheduleKnownClassroomSheetJobs7294(runtime, acceptedJob && acceptedJob.jobId ? [{
        jobId: acceptedJob.jobId, requestId: acceptedJob.jobId, operation: "save", date: date, slotKey: acceptedJob.slotKey || "",
        payload: { date: date, room: normalizedRoom, startHour: numericHour, endHour: numericHour + 1, assignedInstructor: assignedInstructor, className: className, memo: memo, override: "0" }
      }] : []);
      return result;
    } catch (error) {
      if (summary) summary.textContent = "저장 실패 · 현황을 다시 확인해주세요.";
      alert(callableMessage727(error) || "강의실 사용 저장에 실패했습니다.");
      setTimeout(function () { subscribeClassroom(date).catch(function () {}); }, 0);
      return null;
    } finally {
  ...
  function scheduleKnownClassroomSheetJobs7294(runtime, jobs) {
    // Firebase-primary 로그인만 복원된 경우에는 Firestore 저장을 완료 상태로 유지하고
    // GAS 호환 세션이 준비될 때까지 시트 작업을 서버 pending 상태로 남깁니다.
    if (!currentAdminToken727()) return;
  ...
  function installWrappers() {
    ...
    if (typeof global.adminReleaseClassroomUsage === "function") {
      global.adminReleaseClassroomUsage = async function (recordId) {
        ...
        const record = (rows || []).find(function (item) { return String(item.recordId || "") === String(recordId || ""); });
        if (!record) return alert("해제할 강의실 기록을 찾지 못했습니다. 현황을 다시 불러와주세요.");
        const startHour = Number(record.startHour);
        const endHour = Number(record.endHour);
        if (!Number.isInteger(startHour) || endHour !== startHour + 1) return alert("강의실 해제는 한 시간 칸씩 처리합니다.");
        if (!confirm(String(record.room || "") + " " + startHour + ":00~" + endHour + ":00\n\n이 한 시간 칸을 해제할까요?")) return;
        try {
          const runtime = await ensureAuthenticated();
          if (!runtime || !runtime.auth || !runtime.auth.currentUser) throw new Error("교직원 로그인 상태를 확인하지 못했습니다. 다시 로그인해주세요.");
          const response = await runtime.releaseClassroom({
            date: dateKeyFromClassroom(), room: record.room, startHour: startHour, endHour: endHour,
            recordId: record.recordId, requestId: makeRequestId("CLASSROOM-RELEASE-72917")
          });
          const result = callableData727(response) || {};
          if (Array.isArray(result.records)) {
          ...
          const summary = document.getElementById("adminClassroomUsageSummary");
          if (summary) summary.textContent = "선택한 한 시간 칸을 해제했습니다.";
          scheduleKnownClassroomSheetJobs7294(runtime, result.jobId ? [{
            jobId: result.jobId, requestId: result.jobId, operation: "release", date: dateKeyFromClassroom(), slotKey: result.slotKey || "",
            payload: { date: dateKeyFromClassroom(), room: record.room, startHour: startHour, endHour: endHour }
          }] : []);
          return result;
        } catch (error) {
          alert(callableMessage727(error) || "한 시간 칸 해제에 실패했습니다.");
          return null;
        }
      };
    }
  ...
  async function start() {
    ...
    state.authPollTimer = setInterval(async function () {
      ...
      const compatibilityToken = currentAdminToken727();
      if (compatibilityToken && compatibilityToken !== state.lastSheetCompatibilityToken) {
        state.lastSheetCompatibilityToken = compatibilityToken;
        setTimeout(function () { flushClassroomSheetSync727().catch(function () {}); }, 0);
      }
      const fingerprint = await sha256(session.type + "|" + session.token);
  ...
  const publicApi728 = Object.freeze({
    ...
    getStableIdToken: getStableIdToken72917,
    resetStableTokenGuard: resetStableTokenGuard72917,
  ...
  global.ULIM_ROOM_CLASSROOM_REALTIME_72917 = publicApi728;
  global.ULIM_ROOM_CLASSROOM_REALTIME_72916 = publicApi728;
  ...
  safeConsole("info", "[ULIM 7.29.17 Firebase-primary classroom realtime module]", VERSION);
