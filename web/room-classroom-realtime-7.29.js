(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72901__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72901__ = true;

  const VERSION = "2026-07-30.729.01";
  ...
  const APP_NAME = "ulim-room-classroom-realtime-729";
  const SESSION_FINGERPRINT_KEY = "ulimRealtimeLegacySessionFingerprint721";
  const REQUEST_PREFIX = "RT729";
  ...
  async function fallbackLegacyClassroomSave727(silentConfirm, forceOverride, error) {
    safeConsole("warn", "[ULIM classroom 7.29 fallback to GAS]", error);
  ...
  async function saveClassroomFirestoreFirst727(silentConfirm, forceOverride) {
    ...
    if (!silentConfirm) {
      ...
      if (!confirm([
        ...
        ...lines,
        "",
        "Firestore에 즉시 저장하고 시트는 백그라운드로 동기화할까요?"
      ].join("\n"))) return;
    }
    ...
    try {
      if (typeof showLoading === "function") showLoading("강의실 사용 즉시 저장 중...");
      const runtime = await ensureAuthenticated();
      if (!runtime) throw new Error("Firebase 실시간 인증이 준비되지 않았습니다.");
      ...
      if (firestoreCommitted) {
        safeConsole("warn", "[ULIM classroom 7.29 committed but UI refresh failed]", error);
        setStatus("ready", "Firestore 저장 완료 · 화면 갱신 대기");
        setTimeout(function () {
  ...
  async function commitSingleClassroomSlot729(room, hour) {
    ...
    const request = {
      ...
      className,
      memo,
      selectedSlots: [{ date, room: normalizedRoom, hour: numericHour }],
    ...
    const summary = document.getElementById("adminClassroomUsageSummary");
    if (summary) {
      summary.textContent = normalizedRoom + " " + numericHour + ":00~" + (numericHour + 1) + ":00 · Firestore 저장 중";
    }
    ...
    try {
      ...
      const response = await runtime.commitClassroom({
        ...
        memo,
        forceOverride: false,
        requestId: makeRequestId("CLASSROOM-SLOT")
      });
      const result = callableData727(response) || {};
      if (!result.ok) throw new Error(result.message || "Firestore 저장에 실패했습니다.");
      ...
      if (summary) {
        summary.textContent = conflicts.length
          ? "다른 사용자가 먼저 저장한 시간칸입니다. 최신 현황을 반영했습니다."
          : "Firestore 즉시 저장 완료 · Google Sheets 백그라운드 기록 중";
      }
      setStatus("ready", "실시간 연결됨 · 즉시 저장");
      setTimeout(flushClassroomSheetSync727, 0);
      return result;
    } catch (error) {
      const message = callableMessage727(error) || "강의실 사용 저장에 실패했습니다.";
      if (summary) summary.textContent = "저장 실패 · 현황을 다시 확인해주세요.";
      safeConsole("warn", "[ULIM classroom 7.29 immediate save]", error);
      alert(message);
      setTimeout(function () { subscribeClassroom(date); }, 0);
      return null;
    } finally {
  ...
  async function processClassroomSheetJob727(runtime, job) {
    ...
    try {
      ...
      safeConsole("warn", "[ULIM classroom 7.29 sheet sync]", error);
      try {
  ...
  async function flushClassroomSheetSync727() {
    ...
    try {
      ...
      safeConsole("warn", "[ULIM classroom 7.29 pending sheet jobs]", error);
    } finally {
      ...
      global.adminLoadClassroomUsage = async function () {
        ...
        if (date && loadedForDate) {
          ...
          const baselinePromise = publishClassroomSnapshot("LOAD-BASELINE", {
            ...
            safeConsole("warn", "[ULIM classroom 7.29 baseline]", error);
            return false;
          }).finally(function () {
            delete state.classroomBaselinePromises[date];
          });
