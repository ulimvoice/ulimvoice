(function (global) {
  ...
  const VERSION = '2026-08-07.731.22.0-canonical-class-no-attendance-auth-refresh';
...
function attendanceMinimal(row) {
  ...
  function scheduleRevisionReload7318(kind, date, classId, revision) {
    // 7.31.21: 출석부는 자동 revision 새로고침을 하지 않습니다.
    // 사용자가 수동으로 다시 불러오거나 명시적으로 반/날짜를 선택할 때만 갱신합니다.
    if (kind === 'attendance') {
  ...
  function ensureRevisionListeners7318() {
    ...
    revisionEnsurePromise73112 = (async function () {
      ...
      global.__ULIM_ATTENDANCE_NO_AUTO_REFRESH_73122__ = true;
      await bindRevisionListener7318(
        'daily',
        dailyRevision.date,
        '',
        true
