(function (global) {
  'use strict';
  if (global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_7355026__) return;
  global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_7355026__ = true;
  global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_7355024__ = true;
  ...
  const VERSION = '2026-08-08.735.05.0.26-daily-canonical-preview-date-fix';
  ...
  function selectedDailyClass7355024() {
    ...
    let item = null;
    try {
      const confirmed = typeof global.adminGetDailyServerClassList7355026_ === 'function'
        ? global.adminGetDailyServerClassList7355026_(date)
        : [];
      item = (confirmed || []).find(function(row) { return directClassId && text(row && (row.classId || row.id)) === directClassId; }) ||
        (confirmed || []).find(function(row) { return normalize(row && row.className) === normalize(className); }) || null;
    } catch (_ignore) {}
    return { date, className, classId: directClassId || text(item && (item.classId || item.id)), item };
  }

  function applyVerifiedDailyClassList7355026(date, classes) {
    const list = Array.isArray(classes) ? classes : [];
    try {
      if (typeof global.adminRememberDailyServerClassList7355026_ === 'function') global.adminRememberDailyServerClassList7355026_(date, list);
      else if (typeof global.adminRememberClassListForDate704_ === 'function') global.adminRememberClassListForDate704_(date, list);
      const dailyDate = text((document.getElementById('adminDailyEvalDate') || {}).value);
      if (dailyDate !== text(date)) return;
      const panel = document.getElementById('adminDailyClassPickerPanel');
      const options = document.getElementById('adminDailyClassPickerPanelOptions');
      if (!panel || !panel.classList.contains('open') || !options || options.childElementCount === 0) {
        if (typeof global.adminRenderClassPicker === 'function') global.adminRenderClassPicker('adminDailyEvalClass', 'adminDailyClassPickerPanel');
        else if (typeof global.adminRenderClassSelectors === 'function') global.adminRenderClassSelectors();
      }
    } catch (_ignore) {}
  }

  async function saveDailyClassVideoLink7355024() {
    const selected = selectedDailyClass7355024();
    ...
    try {
      const data = await call('saveStaffDailyEvaluationsOperational', {
        ...
        requestId: requestId('daily-class-video-7355024'),
        date: selected.date,
      ...
      // Server round-trip verification: never claim persistence from a local cache alone.
      const verify = await call('getStaffClassListOperationalSnapshot', {
        date: selected.date,
        force: true,
        requestId: requestId('daily-class-video-verify-7355024')
      });
      const classes = Array.isArray(verify.classes) ? verify.classes : [];
      const verified = classes.find(function(row) { return text(row && row.classId) === selected.classId; }) ||
        classes.find(function(row) { return normalize(row && row.className) === normalize(data.className || selected.className); }) || null;
      if (!verified || text(verified.videoLink) !== videoLink) {
        throw new Error('수업영상 링크 저장 후 서버 재확인 값이 일치하지 않습니다.');
      }

      applyVerifiedDailyClassList7355026(selected.date, classes);
  ...
  global.ulimSaveDailyClassVideoLink7355024 = saveDailyClassVideoLink7355024;
  global.ulimSaveDailyClassVideoLink7355026 = saveDailyClassVideoLink7355024;

  function syncDailyVideoLinkInput7355026() {
    const input = document.getElementById('adminDailyClassVideoLink7355021');
    if (!input) return;
    const selected = selectedDailyClass7355024();
    const value = selected.item && Object.prototype.hasOwnProperty.call(selected.item, 'videoLink')
      ? text(selected.item.videoLink)
      : '';
    input.value = value;
    setDailyVideoStatus7355021('서버에 저장된 반별 영구값입니다.', true);
  }
  global.ulimSyncDailyClassVideoLink7355024 = syncDailyVideoLinkInput7355026;
  global.ulimSyncDailyClassVideoLink7355026 = syncDailyVideoLinkInput7355026;

  function resetDailyClassSelection7355026() {
    const input = document.getElementById('adminDailyEvalClass');
    const display = document.getElementById('adminDailyEvalClassDisplay');
    if (input) { input.value = ''; input.dataset.classId = ''; }
    if (display) { display.textContent = '반 목록에서 선택하거나 검색하세요'; display.classList.add('empty'); }
    try { adminDailyEvalRows = []; global.adminDailyEvalRows = []; } catch (_ignore) {}
    const wrap = document.getElementById('adminDailyEvalTableWrap');
    if (wrap) wrap.innerHTML = '';
    const link = document.getElementById('adminDailyClassVideoLink7355021');
    if (link) link.value = '';
  }

  async function loadDailyClassList7355026(dateOverride, force) {
    const date = text(dateOverride || (document.getElementById('adminDailyEvalDate') || {}).value) || seoulDateString();
    const data = await call('getStaffClassListOperationalSnapshot', {
      date,
      force: force === true,
      requestId: requestId('daily-class-list-7355026')
    });
    const classes = Array.isArray(data.classes) ? data.classes : [];
    if (typeof global.adminRememberDailyServerClassList7355026_ === 'function') {
      global.adminRememberDailyServerClassList7355026_(date, classes);
    } else if (typeof global.adminRememberClassListForDate704_ === 'function') {
      global.adminRememberClassListForDate704_(date, classes);
    }

    const dailyDate = text((document.getElementById('adminDailyEvalDate') || {}).value);
    if (dailyDate !== date) return classes;
    ...
    try {
      ...
      if (typeof global.adminClearInvalidClassSelection704_ === 'function') global.adminClearInvalidClassSelection704_(date);
    } catch (_ignore) {}
    syncDailyVideoLinkInput7355026();
    return classes;
  }
  global.ulimLoadDailyClassList7355026 = loadDailyClassList7355026;

  function installDailyVideoLinkSync7355026() {
    const panel = document.getElementById('adminPanelDailyEval');
    if (!panel || panel.dataset.ulimVideoLinkSync7355026 === '1') return;
    panel.dataset.ulimVideoLinkSync7355026 = '1';
    const date = document.getElementById('adminDailyEvalDate');
    if (date) date.addEventListener('change', function () {
      resetDailyClassSelection7355026();
      Promise.resolve(loadDailyClassList7355026(date.value, true)).catch(function(error) {
        setDailyVideoStatus7355021(text(error && error.message) || '반 목록을 불러오지 못했습니다.', false);
      });
    });
  }
  ...
  async function install() {
    ...
    installOperationalDataOverrides(); installMessageOverrides(); installAttendancePreview7355021(); installDailyVideoLinkSync7355026(); installShowPanelHook(); installStaffNotes();
    global.__ULIM_ATTENDANCE_AUTO_REFRESH_DISABLED_7355014__ = true;
