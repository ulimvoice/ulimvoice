(function (global) {
  ...
  function credentialLabel(account) {
    if (account.passwordLoginEnabled === true && account.mustChangePassword !== true) return '<span class="ulim-credential ok">로그인 가능</span>';
    if (account.passwordLoginEnabled === true) return '<span class="ulim-credential wait">임시암호 변경 대기</span>';
    return '<span class="ulim-credential fail">로그인 암호 미등록</span>';
  }
  function isIncompleteAccount(account) {
    return !text(account && account.loginId) && !text(account && account.name);
  }
  function incompleteLabel(account) {
    return isIncompleteAccount(account) ? '<span class="ulim-incomplete-badge">불완전 계정</span>' : '';
  }
  ...
  function render() {
    ...
    const rows = accounts.map(function (account) {
      ...
      const incomplete = isIncompleteAccount(account);
      const rowClasses = [account.active === true ? '' : 'ulim-staff-account-disabled', incomplete ? 'ulim-staff-account-incomplete' : ''].filter(Boolean).join(' ');
      ...
      return `<tr${rowClassAttr}>
        ...
        <td><div class="ulim-staff-account-actions">
          <button type="button" class="admin-btn blue" onclick="ulimStaffAccountSave7342('${key}')">${incomplete ? '복구 저장' : '저장'}</button>
          <button type="button" class="admin-btn orange" onclick="ulimStaffAccountResetPassword7342('${key}')"${incomplete ? ' disabled title="먼저 복구 저장을 완료해주세요."' : ''}>암호 초기화</button>
          <button type="button" class="admin-btn red" onclick="ulimStaffAccountRetire7342('${key}')">${incomplete ? '완전 삭제' : '퇴사처리'}</button>
        </div></td>
      </tr>`;
    }).join('');
  ...
  function accountFromDoc(doc) {
    ...
    return {
      ...
      incomplete: !text(user.loginId || user.adminId || user.legacyAdminId) && !text(user.name || user.displayName || user.teacherName || user.adminName)
    };
  }
  ...
  function panelIsActive() {
    const panel = document.getElementById('adminPanelStaffAccounts');
    return !!(panel && panel.classList.contains('active'));
  }
  function shouldSkipAutomaticRefresh() {
    if (!panelIsActive()) return true;
    if (rowEditDirty) return true;
    if (listLoadingPromise) return true;
    return accounts.length > 0 && Date.now() - lastListLoadedAt < AUTO_REFRESH_MIN_INTERVAL_MS;
  }
  function queueAutomaticRefresh(reason) {
    if (autoRefreshQueued || shouldSkipAutomaticRefresh()) return;
    autoRefreshQueued = true;
    setTimeout(function () {
      autoRefreshQueued = false;
      if (shouldSkipAutomaticRefresh()) return;
      refreshFirestoreOnly({ automatic: true, reason: reason || 'auto' }).catch(function () {});
    }, 120);
  }
  async function refreshFirestoreOnly(options) {
    ...
    if (opts.automatic === true && shouldSkipAutomaticRefresh()) {
      return { ok: true, skipped: true, reason: rowEditDirty ? 'editing' : 'fresh-or-busy', accounts: accounts };
    }
    if (listLoadingPromise) return listLoadingPromise;
    if (rowEditDirty && opts.force !== true) {
      status('교직원 정보를 수정 중이므로 자동 새로고침을 멈췄습니다. 저장하거나 “Firestore만 다시조회”를 누르면 다시 불러옵니다.', 'warn');
      return { ok: true, skipped: true, reason: 'editing', accounts: accounts };
    }
    const run = (async function () {
      if (opts.keepStatus !== true && opts.automatic !== true) status('Firestore 교직원 목록을 불러오는 중...', 'loading');
      const listResult = await listAccounts();
      lastListLoadedAt = Date.now();
      if (opts.keepStatus !== true && opts.automatic !== true) {
      ...
      try { console.error('[ULIM 7.34.3.6 staff list] failed', error); } catch (_ignore) {}
      throw error;
    }).finally(function () {
  ...
  async function syncDirectoryFromSheet() {
    ...
    const run = (async function () {
      ...
      try { console.info('[ULIM 7.34.3.5 staff sheet sync] click accepted'); } catch (_ignore) {}
      ...
      try {
        ...
        try { console.info('[ULIM 7.34.3.5 staff sheet sync] callable complete', { total: Number(syncResult && syncResult.total || 0), created: Number(syncResult && syncResult.created || 0), failed: Number(syncResult && syncResult.failed || 0) }); } catch (_ignore) {}
      } catch (error) {
        try { console.error('[ULIM 7.34.3.5 staff sheet sync] callable failed', error); } catch (_ignore) {}
      ...
      try { console.error('[ULIM 7.34.3.5 staff sheet sync] unexpected failure', error); } catch (_ignore) {}
      return false;
    }).finally(function () {
  ...
  async function load(syncFromSheet) {
    return syncFromSheet === true ? syncDirectoryFromSheet() : refreshFirestoreOnly({ automatic: true, reason: 'legacy-load' });
  }
  ...
  async function save(key) {
    ...
    const account = accounts.find(function (item) { return text(item.firebaseUid) === text(data.firebaseUid); }) || {};
    const incomplete = isIncompleteAccount(account);
    if (!data.firebaseUid || !data.loginId || !data.name) return alert(incomplete ? '복구할 이름과 로그인 ID를 모두 입력해주세요.' : '이름과 로그인 ID를 확인해주세요.');

    if (incomplete) {
      const first = prompt('공백 계정을 복구합니다.\n' + data.name + ' 계정에서 사용할 임시 비밀번호를 입력해주세요.\n6자 이상');
      if (!first) return;
      if (first.length < 6) return alert('비밀번호는 6자 이상이어야 합니다.');
      const second = prompt('임시 비밀번호를 한 번 더 입력해주세요.');
      if (first !== second) return alert('비밀번호가 서로 다릅니다.');
      if (!confirm('UID ' + maskedUid(data.firebaseUid) + ' 공백 계정을 ' + data.name + '(' + data.loginId + ') 계정으로 복구할까요?')) return;
      data.repairIncomplete = true;
      data.repairPassword = first;
      data.mustChangePassword = true;
    } else if (!confirm(data.name + ' 계정 정보를 저장할까요?\n권한이나 상태 변경 시 해당 계정은 다시 로그인해야 합니다.')) {
      return;
    }

    try {
      showLoading(incomplete ? '불완전 교직원 계정 복구 중...' : '교직원 계정 저장 중...');
      const result = await call('updateStaffAccountAdmin', data);
      rowEditDirty = false;
      await refreshFirestoreOnly({ force: true, reason: 'save' });
      const state = result && result.legacySync && result.legacySync.state;
      if (result && result.repairedIncomplete === true) {
        alert(state === 'complete' ? '공백 계정이 복구되었습니다. 새 임시 비밀번호로 로그인할 수 있습니다.' : 'Firebase 계정 복구는 완료되었습니다. Google Sheets 호환 비밀번호 반영은 해당 계정에서 다시 암호 초기화를 실행해 확인해주세요.');
      } else {
        alert(state === 'complete' ? '교직원 계정이 저장되었습니다.' : 'Firebase 계정은 저장되었고 Google Sheets 호환 반영은 백그라운드에서 재시도합니다.');
      }
    } catch (error) {
      alert(text(error && error.message) || (incomplete ? '불완전 계정 복구에 실패했습니다.' : '교직원 계정 저장에 실패했습니다.'));
    } finally { hideLoading(); }
  }
  async function resetPassword(key) {
    const data = rowData(key);
    const account = accounts.find(function (item) { return text(item.firebaseUid) === text(data.firebaseUid); }) || {};
    if (isIncompleteAccount(account)) return alert('공백 계정은 먼저 이름과 로그인 ID를 입력한 뒤 복구 저장을 완료해주세요.');
    ...
    try {
      ...
      rowEditDirty = false;
      await refreshFirestoreOnly({ force: true, reason: 'password-reset' });
  ...
  async function retire(key) {
    ...
    if (incomplete) {
      const suffix = text(data.firebaseUid).slice(-6);
      const entered = prompt('이 계정은 이름과 로그인 ID가 모두 없는 불완전 계정입니다.\nFirebase Auth 및 교직원 프로필만 완전히 삭제하고 기존 출석·평가 작성 기록은 건드리지 않습니다.\n삭제하려면 화면에 표시된 UID 끝 6자리 [' + suffix + ']를 입력해주세요.');
      if (!entered) return;
      if (text(entered).toLowerCase() !== suffix.toLowerCase()) return alert('UID 끝 6자리가 일치하지 않습니다.');
      if (!confirm('UID ' + maskedUid(data.firebaseUid) + ' 불완전 계정을 완전히 삭제할까요?\n이 작업은 되돌릴 수 없습니다.')) return;
      payload = { firebaseUid: data.firebaseUid, purgeIncomplete: true, confirmUidSuffix: entered };
    } else {
      const confirmId = prompt(data.name + ' 계정을 퇴사 처리합니다.\n작성 기록은 보존되고 로그인은 차단됩니다.\n확인을 위해 로그인 ID를 입력해주세요.');
      if (!confirmId) return;
      payload = { firebaseUid: data.firebaseUid, confirmLoginId: confirmId };
    }

    try {
      showLoading(incomplete ? '불완전 계정 완전 삭제 중...' : '퇴사 처리 중...');
      const result = await call('deleteStaffAccountAdmin', payload);
      rowEditDirty = false;
      await refreshFirestoreOnly({ force: true, reason: 'retire' });
      alert(result && result.purgedIncomplete === true ? '불완전 공백 계정이 완전히 삭제되었습니다.' : '퇴사 처리가 완료되었습니다.');
    } catch (error) {
      alert(text(error && error.message) || (incomplete ? '불완전 계정 삭제에 실패했습니다.' : '퇴사 처리에 실패했습니다.'));
    } finally { hideLoading(); }
  }
  function installPanelHook() {
    ...
    if (typeof original === 'function' && !original.__ulim7342Wrapped) {
      const wrapped = function (panelId) {
        ...
        if (panelId === 'adminPanelStaffAccounts') queueAutomaticRefresh('panel-open');
        return result;
      };
  ...
  function install() {
    ...
    if (staffPanel && !staffPanel.dataset.ulimEditGuard73436) {
      staffPanel.dataset.ulimEditGuard73436 = '1';
      const markDirty = function (event) {
        const target = event && event.target;
        const id = text(target && target.id);
        if (!/^u_.*_(name|login|phone|role|active|must)$/.test(id)) return;
        rowEditDirty = true;
        status('교직원 정보를 수정 중입니다. 입력값 보호를 위해 자동 새로고침을 일시정지했습니다.', 'warn');
      };
    ...
    global.addEventListener('ulim-staff-password-changed', function () {
      if (!panelIsActive()) return;
      rowEditDirty = false;
      lastListLoadedAt = 0;
      refreshFirestoreOnly({ force: true, reason: 'password-changed' }).catch(function () {});
    });
    ...
    try {
      ...
      console.info('[ULIM 7.34.3.6 CLEAN START] edit-safe single refresh + password status labels');
    } catch (_ignore) {}
  }
