(function (global) {
  'use strict';
  if (global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_7355024__) return;
  ...
  const VERSION = '2026-08-08.735.05.0.24-daily-persistence-pc-picker-fix';
  ...
  function applyVerifiedClassList7355024(date, classes) {
    const list = Array.isArray(classes) ? classes : [];
    try {
      if (typeof global.adminRememberClassListForDate704_ === 'function') global.adminRememberClassListForDate704_(date, list);
      adminClassList = list;
      global.adminClassList = list;
      if (typeof global.adminRenderClassSelectors === 'function') global.adminRenderClassSelectors();
      else if (typeof adminRenderClassSelectors === 'function') adminRenderClassSelectors();
    } catch (_ignore) {}
  }
  ...
  async function saveDailyClassVideoLink7355024() {
    ...
    try {
      ...
      applyVerifiedClassList7355024(selected.date, classes);
  ...
  function syncDailyVideoLinkInput7355024() {
    const input = document.getElementById('adminDailyClassVideoLink7355021');
    if (!input) return;
    const selected = selectedDailyClass7355024();
    let value = '';
    ...
    input.value = value;
    setDailyVideoStatus7355021('반별 영구 저장값입니다. 평가 저장 시에도 같은 값이 함께 확인됩니다.', true);
  }
  global.ulimSyncDailyClassVideoLink7355024 = syncDailyVideoLinkInput7355024;

  function installDailyVideoLinkSync7355024() {
    const panel = document.getElementById('adminPanelDailyEval');
    if (!panel || panel.dataset.ulimVideoLinkSync7355024 === '1') return;
    panel.dataset.ulimVideoLinkSync7355024 = '1';
    const date = document.getElementById('adminDailyEvalDate');
    if (date) date.addEventListener('change', function () { setTimeout(syncDailyVideoLinkInput7355024, 80); });
  }
  ...
  async function install() {
    ...
    installOperationalDataOverrides(); installMessageOverrides(); installAttendancePreview7355021(); installDailyVideoLinkSync7355024(); installShowPanelHook(); installStaffNotes();
    global.__ULIM_ATTENDANCE_AUTO_REFRESH_DISABLED_7355014__ = true;
