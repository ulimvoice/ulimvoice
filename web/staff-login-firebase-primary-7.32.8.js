(function (global) {
  ...
  if (global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7328__) return;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7328__ = true;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7327__ = true;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7326__ = true;

  const VERSION = '2026-08-09.732.08-claims-profile-no-client-user-read';
  ...
  function safeConsole() {
    // 운영 배포본에서는 앱 내부 로그인 정보를 콘솔에 출력하지 않습니다.
    return undefined;
  }

  function timeoutError(message) {
    const error = new Error(message || '로그인 처리 시간이 초과되었습니다.');
    error.code = 'ulim/login-timeout';
    return error;
  }
  ...
  function withTimeout(promise, timeoutMs, message) {
    ...
    const timeout = new Promise(function (_resolve, reject) {
      timer = setTimeout(function () { reject(timeoutError(message)); }, Math.max(1000, Number(timeoutMs) || 15000));
    });
  ...
  function roomRealtime() {
    return global.ULIM_ROOM_CLASSROOM_REALTIME_72917 ||
  ...
  async function runtime() {
    ...
    if (!room || typeof room.preloadRuntime !== 'function') {
      throw new Error('로그인 기능을 준비하지 못했습니다. 페이지를 다시 열어주세요.');
    }
    const rt = await room.preloadRuntime();
    if (!rt || !rt.sdk || !rt.auth || !rt.db || !rt.functions) {
      throw new Error('로그인 기능 초기화에 실패했습니다. 페이지를 다시 열어주세요.');
    }
    return rt;
  }
  ...
  async function deriveLoginEmail(loginId) {
    ...
    if (!global.crypto || !global.crypto.subtle || typeof TextEncoder === 'undefined') {
      throw new Error('현재 브라우저에서는 안전한 로그인을 사용할 수 없습니다.');
    }
  ...
  function readStoredFirebaseProfile7328() {
    var candidates = [];
    try {
      if (global.adminInfo && typeof global.adminInfo === 'object') candidates.push(global.adminInfo);
    } catch (_ignore1) {}
    try {
    ...
    try {
      var rawAdmin = localStorage.getItem('adminInfo') || sessionStorage.getItem('adminInfo') || '';
      if (rawAdmin) {
        var parsedAdmin = JSON.parse(rawAdmin);
        if (parsedAdmin && typeof parsedAdmin === 'object') candidates.push(parsedAdmin);
      }
    } catch (_ignore3) {}
  ...
  async function readCurrentStaffProfile(rt, fallbackId) {
    ...
    if (!user) throw new Error('로그인 사용자를 확인하지 못했습니다.');
    const room = roomRealtime();
    ...
    if (!(typeof authVersion === 'number' || (typeof authVersion === 'string' && text(authVersion)))) {
      throw new Error('계정 인증정보를 확인하지 못했습니다. 다시 로그인해주세요.');
    }
    if (role === 'teacher' && !text(claims.teacherUid)) {
      throw new Error('강사 계정 정보가 일치하지 않습니다.');
    }

    /*
     * 7.32.8:
     * users/{uid} is not a browser-readable authority. Firestore Rules may
     * intentionally deny the client read even though Firebase Auth and every
     * protected Callable are valid. The staff shell therefore derives its
     * display/session profile from the signed ID-token claims plus the profile
     * already saved by a previous successful login.
     *
     * Protected data remains server-authorized: all operational Callables
     * revalidate users/{uid}, active/role/authVersion (and teacherUid) with the
     * Admin SDK before returning or mutating data.
     */
    const stored = readStoredFirebaseProfile7328();
    const storedUid = text(stored.firebaseAuthUid || stored.principalUidV2);
    const safeStored = !storedUid || storedUid === text(user.uid) ? stored : {};
    const id = text(
      fallbackId ||
      claims.loginId ||
      claims.adminId ||
      claims.legacyAdminId ||
      safeStored.id ||
      safeStored.loginId
    );
    const name = text(
      ...
      user.displayName ||
      safeStored.name ||
      safeStored.displayName ||
      safeStored.teacherName ||
      safeStored.adminName ||
      id
    );
    return {
      id: id,
      ...
      role: roleLabel(role),
      firebaseRole: role,
      phone: text(safeStored.phone || claims.phone || ''),
      mustChangePassword: safeStored.mustChangePassword === true,
      accountUid: text(claims.accountUid || claims.teacherUid || safeStored.accountUid || safeStored.teacherUid || user.uid),
      principalUidV2: text(claims.principalUidV2 || safeStored.principalUidV2 || user.uid),
      firebaseAuthUid: user.uid,
      permissions: safeStored.permissions || claims.permissions || null,
      passwordLoginEnabled: safeStored.passwordLoginEnabled !== false,
  ...
  function showStaffShell() {
    ...
    /*
     * 7.32.5: 기존 UI 패치들은 adminToken이 생길 때까지 교직원 탭을 잠급니다.
  ...
  function dispatchAuthReady(rt, reason) {
    try {
      const user = rt && rt.auth && rt.auth.currentUser;
      const current = global.adminInfo && typeof global.adminInfo === 'object' ? global.adminInfo : {};
      global.dispatchEvent(new CustomEvent('ulim-firebase-auth-ready', {
        detail: {
          reason: reason || 'staff-login',
          version: VERSION,
          uid: text(user && user.uid),
          role: text(current.firebaseRole || current.role),
          accountUid: text(current.accountUid || current.principalUidV2)
        }
      }));
    } catch (_ignore) {}
  }
  ...
  function verifyLegacyResponseOwner(data, firebaseUid, loginId) {
    ...
    if (expectedUid && expectedUid !== firebaseUid) {
      throw new Error('계정 식별정보가 일치하지 않습니다.');
    }
    const returnedId = normalizeLoginId(admin.id || '');
    if (returnedId && returnedId !== normalizeLoginId(loginId)) {
      throw new Error('로그인 ID가 계정정보와 일치하지 않습니다.');
    }
  }

  async function migratePasswordCredential(rt, loginId, password, migrationProof, markPasswordChanged) {
    if (!migrationProof) throw new Error('최근 비밀번호 확인정보를 받지 못했습니다. 서버 배포 상태를 확인해주세요.');
    const callable = rt.sdk.httpsCallable(rt.functions, 'migrateCurrentStaffPasswordCredential');
    const result = await callable({ loginId: loginId, password: password, migrationProof: migrationProof, markPasswordChanged: markPasswordChanged === true });
    const data = result && result.data || {};
    if (!data.ok) throw new Error('비밀번호 로그인 전환에 실패했습니다.');
    ...
    safeConsole('info', '[ULIM 7.32.5 password credential migrated]', { version: data.version || VERSION });
    ...
    legacyCompatibilityPromise = Promise.resolve()
      ...
      .then(async function (data) {
        ...
        try {
          ...
          safeConsole('warn', '[ULIM 7.32.5 legacy initial data]', error);
        }
        try {
        ...
        safeConsole('info', '[ULIM 7.32.5 GAS compatibility ready]', {
      ...
      .catch(function (error) {
        ...
        safeConsole('warn', '[ULIM 7.32.5 GAS compatibility failed]', error);
        throw error;
      })
  ...
  function friendlyLoginError(error) {
    ...
    if (code.indexOf('network-request-failed') >= 0) return '로그인 서버 연결에 실패했습니다. 인터넷 연결을 확인해주세요.';
    if (code.indexOf('operation-not-allowed') >= 0) return '이 계정의 비밀번호 로그인이 활성화되지 않았습니다.';
  ...
  async function directFirebaseLogin(rt, loginId, password, startedAt) {
    ...
    await withTimeout(applyAdminFirebasePersistence(rt), 8000, '로그인 저장방식을 준비하지 못했습니다.');
    const email = await deriveLoginEmail(loginId);
    if (rt.auth.currentUser) {
      try { await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
    }
    const credential = await withTimeout(
      rt.sdk.signInWithEmailAndPassword(rt.auth, email, password),
      15000,
      '로그인 응답이 지연되고 있습니다. 잠시 후 다시 시도해주세요.'
    );
    {
      const room = roomRealtime();
      if (room && typeof room.resetStableTokenGuard === 'function') room.resetStableTokenGuard(credential.user.uid);
      if (room && typeof room.getStableIdToken === 'function') {
        await withTimeout(room.getStableIdToken(rt, false, 'password-signin'), 10000, '로그인 확인 시간이 초과되었습니다.');
      } else {
        await withTimeout(rt.sdk.getIdToken(credential.user, false), 10000, '로그인 확인 시간이 초과되었습니다.');
      }
    }
    const profile = await withTimeout(
      readCurrentStaffProfile(rt, loginId),
      12000,
      '교직원 계정정보 확인 시간이 초과되었습니다.'
    );
    primaryCredentialReady = true;
    ...
    safeConsole('info', '[ULIM 7.32.5 Firebase-primary shell displayed]', { elapsedMs: Date.now() - startedAt });
  ...
  async function legacyFirstLogin(rt, loginId, password, startedAt) {
    ...
    const data = await withTimeout(adminApiFn('adminLogin', {
      adminId: loginId,
      adminPw: password,
      noCache: 1,
      loginFastVersion: VERSION
    }), 18000, '기존 계정 확인 시간이 초과되었습니다.');
    saveLegacySession(data);
    feedBundledProof(data);

    const room = roomRealtime();
    const authenticated = room && typeof room.forceReauthenticate === 'function'
      ? await withTimeout(room.forceReauthenticate('staff-first-migration'), 15000, '교직원 인증 시간이 초과되었습니다.')
      : rt;
    if (!authenticated || !authenticated.auth || !authenticated.auth.currentUser) {
      throw new Error('교직원 인증에 실패했습니다.');
    }

    const profile = await withTimeout(readCurrentStaffProfile(authenticated, loginId), 12000, '교직원 계정정보 확인 시간이 초과되었습니다.');
    lastLoginId = profile.id || loginId;
    saveFirebaseProfile(Object.assign({}, data.admin || {}, profile));
    await domReadyPromise;
    showStaffShell();
    setCompatibilityStatus('로그인 정보를 정리하는 중...', 'loading');
    activateFirestoreStaffData(authenticated, 'legacy-first-login');
    try { if (typeof global.hideLoading === 'function') global.hideLoading(); } catch (_ignore) {}

    safeConsole('info', '[ULIM 7.32.5 legacy first shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      serverTiming: data && data.loginTiming || null
    });
    ...
    if (data && data.admin && data.admin.mustChangePassword) {
      ...
      setTimeout(function () {
        migratePasswordCredential(authenticated, loginId, password, text(data && data.firebasePasswordMigrationProof))
          .then(function () { setCompatibilityStatus('', 'ready'); })
          .catch(function (error) {
            setCompatibilityStatus('', 'error');
            safeConsole('warn', '[ULIM 7.32.5 credential migration deferred]', error);
          });
      }, 0);
    }
  ...
  async function firebasePrimaryAdminLogin() {
    ...
    loginPromise = (async function () {
      ...
      try { if (typeof global.showLoading === 'function') global.showLoading('로그인 중...'); } catch (_ignore) {}
      const rt = await withTimeout(
        runtime(),
        15000,
        '로그인 기능 준비 시간이 초과되었습니다. 페이지를 다시 연 뒤 시도해주세요.'
      );

      try {
        return await withTimeout(
          directFirebaseLogin(rt, loginId, password, startedAt),
          30000,
          '로그인 처리 시간이 초과되었습니다. 잠시 후 다시 시도해주세요.'
        );
      } catch (loginError) {
        if (isDisabledError(loginError) || !mayUseLegacyFallback(loginError)) throw loginError;
        try { if (rt.auth.currentUser) await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
        return withTimeout(
          legacyFirstLogin(rt, loginId, password, startedAt),
          35000,
          '기존 계정 로그인 처리 시간이 초과되었습니다.'
        );
      }
    })().catch(function (error) {
      try { if (typeof global.hideLoading === 'function') global.hideLoading(); } catch (_ignore) {}
  ...
  async function firebasePrimaryPasswordChange(force, knownCurrentPassword) {
    ...
    try {
      ...
      await migratePasswordCredential(rt, loginId, newPassword, text(data && data.firebasePasswordMigrationProof), true);
      const profile = await readCurrentStaffProfile(rt, loginId);
      profile.mustChangePassword = false;
      profile.passwordLoginEnabled = true;
      saveFirebaseProfile(Object.assign({}, data.admin || {}, profile));
      try { global.dispatchEvent(new CustomEvent('ulim-staff-password-changed', { detail: { version: VERSION } })); } catch (_ignore) {}
      setCompatibilityStatus('비밀번호와 로그인 정보가 갱신되었습니다.', 'ready');
  ...
  function installLogoutWrapper() {
    ...
    const wrapped = async function () {
      ...
      try {
        ...
        safeConsole('warn', '[ULIM 7.32.5 Firebase signOut delayed]', error);
      }
      try {
        return await original.apply(this, arguments);
      } finally {
        /* signOut가 네트워크 오류로 지연돼도 자동복원은 로그아웃 플래그로 계속 차단됩니다. */
        safeConsole('info', '[ULIM 7.32.5 explicit staff logout]', { signOutCompleted: signOutCompleted });
      }
    };
  ...
  async function validateStoredLegacySessionQuietly() {
    ...
    try {
      ...
      if (/세션.*(만료|유효하지)|로그인.*필요|다시.*로그인|토큰.*(만료|유효하지)/i.test(message)) {
        clearLegacySession();
        setCompatibilityStatus('일부 기존 기능 연결은 다음 로그인 때 복구됩니다.', 'error');
      ...
      if (!(error && error.silent)) safeConsole('warn', '[ULIM 7.32.5 stored GAS session check delayed]', error);
    }
    return false;
  }
  ...
  async function restoreFirebaseStaffSession() {
    ...
    try {
      ...
      rt.sdk.onAuthStateChanged(rt.auth, async function (user) {
        ...
        try {
          const profile = await withTimeout(readCurrentStaffProfile(rt, ''), 12000, '로그인 상태 확인 시간이 초과되었습니다.');
          ...
          safeConsole('info', '[ULIM 7.32.5 Firebase-primary restored]', {
          ...
          safeConsole('warn', '[ULIM 7.32.5 stored Firebase session rejected]', error);
          try { await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
        }
      });
    } catch (error) {
      restoreListenerInstalled = false;
      safeConsole('warn', '[ULIM 7.32.5 restore unavailable]', error);
    }
  }

  let invalidTokenHandling7325 = false;
  async function handleInvalidFirebaseToken7325(event) {
    if (invalidTokenHandling7325 || explicitLogoutActive()) return;
    invalidTokenHandling7325 = true;
    ...
    try {
      ...
      try {
        if (!global.__ULIM_TOKEN_INVALID_ALERT_7325__) {
          global.__ULIM_TOKEN_INVALID_ALERT_7325__ = true;
          alert('로그인 세션이 만료되었습니다. 다시 로그인해주세요.');
          setTimeout(function () { global.__ULIM_TOKEN_INVALID_ALERT_7325__ = false; }, 1000);
        }
      } catch (_ignore) {}
      invalidTokenHandling7325 = false;
    }
  }
  ...
  function install() {
    ...
    if (!global.__ULIM_FIREBASE_TOKEN_INVALID_LISTENER_7325__) {
      global.__ULIM_FIREBASE_TOKEN_INVALID_LISTENER_7325__ = true;
      global.addEventListener('ulim-firebase-token-invalid', handleInvalidFirebaseToken7325);
    }
    if (!global.__ULIM_LEGACY_SESSION_STATUS_7325__) {
      global.__ULIM_LEGACY_SESSION_STATUS_7325__ = true;
      global.addEventListener('ulim-legacy-session-expired', function () {
        setCompatibilityStatus('일부 기존 기능 연결이 만료되었습니다. 다시 로그인해주세요.', 'error');
      });
      global.addEventListener('ulim-legacy-session-ready', function () {
        setCompatibilityStatus('연습일지·연습실 예약 호환 연결됨', 'ready');
      });
    }
    safeConsole('info', '[ULIM 7.32.5 Firebase-primary staff login installed]', VERSION);
  }
  ...
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7328_API__ = Object.freeze({
    version: VERSION,
