(function (global) {
  'use strict';
  if (global.__ULIM_ATTENDANCE_DAILY_UI_734310__) return;
  global.__ULIM_ATTENDANCE_DAILY_UI_734310__ = true;

  const VERSION = '2026-08-01.734.0310';
  ...
  function isLegacyAutoCurrentStatus(value) {
  ...
  function currentStatus(record) {
    ...
    if (direct) return isLegacyAutoCurrentStatus(direct) ? '' : direct;

    // 구버전에서 특이상태 칸으로 잘못 섞인 일반 비고만 임시 호환합니다.
    const legacy = text(record && record.specialStatus);
    if (isLegacyAutoCurrentStatus(legacy)) return '';
  ...
  function renderAttendanceStable(force) {
    ...
    records.forEach(function (record, idx) {
      ...
      html += '<tr data-att-index="' + idx + '">' +
        ...
        '</div></td>' +
        '<td data-label="현재상태"><input class="admin-small-input ulim-current-status-input-734310" data-field="currentStatus" maxlength="500" value="' + esc(current) + '" placeholder="강사 입력사항"></td>' +
    ...
    if (input.matches('#adminAttendanceTableWrap input[data-field="currentStatus"]')) {
      ...
      if (row) {
        ...
        if (records[idx]) {
          ...
          records[idx].__currentStatusDirty734310 = true;
        }
      }
      return;
    }
  ...
  function clearCurrentStatusDirtyFlags() {
    attendanceRecords().forEach(function (record) {
      if (!record) return;
      record.currentStatusDirty = false;
      record.remarkDirty = false;
      record.sheetRemarkDirty = false;
      record.__currentStatusDirty734310 = false;
    });
    document.querySelectorAll('#adminAttendanceTableWrap tr[data-att-index]').forEach(function (row) {
      delete row.dataset.currentStatusDirty;
      const input = row.querySelector('input[data-field="currentStatus"]');
  ...
  style.id = 'ulim-attendance-daily-ui-734310-style';
  style.textContent = [
    ...
    '#adminAttendanceTableWrap .ulim-current-status-input-734310{width:100%;min-width:150px;box-sizing:border-box;}',
