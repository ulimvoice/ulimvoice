(function (global) {
  'use strict';
  if (global.__ULIM_PRACTICE_ROOM_FIRESTORE_PRIMARY_7355036__) return;

  const VERSION = '2026-08-11.7355036-practice-room-admin-calendar-group-decision';
  const API = {};
  ...
  API.adminDecision = function (reservationId, actionType, option) {
    ...
    return call('decidePracticeRoomReservationAdmin7355033', {
      ...
      requestId:requestId('room-decision-7355036')
    });
  };
  API.adminDecisionGroup = function (reservationIds, actionType, option) {
    ...
    return call('decidePracticeRoomReservationAdmin7355033', {
      ...
      newTime:text(option.newTime),
      targets:Array.isArray(option.targets) ? option.targets.slice(0, 2) : [],
      requestId:requestId('room-decision-group-7355036')
    });
  };
  API.listAdmin = function (date) {
    return call('listRoomReservationsAdmin73550', { date:text(date), requestId:requestId('room-admin-list-7355036') });
  };
  API.listAdminMonth = function (year, month) {
    return call('listRoomReservationsAdmin73550', { year:Number(year), monthNumber:Number(month), requestId:requestId('room-admin-month-7355036') });
  };
  ...
  global.__ULIM_PRACTICE_ROOM_FIRESTORE_PRIMARY_7355036__ = true;
