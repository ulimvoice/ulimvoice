(function (global) {
  ...
  if (global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7320__) return;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7320__ = true;

  const VERSION = '2026-07-31.732.00';
  ...
  async function migratePasswordCredential(rt, loginId, password, migrationProof) {
    ...
    safeConsole('info', '[ULIM 7.32.0 password credential migrated]', {
    ...
    legacyCompatibilityPromise = Promise.resolve()
      ...
      .then(async function (data) {
        ...
        try {
          ...
          safeConsole('warn', '[ULIM 7.32.0 legacy initial data]', error);
        }
        try {
        ...
        safeConsole('info', '[ULIM 7.32.0 GAS compatibility ready]', {
      ...
      .catch(function (error) {
        ...
        safeConsole('warn', '[ULIM 7.32.0 GAS compatibility failed]', error);
        throw error;
      })
  ...
  async function directFirebaseLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.0 Firebase-primary shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      uid: credential.user.uid,
  ...
  async function legacyFirstLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.0 legacy first shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      serverTiming: data && data.loginTiming || null
    });
      ...
      setTimeout(function () {
        migratePasswordCredential(authenticated, loginId, password, text(data && data.firebasePasswordMigrationProof))
          ...
          .catch(function (error) {
            ...
            safeConsole('warn', '[ULIM 7.32.0 credential migration deferred]', error);
          });
      }, 0);
    }
  ...
  async function firebasePrimaryAdminLogin() {
    ...
    loginPromise = (async function () {
      ...
      try {
        ...
        safeConsole('info', '[ULIM 7.32.0 legacy migration fallback]', text(firebaseError && firebaseError.code));
    ...
    try {
      ...
      rt.sdk.onAuthStateChanged(rt.auth, async function (user) {
        ...
        try {
          ...
          safeConsole('info', '[ULIM 7.32.0 Firebase-primary restored]', { uid: user.uid, role: profile.firebaseRole });
        } catch (error) {
          safeConsole('warn', '[ULIM 7.32.0 stored Firebase session rejected]', error);
          try { await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
        }
      });
    } catch (error) {
      safeConsole('warn', '[ULIM 7.32.0 restore unavailable]', error);
    }
  }
  ...
  function install() {
    ...
    safeConsole('info', '[ULIM 7.32.0 Firebase-primary staff login installed]', VERSION);
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install, { once: true });
  else install();

  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7320_API__ = Object.freeze({
    version: VERSION,
