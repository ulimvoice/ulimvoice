(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72910__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72910__ = true;

  const VERSION = "2026-07-31.729.10";
  ...
  function acceptLoginProof72910(bundle) {
  ...
  function consumeLoginProof72910(session) {
  ...
  async function requestFreshProof(session) {
    const bundledProof = consumeLoginProof72910(session);
    ...
    if (state.signingIn) {
      try {
        ...
        if (existing && existing.auth && existing.auth.currentUser) {
          await existing.sdk.getIdToken(existing.auth.currentUser, true);
          return existing;
        }
      } catch (_ignore) {}
    }
  ...
  const publicApi728 = Object.freeze({
    ...
    acceptLoginProof: acceptLoginProof72910,
