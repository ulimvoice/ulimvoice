(function (global) {
  ...
  if (global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7321__) return;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7321__ = true;

  const VERSION = '2026-07-31.732.01';
  ...
  function showStaffShell() {
    ...
    /*
     * 7.32.1: 기존 UI 패치들은 adminToken이 생길 때까지 교직원 탭을 잠급니다.
  ...
  async function migratePasswordCredential(rt, loginId, password, migrationProof) {
    ...
    safeConsole('info', '[ULIM 7.32.1 password credential migrated]', {
    ...
    legacyCompatibilityPromise = Promise.resolve()
      ...
      .then(async function (data) {
        ...
        try {
          ...
          safeConsole('warn', '[ULIM 7.32.1 legacy initial data]', error);
        }
        try {
        ...
        safeConsole('info', '[ULIM 7.32.1 GAS compatibility ready]', {
      ...
      .catch(function (error) {
        ...
        safeConsole('warn', '[ULIM 7.32.1 GAS compatibility failed]', error);
        throw error;
      })
  ...
  async function directFirebaseLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.1 Firebase-primary shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      uid: credential.user.uid,
  ...
  async function legacyFirstLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.1 legacy first shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      serverTiming: data && data.loginTiming || null
    });
      ...
      setTimeout(function () {
        migratePasswordCredential(authenticated, loginId, password, text(data && data.firebasePasswordMigrationProof))
          ...
          .catch(function (error) {
            ...
            safeConsole('warn', '[ULIM 7.32.1 credential migration deferred]', error);
          });
      }, 0);
    }
  ...
  async function firebasePrimaryAdminLogin() {
    ...
    loginPromise = (async function () {
      ...
      try {
        ...
        safeConsole('info', '[ULIM 7.32.1 legacy migration fallback]', text(firebaseError && firebaseError.code));
  ...
  function installLogoutWrapper() {
    ...
    if (typeof original !== 'function' || original.__ulim7320Wrapped) return;
    ...
    wrapped.__ulim7320Wrapped = true;
    ...
    try {
      ...
      rt.sdk.onAuthStateChanged(rt.auth, async function (user) {
        ...
        try {
          ...
          safeConsole('info', '[ULIM 7.32.1 Firebase-primary restored]', {
            uid: user.uid,
            role: profile.firebaseRole,
          ...
          safeConsole('warn', '[ULIM 7.32.1 stored Firebase session rejected]', error);
          try { await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
        }
      });
    } catch (error) {
      restoreListenerInstalled = false;
      safeConsole('warn', '[ULIM 7.32.1 restore unavailable]', error);
    }
  }
  ...
  function install() {
    ...
    safeConsole('info', '[ULIM 7.32.1 Firebase-primary staff login installed]', VERSION);
  }
  ...
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7321_API__ = Object.freeze({
    version: VERSION,
