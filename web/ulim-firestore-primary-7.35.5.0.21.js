(function (global) {
  'use strict';
  if (global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_7355021__) return;
  ...
  const VERSION = '2026-08-08.735.05.0.21-preview-video-staffmemo-classroom';
  ...
  async function saveDailyClassVideoLink7355021() {
    ...
    if (!className || className === '전체반') return alert('수업영상 링크를 저장할 반을 먼저 선택해주세요.');
    ...
    if (!item || !text(item.classId)) return alert('선택한 반 정보를 찾지 못했습니다. 반을 다시 선택해주세요.');
    ...
    try {
      const data = await call('saveStaffDailyEvaluationsOperational', {
        ...
        requestId: requestId('daily-class-video-7355021'),
        date: date,
  ...
  global.ulimSaveDailyClassVideoLink7355021 = saveDailyClassVideoLink7355021;

  function installDailyVideoLinkSync7355021() {
    const panel = document.getElementById('adminPanelDailyEval');
    if (!panel || panel.dataset.ulimVideoLinkSync7355021 === '1') return;
    panel.dataset.ulimVideoLinkSync7355021 = '1';
    ['adminDailyEvalDate','adminDailyEvalClass'].forEach(function (id) {
      ...
      el.addEventListener('change', function () {
        setTimeout(function () {
          ...
          let item = null;
          try { if (typeof global.adminGetClassItemByName_ === 'function') item = global.adminGetClassItemByName_(className, date); } catch (_ignore) {}
          input.value = item && item.videoLink ? text(item.videoLink) : '';
  ...
  async function install() {
    ...
    installOperationalDataOverrides(); installMessageOverrides(); installAttendancePreview7355021(); installDailyVideoLinkSync7355021(); installShowPanelHook(); installStaffNotes();
    global.__ULIM_ATTENDANCE_AUTO_REFRESH_DISABLED_7355014__ = true;
