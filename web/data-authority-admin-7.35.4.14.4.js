(function (global) {
  'use strict';
  if (global.__ULIM_DATA_AUTHORITY_7354144__) return;
  ...
  global.ULIM_DATA_AUTHORITY_VERSION = '2026-08-05.735.04.14.4-resumable-sheets-backup';
  ...
  async function backup() {
    ...
    try {
      ...
      var result = await call('runFirestoreBackupAdmin735414', { full: full, requestId: clientRequestId });
      ...
      setStatus((text(error && error.message) || '백업에 실패했습니다.') + '\nFirestore 운영자료는 이미 저장되어 있으며, 백업 실패가 앱 저장을 취소하지 않습니다. 교정본 배포 후 전체 백업을 다시 누르면 이어서 처리됩니다.', 'error');
    } finally { hideLoading(); }
  }
