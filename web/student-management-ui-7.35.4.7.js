(function (global) {
  ...
  if (global.__ULIM_STUDENT_MANAGEMENT_UI_73547__) return;
  global.__ULIM_STUDENT_MANAGEMENT_UI_73547__ = true;

  const VERSION = '2026-08-04.735.04.7';
  ...
  const DRAFT_KEY = 'ulim_notice_message_draft_73547';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .ulim-sm-modal-panel73546{position:relative;z-index:1;width:min(960px,96vw);max-height:92vh;display:flex;flex-direction:column;background:#fff;border-radius:18px;box-shadow:0 24px 80px rgba(15,23,42,.32);overflow:hidden}
  ...
  function transformTable() {
    ...
    Array.from(table.querySelectorAll('tbody tr[data-row-key]')).forEach(function (row) {
      ...
      if (actionCell && !actionCell.querySelector('.ulim-sm-manage73546')) {
        actionCell.innerHTML = '';
        const manage = document.createElement('button');
        manage.type = 'button';
        manage.className = 'admin-btn gray ulim-sm-manage73546';
        manage.textContent = '관리';
        manage.addEventListener('click', function () { openManageModal(key); });
  ...
  function installLegacyButtonObserver() {
    if (!document.body || document.body.dataset.ulimStudentLegacyButtonObserver73547) return;
    document.body.dataset.ulimStudentLegacyButtonObserver73547 = '1';
