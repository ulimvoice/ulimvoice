(function (global) {
  'use strict';
  if (global.__ULIM_STUDENT_MANAGEMENT_V2_735410__) return;
  global.__ULIM_STUDENT_MANAGEMENT_V2_735410__ = true;

  const VERSION = '2026-08-04.735.04.13';
  ...
  function cardHtml() {
    return `<div id="${CARD_ID}" class="admin-card admin-full-only">
      ...
      <details class="ulim-create-box"><summary style="cursor:pointer;font-weight:900;color:#7c3aed;">운영 반 목록 관리</summary>
        <div style="margin:12px 0;padding:11px 13px;border-radius:10px;background:#f5f3ff;color:#5b21b6;font-size:12px;line-height:1.55;"><b>강의실은 반정보에 저장하지 않습니다.</b> 수업 당일 강의실 사용일지의 날짜·시간·담당강사를 기준으로 출석부·태블릿·알림톡에 자동 반영됩니다.</div>
        ...
        <div class="ulim-create-actions"><button type="button" class="admin-btn" onclick="ulimStudentManagementReloadClasses7352()">반 목록 새로고침</button><button type="button" class="admin-btn" onclick="ulimStudentManagementCleanupFixedClassrooms735413()">고정 강의실 정리</button><button type="button" class="admin-btn blue" onclick="ulimClassCatalogSave7354()">반 추가</button></div>
      </details>
    ...
    loadingPromise = (async function () {
      ...
      classes = Array.isArray(result.classes) ? result.classes.map(function (item) {
        return {
          ...
          timeSlots: Array.isArray(item.timeSlots) ? item.timeSlots.map(Number) : []
        };
  ...
  async function reloadClasses() {
    try {
      ...
      classes = Array.isArray(result.classes) ? result.classes.map(function (item) {
        return { classId: text(item.classId), className: text(item.className), instructorUid: text(item.instructorUid), instructorName: text(item.instructorName), selectable: item.selectable !== false, dates: Array.isArray(item.dates) ? item.dates : [], baseName: text(item.baseName), weekday: Number(item.weekday), startTime: text(item.startTime), endTime: text(item.endTime), timeSlots: Array.isArray(item.timeSlots) ? item.timeSlots.map(Number) : [] };
      }) : [];
  ...
  async function reconcileRosters() {
    ...
    try {
      ...
      await load(true);
      setStatus('출석부 참고 정합성 반영 완료: ' + Number(applied.appliedStudents || 0) + '명\n미연결·동명이인·판단 불가 항목은 변경하지 않고 관리자 보류목록에 저장했습니다.', Number((applied.reconciliationAudit || {}).openIssueCount || 0) ? 'warn' : (applied.ok === false ? 'warn' : 'ok'));
    } catch (error) { setStatus(text(error && error.message) || '출석부 반 자동매칭에 실패했습니다.', 'error'); }
    finally { hideLoading(); }
  }

  async function cleanupFixedClassrooms735413() {
    if (dirtyKeys.size) return alert('먼저 변경사항을 저장해주세요.');
    try {
      showLoading('반정보의 고정 강의실 필드를 점검하는 중...');
      const preview = await call('cleanupFixedClassroomFieldsAdmin735413', { mode: 'preview', requestId: requestId('classroom-cleanup-preview-735413') });
      hideLoading();
      const counts = preview.counts || {};
      const message = '고정 강의실 필드가 남아 있는 문서 ' + Number(preview.candidateCount || 0) + '건\n- 운영 반: ' + Number(counts.classes || 0) + '건\n- 수강 관계: ' + Number(counts.studentEnrollments || 0) + '건\n- 반 구성원: ' + Number(counts.classMembers || 0) + '건\n\n이 필드만 제거할까요? 출석기록의 날짜별 강의실 이력은 삭제하지 않습니다.';
      if (!Number(preview.candidateCount || 0)) { setStatus('고정 강의실 필드가 없습니다. 강의실은 사용일지 기준으로 운영됩니다.', 'ok'); return; }
      if (!confirm(message)) { setStatus(message, 'warn'); return; }
      showLoading('고정 강의실 필드를 안전하게 제거하는 중...');
      const result = await call('cleanupFixedClassroomFieldsAdmin735413', { mode: 'apply', requestId: requestId('classroom-cleanup-apply-735413') });
      await reloadClasses();
      setStatus(text(result.message) || '고정 강의실 필드를 제거했습니다.', 'ok');
    } catch (error) {
      setStatus(text(error && error.message) || '고정 강의실 정리에 실패했습니다.', 'error');
    } finally { hideLoading(); }
  }

  function closeReconciliationIssues735413() {
    const overlay = document.getElementById('ulimReconciliationIssues735413');
    if (overlay) overlay.remove();
  }

  function renderReconciliationIssues735413(result) {
    closeReconciliationIssues735413();
    const issues = Array.isArray(result && result.issues) ? result.issues : [];
    const label = {
      homonym_or_identity_ambiguous: '동명이인·식별 충돌',
      attendance_student_missing_from_firestore: '출석부 학생 미연결',
      class_ambiguous: '반 판정 충돌',
      attendance_class_missing_from_catalog: '운영 반 미연결'
    };
    const rows = issues.map(function (item) {
      return '<tr>' +
        '<td style="padding:9px;border-bottom:1px solid #e2e8f0;font-weight:800;">' + escapeHtml(label[text(item.issueType)] || text(item.issueType) || '확인 필요') + '</td>' +
        '<td style="padding:9px;border-bottom:1px solid #e2e8f0;">' + escapeHtml(text(item.studentName) || '-') + '<br><span style="color:#64748b;font-size:11px;">출결 ' + escapeHtml(text(item.attendanceNo) || '-') + ' · ' + escapeHtml(text(item.birthDate) || '생년월일 없음') + '</span></td>' +
        '<td style="padding:9px;border-bottom:1px solid #e2e8f0;">' + escapeHtml(text(item.className) || '-') + '<br><span style="color:#64748b;font-size:11px;">' + escapeHtml(text(item.instructorName) || '담당강사 미확인') + '</span></td>' +
        '<td style="padding:9px;border-bottom:1px solid #e2e8f0;color:#b91c1c;">' + escapeHtml([item.studentReason, item.classReason].filter(Boolean).join(' / ') || '관리자 확인 필요') + '</td>' +
      '</tr>';
    }).join('');
    const overlay = document.createElement('div');
    overlay.id = 'ulimReconciliationIssues735413';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:10060;background:rgba(15,23,42,.65);display:flex;align-items:center;justify-content:center;padding:18px;';
    overlay.innerHTML = '<div style="width:min(1120px,96vw);max-height:90vh;background:#fff;border-radius:18px;overflow:hidden;display:flex;flex-direction:column;">' +
      '<div style="padding:17px 20px;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;"><div><b style="font-size:20px;">데이터 정합성 보류항목</b><div style="font-size:12px;color:#64748b;margin-top:4px;">자동 연결하지 않은 항목 ' + issues.length + '건</div></div><button type="button" class="admin-btn" onclick="ulimStudentManagementCloseReconciliationIssues735413()">닫기</button></div>' +
      '<div style="padding:11px 20px;background:#fff7ed;color:#9a3412;font-size:13px;">동명이인이나 정보가 부족한 학생은 자동 병합·생성·삭제하지 않습니다. 학생정보와 출석부를 확인한 뒤 관리자 화면에서 직접 수정하세요.</div>' +
      '<div style="overflow:auto;"><table style="width:100%;min-width:850px;border-collapse:collapse;font-size:13px;"><thead style="position:sticky;top:0;background:#f8fafc;"><tr><th style="padding:9px;text-align:left;">구분</th><th style="padding:9px;text-align:left;">학생</th><th style="padding:9px;text-align:left;">출석부 반</th><th style="padding:9px;text-align:left;">보류 사유</th></tr></thead><tbody>' + (rows || '<tr><td colspan="4" style="padding:28px;text-align:center;color:#64748b;">보류 중인 항목이 없습니다.</td></tr>') + '</tbody></table></div></div>';
    overlay.addEventListener('click', function (event) { if (event.target === overlay) closeReconciliationIssues735413(); });
    document.body.appendChild(overlay);
  }

  async function showReconciliationIssues735413() {
    try {
      showLoading('보류항목을 불러오는 중...');
      const result = await call('listReconciliationIssuesAdmin735413', { limit: 500, requestId: requestId('reconciliation-issues-735413') });
      hideLoading();
      renderReconciliationIssues735413(result);
      setStatus('관리자 확인 보류항목 ' + Number(result.count || 0) + '건을 불러왔습니다.', Number(result.count || 0) ? 'warn' : 'ok');
    } catch (error) {
      setStatus(text(error && error.message) || '보류항목을 불러오지 못했습니다.', 'error');
    } finally { hideLoading(); }
  }
  ...
  async function saveClassCatalog7354() {
    ...
    try {
      ...
      const result = await call('saveClassCatalogAdmin7354', { instructorUid: instructorUid, baseName: baseName, hours: hours, requestId: requestId('class-catalog-save-7354') });
  ...
  function install() {
    ...
    global.ulimStudentManagementCleanupFixedClassrooms735413 = cleanupFixedClassrooms735413;
    global.ulimStudentManagementShowReconciliationIssues735413 = showReconciliationIssues735413;
    global.ulimStudentManagementCloseReconciliationIssues735413 = closeReconciliationIssues735413;
