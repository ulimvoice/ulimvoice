(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73103__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73103__ = true;

  const VERSION = '2026-07-31.731.03';
  const CACHE_PREFIX = 'ulim_staff_fsop_7313_';
        ...
        if (forceSheetSync) {
          ...
          try {
            ...
            if (rt.staffOperational731.getClasses) {
              const classData = callableData(await rt.staffOperational731.getClasses({ date: ctx.date }));
  ...
  global.adminLoadClassList = async function (dateOverride, force) {
    ...
    const promise = (async function () {
      ...
      async function readFirestoreClasses() {
        if (rt.staffOperational731.getClasses) {
          const classData = callableData(await rt.staffOperational731.getClasses({ date: date }));
