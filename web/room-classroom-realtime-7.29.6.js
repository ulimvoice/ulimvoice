(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72906__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72906__ = true;
  ...
  const APP_NAME = "ulim-room-classroom-realtime-7296";
  const SESSION_FINGERPRINT_KEY = "ulimRealtimeLegacySessionFingerprint721";
  const REQUEST_PREFIX = "RT7296";
  ...
  async function subscribeClassroom(date) {
    ...
    if (!runtime) return;
    if (state.classroomDate === date && state.classroomUnsubscribe && state.classroomClaimsUnsubscribe) return;
    ...
    if (date) {
      ...
      setTimeout(function () {
        refreshClassroomFromSheet7295(date, {
          ...
          safeConsole("warn", "[ULIM classroom 7.29.6 initial sheet load]", error);
        });
      }, 250);
    }
  ...
  safeConsole("info", "[ULIM 7.29.6 sheet-canonical optimistic realtime module]", VERSION);
