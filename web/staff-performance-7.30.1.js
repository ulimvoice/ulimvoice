(function () {
  ...
  if (window.__ULIM_STAFF_PERFORMANCE_73001__) return;
  window.__ULIM_STAFF_PERFORMANCE_73001__ = true;

  const VERSION = '2026-07-31.730.01';
  ...
  async function validateSessionInBackground() {
    ...
    sessionValidationPromise = (async () => {
      ...
      try {
        const data = await api('adminGetSession', { adminToken: tokenAtStart, noCache: '1', performanceVersion: '73001' });
        ...
        if (isDefinitelyExpired(error)) {
          const firebasePrimaryActive = !!(
            window.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7323__ ||
            window.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7322__ ||
            (getInfo() && (getInfo().firebaseAuthUid || getInfo().principalUidV2))
          );
          if (firebasePrimaryActive) {
            try { localStorage.removeItem('adminToken'); sessionStorage.removeItem('adminToken'); } catch (ignore) {}
            try { if (typeof adminToken !== 'undefined') adminToken = ''; } catch (ignore) {}
            try { window.adminToken = ''; } catch (ignore) {}
            try { window.dispatchEvent(new CustomEvent('ulim-legacy-session-expired', { detail: { source: 'staff-performance-73001', version: VERSION } })); } catch (ignore) {}
          } else {
            safeConsole('warn', '[ULIM 7.30.1] 저장 세션 만료:', error);
        ...
        } else if (!(error && error.silent)) {
          safeConsole('warn', '[ULIM 7.30.1] 세션 백그라운드 확인 지연 — 현재 화면 유지:', error);
        }
        return false;
      }
    })();
  ...
  async function buildCanonicalDailyRows(ctx, tokenAtStart) {
    const attendancePromise = api('adminGetAttendanceSnapshot', {
      ...
      statusFilter: '', forceRefresh: 'Y', noCache: 'Y', performanceVersion: '73001'
    });
    const savedPromise = api('adminGetDailyEvaluations', {
      adminToken: tokenAtStart, date: ctx.date, className: ctx.className, keyword: ctx.keyword,
      forceRefresh: 'Y', noCache: 'Y', performanceVersion: '73001'
    });
