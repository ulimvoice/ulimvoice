(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735427__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-09.735.04.27-responsive-dual-roster-manual-refresh-owner';

  var VERSION = '2026-08-09.735.04.27-responsive-dual-roster-manual-refresh-owner';
  var loadSequence = 0;
  ...
  function ensureAllClassesModal735410() {
    ...
    modal.innerHTML = ''
      ...
      +       '<div class="ulim-ledger-heading735427"><h3>전체반 출석부</h3><div id="ulimAllClassesContext735410">전월·현재월 동시 표시</div></div>'
  ...
  function ledgerStudentIdentityCells735427(group, student, rowIndex, monthKey) {
    var selectKey = group.classId + '|' + student.studentUid;
    var selected = allClassesState735410.selectedCards.has(selectKey);
    var selector = isFullAdmin()
      ? '<input type="checkbox" data-ledger-select="1" aria-label="학생 선택" ' + (selected ? 'checked' : '') + '>'
      : '';
    var gear = isFullAdmin() ? '<button type="button" data-ledger-gear="1" title="학생정보">⚙</button>' : '';
    var drag = isFullAdmin() ? '<span class="ulim-ledger-drag735423" data-ledger-drag="1" draggable="true" title="학생 이동">⋮⋮</span>' : '';
    return '<td class="ulim-ledger-no735427">' + selector + '<span>' + (rowIndex + 1) + '</span></td>'
      + '<td class="ulim-ledger-name735427" data-month-roster="' + escapeHtml(monthKey) + '">' + drag + '<b>' + escapeHtml(student.studentName) + '</b>' + gear + '</td>';
  }

  function ledgerMonthBlockHtml735427(group, monthKey, sessions, kind) {
    var isCurrent = kind === 'current';
    var students = group.students || [];
    var safeSessions = Array.isArray(sessions) ? sessions : [];
    var columns = 2 + Math.max(1, safeSessions.length) + (isCurrent ? 1 : 0);
    var monthClass = isCurrent ? ' current' : ' prev';
    var header = '<tr><th colspan="' + columns + '" class="ulim-ledger-month735423' + monthClass + '">' + escapeHtml(monthTitle7355033(monthKey)) + (isCurrent ? ' · 현재월' : ' · 전월') + '</th></tr>';
    ...
    if (safeSessions.length) {
      header += safeSessions.map(function (session) {
        var badge = session.state === 'cancelled' ? '<span>휴강</span>' : session.state === 'substitute' ? '<span>대강</span>' : session.state === 'moved' ? '<span>변경→' + escapeHtml(dateLabel7355033(session.targetDate)) + '</span>' : '';
        return '<th data-ledger-header="1" data-class-id="' + escapeHtml(group.classId) + '" data-date="' + escapeHtml(session.date) + '" class="ulim-ledger-date735423"><b>' + escapeHtml(dateLabel7355033(session.date)) + '</b><small>' + escapeHtml(weekdayLabel7355033(session)) + '</small>' + badge + '</th>';
      }).join('');
    } else {
      header += '<th class="ulim-ledger-date735423">-</th>';
    }
    if (isCurrent) header += '<th class="ulim-ledger-note735423">비고</th>';
    header += '</tr>';

    var rows = students.map(function (student, rowIndex) {
      var cells = student.cells || {};
      var lastMemo = '';
      safeSessions.forEach(function (session) { var cell = cells[session.date] || {}; if (text(cell.memo)) lastMemo = text(cell.memo); });
      var row = '<tr data-ledger-student="' + escapeHtml(student.studentUid) + '" data-source-class="' + escapeHtml(group.classId) + '">';
      row += ledgerStudentIdentityCells735427(group, student, rowIndex, monthKey);
      if (safeSessions.length) {
      ...
      if (isCurrent) row += '<td class="ulim-ledger-note735423" title="' + escapeHtml(lastMemo) + '">' + escapeHtml(lastMemo) + '</td>';
      return row + '</tr>';
    }).join('');

    if (isFullAdmin()) {
      var addDate = safeSessions.filter(function (session) { return session.state !== 'cancelled' && session.state !== 'moved'; })[0];
      rows += '<tr class="ulim-ledger-add-row735423"><td></td><td><button type="button" data-ledger-add-class="1" data-class-id="' + escapeHtml(group.classId) + '" data-date="' + escapeHtml(addDate && addDate.date || '') + '">＋ 학생 추가</button></td><td colspan="' + (Math.max(1, safeSessions.length) + (isCurrent ? 1 : 0)) + '"></td></tr>';
    }
    return '<div class="ulim-ledger-month-card735427' + monthClass + '" data-ledger-month="' + escapeHtml(monthKey) + '"><table class="ulim-ledger-table735423"><thead>' + header + '</thead><tbody>' + rows + '</tbody></table></div>';
  }
  ...
  function renderAllClassesBoardLocal735410() {
    ...
    board.innerHTML = groups.map(function (group) {
      ...
      return '<section class="ulim-ledger-class735423" data-ledger-class="' + escapeHtml(group.classId) + '">'
        + '<div class="ulim-ledger-class-title735423"><div><b>' + escapeHtml(group.className) + '</b><span>' + escapeHtml(group.instructorName + 'T · ' + group.startTime + '~' + group.endTime) + '</span></div><span>' + (group.students || []).length + '명</span></div>'
        + '<div class="ulim-ledger-scroll735423"><div class="ulim-ledger-month-pair735427">'
        + ledgerMonthBlockHtml735427(group, prevMonth, prevSessions, 'prev')
        + ledgerMonthBlockHtml735427(group, currentMonth, currentSessions, 'current')
        + '</div></div></section>';
    }).join('');
  ...
  function syncLedgerCellDom735427(group, student, session) {
    var board = document.getElementById('ulimAllClassesBoard735410');
    if (!board || !group || !student || !session) return;
    Array.from(board.querySelectorAll('[data-ledger-cell]')).forEach(function (node) {
  ...
  async function applyScheduleOperation735423() {
    ...
    var notify = !!(document.getElementById('ulimLedgerScheduleNotify735427') && document.getElementById('ulimLedgerScheduleNotify735427').checked);
    ...
    var label = operation === 'cancel' ? '휴강' : operation === 'substitute' ? '대강' : '수업일 변경';
  ...
  function ensureLedgerStyles735425() {
    var legacyIds = ['ulim-attendance-ledger-style-735423', 'ulim-attendance-ledger-style-735426'];
    legacyIds.forEach(function (id) { var node = document.getElementById(id); if (node) node.remove(); });
    if (document.getElementById('ulim-attendance-ledger-style-735427')) return;
    var style = document.createElement('style');
    style.id = 'ulim-attendance-ledger-style-735427';
    style.textContent = ''
      + '#ulimAllClassesAttendanceModal735410{display:none;position:fixed;inset:0;z-index:2147483645;background:#f1f5f9;overflow:auto;font-family:inherit;color:#0f172a;pointer-events:auto!important;overscroll-behavior:contain}'
      ...
      + '.ulim-ledger-scroll735423{overflow-x:auto;overflow-y:hidden;max-width:100%;background:#fff;-webkit-overflow-scrolling:touch}.ulim-ledger-month-pair735427{display:grid;grid-template-columns:max-content max-content;align-items:start;width:max-content;min-width:100%}.ulim-ledger-month-card735427{overflow:visible}.ulim-ledger-month-card735427.current{border-left:3px solid #2563eb}'
      + '.ulim-ledger-table735423{border-collapse:collapse;border-spacing:0;width:max-content;table-layout:fixed;font-size:12px}.ulim-ledger-table735423 th,.ulim-ledger-table735423 td{border:1px solid #cbd5e1;padding:5px 4px;text-align:center;background:#fff;min-width:92px;height:40px;vertical-align:middle}.ulim-ledger-table735423 thead th{background:#f8fafc;font-weight:900;color:#1e293b}'
      ...
      + '.ulim-ledger-month735423{font-size:13px!important;height:31px!important}.ulim-ledger-month735423.prev{background:#eef2f7!important;color:#475569}.ulim-ledger-month735423.current{background:#dbeafe!important;color:#1e3a8a}'
      ...
      + '.ulim-ledger-note735423{min-width:140px!important;width:140px!important;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ulim-ledger-empty-month735427{color:#94a3b8}'
