(function (global) {
  'use strict';
  if (global.__ULIM_STAFF_ACCOUNT_ADMIN_73431__) return;
  global.__ULIM_STAFF_ACCOUNT_ADMIN_73431__ = true;

  const VERSION = '2026-08-01.734.03.1';
  let accounts = [];
  ...
  async function runtime() {
    ...
    if (typeof room.getStableIdToken === 'function') await room.getStableIdToken(rt, false, 'staff-account-admin-7342');
    ...
    try {
      const response = await Promise.race([
        ...
        new Promise(function (_resolve, reject) {
          timer = setTimeout(function () { reject(timeoutError(name, limit)); }, limit);
        })
      ]);
    ...
    panel.innerHTML = `
      <div class="admin-card admin-full-only">
        ...
        <div class="admin-btn-row" style="margin-top:10px;">
          ...
          <button type="button" class="admin-btn blue" onclick="ulimStaffAccountLoad7342(true)">목록 새로고침</button>
          <button type="button" class="admin-btn" onclick="ulimStaffAccountLoad7342(false)">Firestore만 다시조회</button>
        </div>
  ...
  async function load(syncFromSheet) {
    ...
    loadingPromise = (async function () {
      ...
      status('교직원 목록을 불러오는 중...', 'loading');
      const listResult = await listAccounts();
      if (syncResult) {
        const message = '연결 복구 완료: 신규 ' + Number(syncResult.created || 0) + '명 · 연결수정 ' + Number(syncResult.repaired || 0) + '명 · 유지 ' + Number(syncResult.preserved || 0) + '명 · 암호초기화 필요 ' + Number(syncResult.resetRequired || 0) + '명' + (Number(syncResult.roleReviewRequired || 0) ? ' · 관리자 권한 검토 ' + Number(syncResult.roleReviewRequired || 0) + '명' : '') + (Number(syncResult.failed || 0) ? ' · 실패 ' + Number(syncResult.failed || 0) + '명' : '');
        status(message, Number(syncResult.failed || 0) || Number(syncResult.bindingFailed || 0) ? 'warn' : 'ok');
      } else if (syncError) {
        status((syncError && syncError.code === 'ulim/client-timeout' ? '시트 동기화 응답이 제한시간을 넘겨 Firestore 현재 목록으로 전환했습니다. 서버 작업은 완료됐을 수 있으므로 잠시 후 Firestore만 다시조회를 눌러주세요. ' : '시트 연결 복구는 실패했지만 현재 Firestore 목록은 불러왔습니다. ') + text(syncError.message || syncError), 'warn');
      } else {
        status(listResult && listResult.source === 'firestore-direct' ? '교직원 목록을 불러왔습니다. 서버 목록 함수는 직접 Firestore 조회로 대체했습니다.' : '교직원 목록을 불러왔습니다.', listResult && listResult.source === 'firestore-direct' ? 'warn' : 'ok');
      }
      return true;
    })().catch(function (error) {
      status(text(error && error.message) || '교직원 계정 목록 조회에 실패했습니다.', 'error');
      return false;
  ...
  function install() {
    ...
    try {
      ...
      console.info('[ULIM 7.34.3 CLEAN START] staff sheet refresh + editable student directory');
    } catch (_ignore) {}
  }
