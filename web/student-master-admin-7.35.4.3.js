(function (global) {
  'use strict';
  if (global.__ULIM_STUDENT_MANAGEMENT_V2_73543__) return;
  global.__ULIM_STUDENT_MANAGEMENT_V2_73543__ = true;

  const VERSION = '2026-08-03.735.04.3';
  ...
  function cardHtml() {
    return `<div id="${CARD_ID}" class="admin-card admin-full-only">
      ...
      <div class="ulim-student-help"><b>필수입력은 학생명과 학생 전화번호뿐입니다.</b> 생년월일·보호자전화·등록일·수강반·메모는 빈칸으로 저장하거나 나중에 수정할 수 있습니다. 처리구분에서 신규·반이동·보강을 선택하면 학생-반 관계와 해당 수업일 운영자료에 반영됩니다. 반이동은 처리구분을 먼저 선택한 뒤 이동할 새 반만 선택하세요. 등록 취소 후에는 학생 삭제 버튼으로 최종 삭제합니다.</div>
  ...
  function rowData(key) {
    ...
    return {
      ...
      registrationType: text(document.getElementById(key + '_operation') && document.getElementById(key + '_operation').value) || 'existing',
  ...
  function render() {
    ...
    const rows = filtered.map(function (student) {
      ...
      return `<tr${rowClass ? ' class="' + rowClass + '"' : ''} data-row-key="${key}">
        ...
        <td><select id="${key}_classes" data-row-key="${key}" data-class-select="true" multiple size="4"${cancelled ? ' disabled' : ''}>${classOptionsHtml(student.selectedClassIds)}</select><div id="${key}_tags" class="ulim-tags">${tagsHtml(student.selectedClassIds, student.legacyUnmappedClassNames)}</div></td>
  ...
  function handleOperationModeChange(key) {
    ...
    if (mode === 'class_move') {
      if (!select.dataset.ulimMovePrepared) {
        select.dataset.ulimPreviousSelection = JSON.stringify(selectedValues(select));
        Array.from(select.options).forEach(function (option) { option.selected = false; });
        select.dataset.ulimMovePrepared = '1';
      }
      if (note) note.innerHTML = '<b>이동할 새 반만 선택하세요.</b><br>기존 반은 처리일부터 자동 종료됩니다.';
      const dateInput = document.getElementById(key + '_operation_date');
      if (dateInput && !dateInput.value) dateInput.value = today();
    } else {
      if (select.dataset.ulimMovePrepared === '1' && !selectedValues(select).length) {
        let previous = [];
        try { previous = JSON.parse(select.dataset.ulimPreviousSelection || '[]'); } catch (_ignore) {}
        Array.from(select.options).forEach(function (option) { option.selected = previous.indexOf(text(option.value)) >= 0; });
      }
      delete select.dataset.ulimMovePrepared;
      delete select.dataset.ulimPreviousSelection;
      if (note) note.innerHTML = mode === 'makeup' ? '보강: 선택 반·처리일에만 추가' : '반이동: 처리구분 선택 후 이동할 새 반만 선택<br>보강: 선택 반·처리일에만 추가';
    }
  ...
  async function saveKeys(keys) {
    ...
    if (!confirm('학생 ' + edits.length + '명의 변경사항을 저장할까요?\n' + operationSummary + (moveDetails.length ? '\n\n반이동 대상\n' + moveDetails.join('\n') : '') + '\n\n반이동은 선택한 새 반목록으로 완전히 교체되고 기존 반은 처리일부터 종료됩니다. 보강은 처리일(빈칸이면 오늘)의 해당 반에만 추가됩니다.')) return;
  ...
  function renderAttendanceCandidates73541() {
    const select = attendancePanelElement73541('ulimAttendanceAddStudent73541');
    const search = normalize(attendancePanelElement73541('ulimAttendanceAddSearch73541') && attendancePanelElement73541('ulimAttendanceAddSearch73541').value);
    if (!select) return;
    ...
    const eligible = students
      ...
      .slice(0, 200);
    select.innerHTML = '<option value="">기존학생을 선택하지 않음</option>' + eligible.map(function (student) {
  ...
  function attendanceKindChanged73541() {
    const kind = text(attendancePanelElement73541('ulimAttendanceAddKind73541') && attendancePanelElement73541('ulimAttendanceAddKind73541').value) || 'existing';
    const directWrap = attendancePanelElement73541('ulimAttendanceAddDirectWrap73541');
    const directInput = attendancePanelElement73541('ulimAttendanceAddDirectName73541');
    const hint = attendancePanelElement73541('ulimAttendanceAddHint73541');
    if (directWrap) directWrap.style.display = kind === 'existing' ? 'none' : '';
    if (directInput && kind === 'existing') directInput.value = '';
    if (hint) {
      hint.textContent = kind === 'existing'
        ? '기존학생은 선택한 반의 정규 수강생으로 추가됩니다. 현재 수강 중인 다른 반은 유지됩니다.'
        : (kind === 'makeup'
          ? '보강은 선택한 날짜에만 추가됩니다. 기존학생을 선택하거나 이름만 직접 입력할 수 있습니다.'
          : '일일특강은 선택한 날짜에만 추가됩니다. 정규 학생계정 없이 이름만으로도 등록할 수 있습니다.');
    }
    renderAttendanceCandidates73541();
  }

  async function ensureAttendanceAddData73541(force) {
    ...
    try {
      const result = await call('listStudentManagementAdmin7352', { requestId: requestId('attendance-add-list-73541') });
      classes = Array.isArray(result.classes) ? result.classes.map(function (item) {
        return {
          classId: text(item.classId), className: text(item.className), instructorUid: text(item.instructorUid),
          instructorName: text(item.instructorName), selectable: item.selectable !== false, dates: Array.isArray(item.dates) ? item.dates : [],
  ...
  async function prepareAttendanceAdd73541(force) {
    if (!isSuperAdmin()) return;
    updateAttendanceAddContext73541();
    attendanceKindChanged73541();
    await ensureAttendanceAddData73541(Boolean(force));
  }

  async function addAttendanceFromPanel73541() {
    if (!isSuperAdmin()) return alert('전체관리자 권한이 필요합니다.');
    const context = attendanceContext73541();
    if (!context.date) return alert('수업일을 선택해주세요.');
    if (!context.className || context.className === '전체반') return alert('추가할 반 출석부를 먼저 선택해주세요.');

    const kind = text(attendancePanelElement73541('ulimAttendanceAddKind73541') && attendancePanelElement73541('ulimAttendanceAddKind73541').value) || 'existing';
    const studentUid = text(attendancePanelElement73541('ulimAttendanceAddStudent73541') && attendancePanelElement73541('ulimAttendanceAddStudent73541').value);
    const directName = text(attendancePanelElement73541('ulimAttendanceAddDirectName73541') && attendancePanelElement73541('ulimAttendanceAddDirectName73541').value);
    const selectedStudent = students.find(function (student) { return student.studentUid === studentUid; }) || null;

    if (kind === 'existing' && !selectedStudent) return alert('기존학생을 검색해 선택해주세요.');
    if (kind !== 'existing' && !selectedStudent && !directName) return alert('기존학생을 선택하거나 학생명을 직접 입력해주세요.');

    const label = kind === 'existing' ? '기존학생' : (kind === 'daily_special' ? '일일특강' : '보강');
    const studentName = selectedStudent ? text(selectedStudent.name) : directName;
    const confirmText = context.date + '\n' + context.className + '\n' + studentName + ' · ' + label +
      (kind === 'existing'
        ? '\n\n선택한 반의 정규 수강생으로 추가하고 다른 수강반은 유지합니다.'
        : '\n\n선택한 날짜에만 출석부에 추가합니다.');
    ...
    try {
      ...
      const result = await call('addTemporaryAttendanceAdmin7354', {
        ...
        requestId: requestId('attendance-page-add-73541')
      });

      const search = attendancePanelElement73541('ulimAttendanceAddSearch73541');
      const select = attendancePanelElement73541('ulimAttendanceAddStudent73541');
      const direct = attendancePanelElement73541('ulimAttendanceAddDirectName73541');
      if (search) search.value = '';
      if (select) select.value = '';
      if (direct) direct.value = '';
      renderAttendanceCandidates73541();

      if (typeof global.adminLoadAttendanceSnapshot === 'function') {
        await global.adminLoadAttendanceSnapshot(true, true);
      }
  ...
  function install() {
    ...
    global.ulimAttendanceAddPrepare73541 = prepareAttendanceAdd73541;
    global.ulimAttendanceAddFilter73541 = renderAttendanceCandidates73541;
    global.ulimAttendanceAddKindChanged73541 = attendanceKindChanged73541;
    global.ulimAttendanceAddFromRoster73541 = addAttendanceFromPanel73541;
