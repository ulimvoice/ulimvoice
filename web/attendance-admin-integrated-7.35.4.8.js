(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_73548__) return;
  global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_73548__ = true;
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-04.735.04.8';

  var VERSION = '2026-08-04.735.04.8';
  var loadSequence = 0;
  ...
  var originalLoadClassList = typeof global.adminLoadClassList === 'function' ? global.adminLoadClassList : null;
  ...
  function assignOperationalClassList(date, rawClasses) {
    ...
    try { if (typeof adminClassListLoadedKey !== 'undefined') adminClassListLoadedKey = 'firebase-operational|' + date + '|73548'; } catch (_ignore3) {}
  ...
  async function loadClassListFirebaseFirst(dateOverride, force) {
    ...
    try {
      var data = await call('getStaffClassListOperationalSnapshot', {
        date: date,
        force: force === true,
        requestId: requestId('attendance-class-list-73548')
      });
      ...
      if (!classes.length && originalLoadClassList && readLegacyToken()) {
        await originalLoadClassList.call(global, date, true);
        if (sequence !== classListSequence || (dateEl && text(dateEl.value) !== date)) return { stale: true };
        try { if (typeof adminRenderClassSelectors === 'function') adminRenderClassSelectors(); } catch (_ignore1) {}
        return { status: 'success', classes: currentGlobalClassList(), source: 'legacy-fallback' };
      }
      ...
      if (sequence !== classListSequence) return { stale: true };
      if (originalLoadClassList && readLegacyToken()) {
        try {
          await originalLoadClassList.call(global, date, true);
          if (sequence !== classListSequence || (dateEl && text(dateEl.value) !== date)) return { stale: true };
          return { status: 'success', classes: currentGlobalClassList(), source: 'legacy-fallback' };
  ...
  async function loadFromFirebase(context) {
    return call('getStaffAttendanceOperationalSnapshot', {
      ...
      requestId: requestId('attendance-snapshot-73548')
    });
  }
  ...
  async function loadFromLegacy(context) {
    ...
    return apiFn('adminGetAttendanceSnapshot', {
      ...
      statusFilter: context.statusFilter,
      noCache: 1,
      requestId: requestId('attendance-legacy-snapshot-73548')
    });
  }
  ...
  async function safeLoadAttendanceSnapshot(showAlert) {
    ...
    global.__ULIM_ATTENDANCE_ACTIVE_REQUEST_73548__ = { sequence: sequence, key: key, startedAt: Date.now() };
    ...
    try {
      ...
      global.__ULIM_ATTENDANCE_LAST_COMPLETED_73548__ = { sequence: sequence, key: key, completedAt: Date.now(), count: records.length };
  ...
  function invalidateAttendanceView(message) {
    loadSequence += 1;
    global.__ULIM_ATTENDANCE_ACTIVE_REQUEST_73548__ = null;
  ...
  async function saveAttendanceStudentDetail() {
    ...
    try {
      ...
      var result = await call('updateStudentAdmin7352', {
        ...
        requestId: requestId('attendance-student-detail-update-73548')
      });
      directoryCache = null;
      directoryLoadedAt = 0;
      closeDetailModal();
    ...
    if (originalSelectClass && !global.adminSelectClass.__ulim73548Wrapped) {
      var wrappedSelectClass = function (className, targetId, panelId) {
        ...
        if ((targetId || 'adminAttendanceClass') === 'adminAttendanceClass' && text(className) === '전체반') {
          setTimeout(function () { safeLoadAttendanceSnapshot(false); }, 0);
        }
        return result;
      };
