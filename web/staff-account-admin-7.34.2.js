(function (global) {
  'use strict';
  if (global.__ULIM_STAFF_ACCOUNT_ADMIN_7342__) return;
  global.__ULIM_STAFF_ACCOUNT_ADMIN_7342__ = true;
    ...
    panel.innerHTML = `
      <div class="admin-card admin-full-only">
        ...
        <p style="font-size:13px;color:#64748b;line-height:1.65;margin-top:-4px;">
          ...
          <b>시트 계정 연결 복구</b>는 Google Sheets의 이름·ID·전화번호·UID 연결만 읽으며 비밀번호·Hash·Salt는 읽지 않습니다.
        ...
        <div class="admin-btn-row" style="margin-top:10px;">
          ...
          <button type="button" class="admin-btn blue" onclick="ulimStaffAccountLoad7342(false)">Firestore 다시조회</button>
          <button type="button" class="admin-btn orange" onclick="ulimStaffAccountLoad7342(true)">시트 계정 연결 복구</button>
        </div>
  ...
  async function listAccounts() {
    const data = await call('listStaffAccountsAdmin', {});
  ...
  async function load(syncFromSheet) {
    ...
    loadingPromise = (async function () {
      ...
      if (syncFromSheet === true) {
        if (!confirm('Google Sheets 관리자 목록에서 일반 정보와 UID 연결만 읽어 Firebase 계정을 복구할까요?\n비밀번호·Hash·Salt는 읽거나 변경하지 않습니다.')) return false;
        status('교직원 UID 연결을 안전하게 복구하는 중...', 'loading');
        try {
          syncResult = await call('syncStaffDirectoryFromSheetsAdmin7342', { requestId: requestId('staff-directory') });
        } catch (error) {
          syncError = error;
        }
      }
      status('Firestore 교직원 목록을 불러오는 중...', 'loading');
      await listAccounts();
      if (syncResult) {
        ...
        status('시트 연결 복구는 실패했지만 현재 Firestore 목록은 불러왔습니다. ' + text(syncError.message || syncError), 'warn');
      } else {
        status('Firestore 교직원 목록을 불러왔습니다.', 'ok');
      }
      return true;
  ...
  function install() {
    ...
    try {
      ...
      console.info('[ULIM 7.34.2 CLEAN START] safe staff/student directory integration');
    } catch (_ignore) {}
  }
