(function (global) {
  ...
  if (global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7322__) return;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7322__ = true;

  const VERSION = '2026-07-31.732.02';
  ...
  function showStaffShell() {
    ...
    /*
     * 7.32.2: 기존 UI 패치들은 adminToken이 생길 때까지 교직원 탭을 잠급니다.
  ...
  async function migratePasswordCredential(rt, loginId, password, migrationProof) {
    ...
    safeConsole('info', '[ULIM 7.32.2 password credential migrated]', {
    ...
    legacyCompatibilityPromise = Promise.resolve()
      ...
      .then(async function (data) {
        ...
        try {
          ...
          safeConsole('warn', '[ULIM 7.32.2 legacy initial data]', error);
        }
        try {
        ...
        safeConsole('info', '[ULIM 7.32.2 GAS compatibility ready]', {
      ...
      .catch(function (error) {
        ...
        safeConsole('warn', '[ULIM 7.32.2 GAS compatibility failed]', error);
        throw error;
      })
  ...
  async function directFirebaseLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.2 Firebase-primary shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      uid: credential.user.uid,
  ...
  async function legacyFirstLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.2 legacy first shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      serverTiming: data && data.loginTiming || null
    });
      ...
      setTimeout(function () {
        migratePasswordCredential(authenticated, loginId, password, text(data && data.firebasePasswordMigrationProof))
          ...
          .catch(function (error) {
            ...
            safeConsole('warn', '[ULIM 7.32.2 credential migration deferred]', error);
          });
      }, 0);
    }
  ...
  async function firebasePrimaryAdminLogin() {
    ...
    loginPromise = (async function () {
      ...
      try {
        ...
        safeConsole('info', '[ULIM 7.32.2 legacy migration fallback]', text(firebaseError && firebaseError.code));
  ...
  function installLogoutWrapper() {
    ...
    const wrapped = async function () {
      ...
      try {
        ...
        safeConsole('warn', '[ULIM 7.32.2 Firebase signOut delayed]', error);
      }
      try {
        return await original.apply(this, arguments);
      } finally {
        /* signOut가 네트워크 오류로 지연돼도 자동복원은 로그아웃 플래그로 계속 차단됩니다. */
        safeConsole('info', '[ULIM 7.32.2 explicit staff logout]', { signOutCompleted: signOutCompleted });
      }
    };
    ...
    try {
      ...
      rt.sdk.onAuthStateChanged(rt.auth, async function (user) {
        ...
        try {
          ...
          let legacyToken = '';
          try { legacyToken = text(typeof adminToken !== 'undefined' && adminToken || localStorage.getItem('adminToken')); } catch (_ignore) {}
          if (legacyToken && typeof global.adminLoadInitialData === 'function') {
            Promise.resolve(global.adminLoadInitialData()).catch(function () {});
          }
          safeConsole('info', '[ULIM 7.32.2 Firebase-primary restored]', {
            uid: user.uid,
            role: profile.firebaseRole,
          ...
          safeConsole('warn', '[ULIM 7.32.2 stored Firebase session rejected]', error);
          try { await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
        }
      });
    } catch (error) {
      restoreListenerInstalled = false;
      safeConsole('warn', '[ULIM 7.32.2 restore unavailable]', error);
    }
  }
  ...
  function install() {
    ...
    safeConsole('info', '[ULIM 7.32.2 Firebase-primary staff login installed]', VERSION);
  }
