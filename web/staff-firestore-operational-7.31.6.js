(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73106__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73106__ = true;

  const VERSION = '2026-07-31.731.06';
  ...
  function filterAttendanceForSelectedClass(records, ctx) {
    ...
    return list.filter(function (row) {
      if (ctx.classId && text(row && row.classId)) return text(row.classId) === text(ctx.classId);
      return classNamesEquivalent(row && (row.className || row.currentClass), ctx.className);
    });
  }
