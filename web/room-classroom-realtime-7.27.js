(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72701__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72701__ = true;

  const VERSION = "2026-07-29.727.01";
  ...
  const APP_NAME = "ulim-room-classroom-realtime-727";
  const SESSION_FINGERPRINT_KEY = "ulimRealtimeLegacySessionFingerprint721";
  const REQUEST_PREFIX = "RT727";
  ...
  function applyClassroomCommitResult727(date, request, result) {
    ...
    if (summary) {
      summary.textContent =
        "Firestore 즉시 확정 " + accepted.length + "건" +
        (conflicts.length ? " · 중복 " + conflicts.length + "건" : "") +
    ...
    if (conflicts.length) {
      alert(
        "다른 사용자가 먼저 확정한 시간 " + conflicts.length +
        "건을 제외하고 가능한 시간만 반영했습니다."
      );
    }
  }
  ...
  async function saveClassroomFirestoreFirst727(silentConfirm, forceOverride) {
    ...
    if (forceOverride || global.ULIM_CLASSROOM_FIRESTORE_FIRST_ENABLED === false) {
  ...
  function installWrappers() {
    ...
    if (typeof global.adminReleaseClassroomUsage === "function") {
      ...
      global.adminReleaseClassroomUsage = async function (recordId) {
        if (String(recordId || "").indexOf("fs:") === 0) {
          return alert("시트 동기화 중인 강의실 기록입니다. 잠시 후 해제해주세요.");
        }
  ...
  const publicApi727 = Object.freeze({
    version: VERSION,
    start: start,
  ...
  global.ULIM_ROOM_CLASSROOM_REALTIME_727 = publicApi727;
  global.ULIM_ROOM_CLASSROOM_REALTIME_721 = publicApi727;
  ...
  safeConsole("info", "[ULIM 7.27 Firestore-first classroom module]", VERSION);
