(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735424__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-09.735.04.24-owner-repair-fast-ledger';

  var VERSION = '2026-08-09.735.04.24-owner-repair-fast-ledger';
  var loadSequence = 0;
  ...
  async function loadAllClassesData735410(force) {
    ...
    setAllClassesStatus735410(force ? '최신 출석부를 갱신하는 중...' : '출석부를 불러오는 중...');

    allClassesState735410.loadingPromise = (async function () {
      try {
        // Ledger is the display authority. Student directory is editing/detail
        // support only, so it must never block or blank the whole-class ledger.
        var ledger = await call('getAttendanceRosterAdmin73550', {
          ledger: true,
          view: 'ledger',
          anchorDate: today(),
          requestId: requestId('attendance-ledger-735424')
        });
        ...
        // Directory is lazy support for gear/add/remove operations. Load in the
        // background and adopt when ready; a directory delay/failure cannot close
        // or suppress the ledger popup.
        Promise.resolve(loadDirectory(force === true)).then(function (directory) {
        ...
        if (allClassesState735410.reloadPending) {
          ...
          setTimeout(function () { loadAllClassesData735410(false).catch(function () {}); }, 0);
        }
      }
    })();
    return allClassesState735410.loadingPromise;
  ...
  function install() {
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355016__ = 'attendance-admin-integrated-7.35.4.24';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355015__ = 'attendance-admin-integrated-7.35.4.24';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355014__ = 'attendance-admin-integrated-7.35.4.24';
