(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72911__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72911__ = true;

  const VERSION = "2026-07-31.729.11";
  ...
  async function signInLegacySessionFresh(reason) {
    ...
    await runtime.sdk.getIdToken(credential.user, true);
  ...
  async function acceptExistingFirebaseSession72911(runtime, reason, forceRefresh) {
    if (!runtime || !runtime.auth || !runtime.auth.currentUser) return null;
    const user = runtime.auth.currentUser;
    const tokenResult = await runtime.sdk.getIdTokenResult(user, forceRefresh === true);
    const role = String(tokenResult && tokenResult.claims && tokenResult.claims.role || "");
    if (!["student", "teacher", "admin", "superAdmin"].includes(role)) {
      throw new Error("Firebase 계정 역할이 올바르지 않습니다.");
    }
    state.ready = true;
  ...
  async function forceReauthenticate(reason) {
    if (state.signingIn) {
      try {
        ...
        if (existing && existing.auth && existing.auth.currentUser) {
          return await acceptExistingFirebaseSession72911(existing, reason || "forced-existing", true);
        }
      } catch (_ignore) {}
    }
    ...
    if (runtime.auth.currentUser) {
      try {
        return await acceptExistingFirebaseSession72911(runtime, reason || "forced-primary", true);
      } catch (primaryError) {
        safeConsole("warn", "[ULIM 7.29.11 primary token refresh failed]", primaryError);
      }
    }
  ...
  async function ensureAuthenticated() {
    ...
    state.signingIn = (async function () {
      /*
       * 7.29.11 Firebase-primary login:
      ...
      if (runtime.auth.currentUser) {
        try {
          return await acceptExistingFirebaseSession72911(runtime, "ensure-primary", false);
        } catch (primaryError) {
          safeConsole("warn", "[ULIM 7.29.11 primary session rejected]", primaryError);
      ...
      if (runtime.auth.currentUser && storedFingerprint === fingerprint) {
        try {
          await runtime.sdk.getIdToken(runtime.auth.currentUser, true);
          state.ready = true;
          state.sessionFingerprint = fingerprint;
          setStatus("ready", "실시간 연결됨");
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (lastRuntime && lastRuntime.auth && lastRuntime.auth.currentUser) {
          await lastRuntime.sdk.getIdToken(lastRuntime.auth.currentUser, true);
          return lastRuntime;
        }
      } catch (_ignore) {}
      ...
      try {
        ...
        if (alreadyDisplayed && String(state.lastSheetRevisionByDate[date] || "")) {
          checkClassroomSheetRevision7296(date).catch(function (error) {
            safeConsole("warn", "[ULIM classroom 7.29.7 fast revision check]", error);
          });
        ...
        if ((first && first.exists) || hasClassroomDisplayData7297(date)) {
          checkClassroomSheetRevision7296(date).catch(function (error) {
            safeConsole("warn", "[ULIM classroom 7.29.7 first revision check]", error);
          });
  ...
  safeConsole("info", "[ULIM 7.29.10 bundled-proof auth-gated sheet-canonical realtime module]", VERSION);
