(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73122__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73122__ = true;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73121__ = true;
...
function currentStatusEdited73115(row) {
  ...
  async function runtime() {
    ...
    runtimePromise = (async function () {
      const room = global.ULIM_ROOM_CLASSROOM_REALTIME_72916 || global.ULIM_ROOM_CLASSROOM_REALTIME_728 || global.ULIM_ROOM_CLASSROOM_REALTIME_727;
      if (!room || typeof room.ensureAuthenticated !== 'function') throw new Error('실시간 로그인 모듈을 찾지 못했습니다.');
      ...
      if (!rt || !rt.sdk || !rt.functions || !rt.auth || !rt.auth.currentUser) {
        throw new Error('교직원 인증을 완료하지 못했습니다. 다시 로그인해주세요.');
      }
  ...
  async function syncDateFromSheets() {
    // 운영 화면에서는 Google Sheets를 읽지 않습니다.
    return {
      status: 'disabled',
      source: 'firestore-only',
      message: '운영 중 시트 동기화는 사용하지 않습니다.'
    };
  }
  ...
  function attendanceDedupeAliases73118(row, index) {
    const r = row || {};
    const date = normalize(r.date || r.sessionDate || r.classDate || '');
    const classId = text(r.classId);
    const classNameIdentity = 'CLASSNAME:' + classCoreKey(r.className || r.currentClass) + '|' + normalize(r.instructor || r.instructorName || '');
    const bases = Array.from(new Set([
      classId ? date + '|CLASSID:' + classId + '|' : '',
      date + '|' + classNameIdentity + '|'
    ].filter(Boolean)));
    const aliases = [];
    const add = function (kind, value) {
      ...
      bases.forEach(function (base) {
        const alias = base + kind + ':' + clean;
        if (aliases.indexOf(alias) < 0) aliases.push(alias);
      });
    };

    const uid = text(r.studentUid || r.studentUID || r.uidV2 || r.uid);
    const identity = text(r.studentIdentityKey || r.identityKey || r.studentKey);
    const phone = text(r.studentPhone || r.phone).replace(/\D/g, '');
    const no = normalize(r.studentNo || r.attendanceNo);
    const name = normalize(r.studentName || r.name);

    add('UID', uid);
    add('KEY', identity);
    if (phone.length >= 8) add('PHONE', phone);
    if (no && name) add('NO_NAME', no + '|' + name);
    if (!aliases.length) add('ROW', String(index));
    return aliases;
  }

  function attendanceUpdatedAt73118(row) {
    const r = row || {};
    const direct = Number(r.updatedAtMs || r.savedAtMs || r.createdAtMs || 0);
    if (Number.isFinite(direct) && direct > 0) return direct;
    const parsed = Date.parse(r.updatedAt || r.savedAt || r.createdAt || '');
    return Number.isFinite(parsed) ? parsed : 0;
  }

  function attendanceMergeGroup73118(rows) {
    const ranked = rows.slice().sort(function (a, b) {
      const aUid = text(a.studentUid || a.studentIdentityKey) ? 1 : 0;
      const bUid = text(b.studentUid || b.studentIdentityKey) ? 1 : 0;
      const aStatus = attendanceOnly73115(a.status || a.attendanceStatus) !== '미체크' ? 1 : 0;
      const bStatus = attendanceOnly73115(b.status || b.attendanceStatus) !== '미체크' ? 1 : 0;
      return (bUid - aUid) || (bStatus - aStatus) || (attendanceUpdatedAt73118(b) - attendanceUpdatedAt73118(a));
    });
    const merged = Object.assign({}, ranked[0] || {});
    const newest = rows.slice().sort(function (a, b) {
      return attendanceUpdatedAt73118(b) - attendanceUpdatedAt73118(a);
    });

    [
      'studentUid','studentIdentityKey','studentKey','studentNo','attendanceNo',
      'studentPhone','parentPhone','classId','className','teacherUid',
      'teacherScopeKey','instructor','instructorName','classroom','roomName',
      'sourceSheet','sourceCell','recordId'
    ].forEach(function (field) {
      if (text(merged[field])) return;
      const source = newest.find(function (row) { return text(row && row[field]); });
      if (source) merged[field] = source[field];
    });

    const statusSource = newest.find(function (row) {
      return attendanceOnly73115(row && (row.status || row.attendanceStatus)) !== '미체크';
    });
    if (statusSource) {
      const status = attendanceOnly73115(statusSource.status || statusSource.attendanceStatus);
      merged.status = status;
      merged.attendanceStatus = status;
    }

    ['memo','currentStatus','remarkText','sheetRemark'].forEach(function (field) {
      const source = newest.find(function (row) { return text(row && row[field]); });
      if (source) merged[field] = source[field];
    });

    merged.specialStatus = specialStatusOnly73115({
      specialStatus: rows.map(function (row) { return text(row && row.specialStatus); }).filter(Boolean).join(' '),
      specialType: rows.map(function (row) { return text(row && row.specialType); }).filter(Boolean).join(' '),
      enrollmentStatus: rows.map(function (row) { return text(row && row.enrollmentStatus); }).filter(Boolean).join(' '),
      studentStatus: rows.map(function (row) { return text(row && row.studentStatus); }).filter(Boolean).join(' ')
    });
    return merged;
  }

  function dedupeAttendanceRecords73118(records) {
    const source = Array.isArray(records) ? records : [];
    const groups = [];
    source.forEach(function (row, index) {
      const aliases = new Set(attendanceDedupeAliases73118(row, index));
      const matches = [];
      groups.forEach(function (group, groupIndex) {
        const hasMatch = Array.from(aliases).some(function (alias) { return group.aliases.has(alias); });
        if (hasMatch) matches.push(groupIndex);
      });
      if (!matches.length) {
        groups.push({ aliases: aliases, rows: [row] });
        return;
      }
      const target = groups[matches[0]];
      aliases.forEach(function (alias) { target.aliases.add(alias); });
      target.rows.push(row);
      for (let i = matches.length - 1; i >= 1; i -= 1) {
        const sourceGroup = groups[matches[i]];
        sourceGroup.aliases.forEach(function (alias) { target.aliases.add(alias); });
        target.rows = target.rows.concat(sourceGroup.rows);
        groups.splice(matches[i], 1);
      }
    });
    return groups.map(function (group) { return attendanceMergeGroup73118(group.rows); });
  }

  function renderAttendance(records, message, ctx) {
    const renderContext = ctx || attendanceContext();
    const dedupedRecords = dedupeAttendanceRecords73118(records);
    const preparedRecords = preserveAttendanceEdits73116(dedupedRecords, renderContext);
    try {
  ...
  global.adminLoadAttendanceSnapshot = async function (showAlert, forceSheetSync) {
    ...
    // 7.31.21: 출석부 자동 revision 구독 자체를 만들지 않습니다.
    // 출석부 갱신은 날짜/반 변경, 사용자 수동 새로고침, 명시적 저장 후 처리로만 수행합니다.
    global.__ULIM_ATTENDANCE_NO_AUTO_REFRESH_73122__ = true;
      ...
      try {
        ...
        if (forceSheetSync) {
          if (canRenderAttendanceLoad73116()) {
            setAttendanceSummary('최신 출석부를 불러오는 중...');
          }
          if (rt.staffOperational731.getClasses) {
            const classData = await invokeStaff(
              rt,
              rt.staffOperational731.getClasses,
              { date: ctx.date, force: true, requestId: requestId('attendance-class-refresh-73120') },
              'getClasses'
            );
            applyClassListFast(
              ctx.date,
              Array.isArray(classData.classes) ? classData.classes : [],
              true
            );
          }
        }
        ...
        let records = dedupeAttendanceRecords73118(Array.isArray(data.records) ? data.records : []);
        ...
        if (
          ...
          try {
            ...
            if (matched.length) {
              records = dedupeAttendanceRecords73118(matched);
        ...
        if (!records.length && !forceSheetSync) {
          ...
          if (refreshed) {
            ...
            records = dedupeAttendanceRecords73118(Array.isArray(data.records) ? data.records : []);
          }
        }
        ...
        const message = sheetSyncError
          ...
          : (
              forceSheetSync
                ? (
                    '최신 자료 확인 완료 · 선택 반 출석부 ' +
                    records.length +
                    '건'
                  )
                : (
                    data.message ||
                    ('출석부 ' + records.length + '명')
                  )
            );
        ...
        if (sheetSyncError && showAlert) {
          alert(
            isFullAdmin()
              ? (
                  '최신 출석부 확인에 실패했습니다. 현재 저장된 자료를 표시합니다.\n' +
          ...
          alert(
            forceSheetSync
              ? '선택한 반의 출석 학생을 찾지 못했습니다. 반 목록을 다시 선택한 뒤 확인해주세요.'
...
function attendanceMinimal(row) {
  ...
  async function saveAttendanceFirestore(records, silent) {
    ...
    setAttendanceState(compact, '저장 중...', false);
    try {
      const rt = await runtime();
      ...
      const summary = document.getElementById('adminAttendanceSummary');
      setAttendanceSummary(data.message || ('출석 ' + compact.length + '건 저장 완료'), '저장 완료');
      const ctx = attendanceContext();
  ...
  global.adminLoadDailyEvalStudents = async function (forceSheetSync, options) {
    ...
    try {
      ...
      if (forceSheetSync) {
        ...
        const finalResult = await readAndRender(
          rt,
          '최신 학생 및 담당강사 평가 반영 완료'
        );
  ...
  global.adminSaveDailyEvaluations = async function (sendSms) {
    ...
    const saveConfirmText = isFullAdmin()
      ? ('일일평가를 저장하고 [' + recipientTypes.join(', ') + ']에게 ' + channelLabel + '으로 발송할까요?')
    ...
    try {
      ...
      try {
        adminDailyEvalRows = (adminDailyEvalRows || []).map(function (row) {
          ...
          return saved ? Object.assign({}, row, saved, { savedAt: '방금 저장', savedBy: info().name || info().id || '' }) : row;
        });
        global.adminDailyEvalRows = adminDailyEvalRows;
      } catch (ignore) {}
      renderDaily(
  ...
  // 7.04의 1분 후 시트 직접 자동저장은 중단합니다. 명시적 평가저장만 저장 작업을 만듭니다.
  global.ulimScheduleDailyEvalSheetSync704_ = function () {
    const statusEl = document.getElementById('adminDailyDraftStatus');
    if (statusEl) statusEl.textContent = isFullAdmin()
      ? '로컬 임시저장 완료 · 평가저장 시 즉시 반영'
      : '입력 내용 임시저장 완료';
    return false;
  };
  ...
  async function legacyClassListForDate() {
    return [];
  }
  ...
  global.adminLoadClassList = async function (dateOverride, force) {
    ...
    const requestKey = 'classlist-firestore|' + owner() + '|' + date + '|' + (force ? 'force' : 'normal');
    if (inflight.has(requestKey)) return inflight.get(requestKey);
    ...
    const promise = (async function () {
      ...
      const classData = await invokeStaff(
        ...
        rt.staffOperational731.getClasses,
        {
          date: date,
          force: force,
          requestId: requestId('class-list-firestore-73120')
        },
        'getClasses'
      );
      const classes = Array.isArray(classData && classData.classes) ? classData.classes : [];
      applyClassListFast(date, classes, true);
      if (!classes.length) {
        setAttendanceSummary('선택한 날짜에 등록된 수업이 없습니다.', '선택한 날짜에 등록된 수업이 없습니다.');
      }
      return classes;
    })().catch(function (error) {
      applyClassListFast(date, [], true);
      setAttendanceSummary(
        friendlyError(error, '반 목록을 불러오지 못했습니다. 다시 로그인한 뒤 시도해주세요.'),
        '반 목록을 불러오지 못했습니다. 다시 시도해주세요.'
      );
      throw error;
    }).finally(function () {
      inflight.delete(requestKey);
  ...
  function scheduleRevisionReload7318(kind, date, classId, revision) {
    // 7.31.23: attendance revisions are delivered to the canonical attendance UI.
    // This runtime never reloads the table itself; the UI decides whether drafts allow an immediate refresh.
    if (kind === 'attendance') {
      const attendanceState = revisionListeners7318.attendance;
      if (attendanceState) {
        attendanceState.pendingRevision = Math.max(Number(attendanceState.pendingRevision || 0), Number(revision || 0));
        attendanceState.lastReloadAt = Date.now();
      }
      try {
        global.dispatchEvent(new CustomEvent('ulim-attendance-revision', { detail: { scope: 'date', date: text(date), revision: Number(revision || 0), source: 'staffOperationalRevisions' } }));
      } catch (_ignoreAttendanceRevision) {}
      return;
    }
    ...
    state.reloadTimer = setTimeout(function () {
      ...
      // Attendance exits at the top of this function. Only daily-evaluation revisions reload here.
      const task = typeof global.adminLoadDailyEvalStudents === 'function'
        ? global.adminLoadDailyEvalStudents(false, {
            source: 'revision',
            bypassCache: true,
  ...
  async function bindOperationalAttendanceRevision73123() {
    if (typeof operationalAttendanceUnsubscribe73123 === 'function') return;
    try {
      const rt = await runtime();
      if (!(await firebaseStaffClaimsReady73111(rt))) return;
      const ref = rt.sdk.doc(rt.db, 'operationalRealtimeRevisions', 'attendance');
      operationalAttendanceUnsubscribe73123 = rt.sdk.onSnapshot(ref, function (snapshot) {
        const data = snapshot && snapshot.exists() ? snapshot.data() || {} : {};
        const revision = Number(data.revision || 0);
        if (!operationalAttendanceRevision73123) { operationalAttendanceRevision73123 = revision; return; }
        if (revision <= operationalAttendanceRevision73123) return;
        operationalAttendanceRevision73123 = revision;
        try {
          global.dispatchEvent(new CustomEvent('ulim-attendance-revision', { detail: {
  ...
  function ensureRevisionListeners7318() {
    ...
    revisionEnsurePromise73112 = (async function () {
      ...
      global.__ULIM_ATTENDANCE_NO_AUTO_REFRESH_73122__ = false;
      await Promise.all([
        bindRevisionListener7318('attendance', attendanceRevision.date, '', true),
        bindRevisionListener7318('daily', dailyRevision.date, '', true),
        bindOperationalAttendanceRevision73123()
      ]);
      return true;
    })().finally(function () {
  ...
  function resetRevisionListeners7318() {
    if (typeof operationalAttendanceUnsubscribe73123 === 'function') { try { operationalAttendanceUnsubscribe73123(); } catch (_ignoreOperationalUnsub) {} }
    operationalAttendanceUnsubscribe73123 = null;
    operationalAttendanceRevision73123 = 0;
  ...
  function retryPrewarm() {
    prewarm();
  }
  setTimeout(function () { retryPrewarm(); }, 0);
  ...
  global.addEventListener('ulim-firebase-auth-ready', function (event) {
    ...
    setTimeout(function () {
      ...
      try {
        const dailyPanel = document.getElementById('adminPanelDailyEval');
        // 7.31.22: auth-ready는 출석부 날짜/반/명단을 절대 다시 불러오지 않습니다.
        // 출석부는 attendance-admin-integrated 단일 소유이며, 사용자 조작 또는 저장/추가/제거 완료시에만 다시 조회합니다.
        global.__ULIM_ATTENDANCE_AUTH_READY_RELOAD_DISABLED_73122__ = true;
        if (dailyPanel && dailyPanel.classList.contains('active')) {
          const activeDate = text(document.getElementById('adminDailyEvalDate') && document.getElementById('adminDailyEvalDate').value);
          if (typeof adminLoadClassList === 'function') adminLoadClassList(activeDate, false);
          const dailyClass = document.getElementById('adminDailyEvalClass');
          if (dailyClass && text(dailyClass.value) && typeof adminLoadDailyEvalStudents === 'function') {
            adminLoadDailyEvalStudents(false, { source: 'auth-ready', bypassCache: true });
          }
        }
      } catch (error) {
        safeConsole(
