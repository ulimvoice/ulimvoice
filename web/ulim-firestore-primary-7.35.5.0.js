(function (global) {
  'use strict';
  if (global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_73550__) return;
  global.__ULIM_FIRESTORE_PRIMARY_OPERATIONAL_73550__ = true;

  const VERSION = '2026-08-06.735.05.0.2-firestore-primary-operational-refactor';
  ...
  function openModal(title, bodyHtml, footerHtml, options) {
    ...
    overlay.innerHTML = '<section class="ulim-modal-card" role="dialog" aria-modal="true">' +
      ...
      '<footer class="ulim-modal-foot">' + (footerHtml || '<button type="button" class="admin-btn gray" data-ulim-close="1">닫기</button>') + '</footer></section>';
  ...
  function installShowPanelHook() {
    ...
    const wrapped = function (panelId) {
      ...
      if (panelId === 'adminPanelAttendance') {
        installAttendanceToolbar();
        if (!attendanceDirty.size && currentClassName()) setTimeout(function () { loadRoster(false); }, 30);
      }
      if (panelId === 'adminPanelStaffAccounts' || panelId === NOTES_PANEL_ID) setTimeout(installStaffNotes, 20);
      if (panelId === COURSE_PANEL_ID) setTimeout(loadCourseApplications, 20);
      return result;
    };
  ...
  async function install() {
    installStyles(); ensureLoading(); replaceGlobalLoading(); installModalCloseGuard(); removeLegacyPanels(); installTextObserver(); installGasOperationalGuard();
    await loadCatalog();
    installAttendanceToolbar(); installAttendanceOverrides(); installOperationalDataOverrides(); installMessageOverrides(); installShowPanelHook(); installStaffNotes(); installCourseApplicationAdmin(); subscribeRevision();
    if (!global.__ULIM_STUDENT_ROSTER_EVENT_BOUND_7355002__) {
      global.__ULIM_STUDENT_ROSTER_EVENT_BOUND_7355002__ = true;
      global.addEventListener('ulim-student-roster-updated', handleRevisionChange73550);
    }
