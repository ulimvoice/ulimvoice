(function (global) {
  'use strict';
  if (global.__ULIM_ATTENDANCE_DAILY_UI_73438__) return;
  global.__ULIM_ATTENDANCE_DAILY_UI_73438__ = true;

  const VERSION = '2026-08-01.734.038';
  ...
  function renderAttendanceStable(force) {
    ...
    wrap.querySelectorAll('tr[data-att-index]').forEach(function (row) {
      const input = row.querySelector('input[data-field="memo"]');
      if (input && (input.dataset.memoDirty === '1' || row.dataset.memoDirty === '1')) {
        memoDrafts[row.dataset.attIndex] = input.value;
      }
    });
    ...
    records.forEach(function (record, idx) {
      ...
      html += '<tr data-att-index="' + idx + '">' +
        ...
        '</div></td>' +
        '<td data-label="현재상태">' + (current ? '<span class="ulim-current-status-73438">' + esc(current) + '</span>' : '-') + '</td>' +
  ...
  style.id = 'ulim-attendance-daily-ui-73438-style';
  style.textContent = [
    ...
    '#adminAttendanceTableWrap .ulim-current-status-73438{display:inline-block;max-width:220px;white-space:normal;word-break:keep-all;line-height:1.35;}',
  ...
  console.info('[ULIM 7.34.3.8] stable attendance columns + clean daily status + blank clear installed', VERSION);
