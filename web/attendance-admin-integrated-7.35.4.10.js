(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735410__) return;
  global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735410__ = true;
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-04.735.04.10';

  var VERSION = '2026-08-04.735.04.10';
  var loadSequence = 0;
  ...
  async function loadDirectory(force) {
    ...
    var result = await call('listStudentManagementAdmin7352', { requestId: requestId('attendance-student-detail-list-735410') });
    directoryCache = {
  ...
  function allClassesVisibleClasses735410(directory) {
    ...
    return directory.classes.filter(function (item) {
      ...
      return weekday === 'all' || item.weekday === Number(weekday);
    }).sort(function (a, b) {
  ...
  function renderAllClassesBoardLocal735410() {
    ...
    (allClassesState735410.temporaryRecords || []).forEach(function (record) {
      ...
      if (seen.has(key)) {
        ...
        if (existing && special) existing.specialStatus = special;
        return;
      }
      seen.add(key);
    ...
    function columnHtml(cls, rows) {
      ...
      return '<section data-all-class-drop="' + escapeHtml(cls.classId) + '" style="border:1px solid #cbd5e1;border-radius:14px;background:#f8fafc;min-height:160px;overflow:hidden">'
        ...
        + rows.map(function (entry) {
          ...
          var draggable = !!student.studentUid && !entry.temporary;
          var sub = [student.attendanceNo ? '출결 ' + student.attendanceNo : '', entry.specialStatus].filter(Boolean).join(' · ');
          return '<div draggable="' + (draggable ? 'true' : 'false') + '" data-all-student="' + escapeHtml(student.studentUid || '') + '" data-source-class="' + escapeHtml(entry.sourceClassId || cls.classId) + '" style="padding:9px 10px;border:1px solid ' + (entry.temporary ? '#f59e0b' : '#e2e8f0') + ';border-radius:10px;background:' + (entry.temporary ? '#fffbeb' : '#fff') + ';cursor:' + (draggable ? 'grab' : 'default') + ';font-size:13px"><b>' + escapeHtml(student.name || '') + '</b>' + (sub ? '<div style="font-size:10px;color:' + (entry.temporary ? '#92400e' : '#64748b') + ';margin-top:3px">' + escapeHtml(sub) + '</div>' : '') + '</div>';
        }).join('')
        + '</div></section>';
    }
    ...
    board.querySelectorAll('[draggable="true"][data-all-student]').forEach(function (card) {
      card.addEventListener('dragstart', function () {
        allClassesDragData735410 = { kind: 'existing', studentUid: text(card.getAttribute('data-all-student')), sourceClassId: text(card.getAttribute('data-source-class')), mode: text(document.getElementById('ulimAllClassesMoveMode735410') && document.getElementById('ulimAllClassesMoveMode735410').value) || 'existing' };
        card.style.opacity = '.55';
      });
      card.addEventListener('dragend', function () { card.style.opacity = ''; });
    });
  ...
  async function applyAllClassesDrop735410(drag, targetClassId) {
    ...
    if (drag.kind === 'existing' && drag.sourceClassId === targetClassId) return;
    ...
    try {
      ...
      if (mode === 'makeup') {
        var dateValue = allClassesSelectedDate735410();
        await call('addTemporaryAttendanceAdmin7354', { studentUid: student.studentUid, studentName: student.name, kind: 'makeup', date: dateValue, classId: target.classId, className: target.className, requestId: requestId('all-class-makeup-735410') });
        allClassesState735410.temporaryRecords.push({ studentUid: student.studentUid, studentName: student.name, attendanceNo: student.attendanceNo, classId: target.classId, className: target.className, date: dateValue, specialStatus: '보강' });
      } else {
