(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72907__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72907__ = true;

  const VERSION = "2026-07-31.729.08";
  ...
  async function createRuntime() {
    ...
    try {
      ...
      safeConsole("warn", "[ULIM classroom 7.29.7 persistent cache fallback]", error);
      db = sdk.getFirestore(app);
  ...
  async function signInLegacySessionFresh(reason) {
    ...
    if (!session) throw new Error("교직원 로그인 세션을 찾지 못했습니다.");
  ...
  async function forceReauthenticate(reason) {
    if (state.signingIn) {
      try { await state.signingIn; } catch (_ignore) {}
    }
    state.signingIn = signInLegacySessionFresh(reason || "forced")
      .finally(function () { state.signingIn = null; });
  ...
  safeConsole("info", "[ULIM 7.29.7 fast cached sheet-canonical realtime module]", VERSION);
