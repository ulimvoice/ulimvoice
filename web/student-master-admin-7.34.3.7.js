(function (global) {
  'use strict';
  if (global.__ULIM_STUDENT_MASTER_ADMIN_73437__) return;
  global.__ULIM_STUDENT_MASTER_ADMIN_73437__ = true;

  const VERSION = '2026-08-01.734.03.7';
  ...
  function source(raw) {
    const student = raw || {};
    const sheet = student.sheetSnapshot && typeof student.sheetSnapshot === 'object' ? student.sheetSnapshot : {};
    const local = student.localOverrides && typeof student.localOverrides === 'object' ? student.localOverrides : {};
    function pick(keys) {
      ...
      for (const key of keys) {
        ...
        if (Array.isArray(value) ? value.length : text(value)) return value;
      }
      for (const key of keys) {
    ...
    const loginId = text(pick(['loginId','studentNo']));
    const name = text(pick(['name','studentName']));
    const classValue = pick(['classNames','className','currentClass']);
    const instructorValue = pick(['instructorNames','instructorName','instructor']);
    return {
      studentUid: text(student.studentUid),
      loginId: loginId,
      name: name,
      phone: text(pick(['phone','studentPhone'])),
      parentPhone: text(pick(['parentPhone'])),
      status: statusValue(pick(['status','enrollmentStatus'])),
      classNames: Array.isArray(classValue) ? classValue.map(text).filter(Boolean) : splitList(classValue),
      instructorNames: Array.isArray(instructorValue) ? instructorValue.map(text).filter(Boolean) : splitList(instructorValue),
      memo: text(pick(['memo'])),
      ...
      incomplete: student.incomplete === true || (!loginId && !name),
      firestorePersisted: student.firestorePersisted !== false,
      mergedFromSheet: student.mergedFromSheet === true
    };
  }
  ...
  function updateSummary() {
    ...
    summary.textContent = '표시 ' + filtered.length + '명 / 전체 ' + students.length + '명 · 재원 ' + active + ' · 휴원 ' + leave + ' · 퇴원 ' + withdrawn + ' · 시트만 ' + sheetOnlyCount + '명 · UID 전용 숨김 ' + hiddenIncompleteCount + '명 · 수정 대기 ' + dirtyKeys.size + '명';
  ...
  function render() {
    ...
    const rows = filtered.map(function (student) {
      ...
      return `<tr class="${dirty}" data-row-key="${key}" data-status="${escapeHtml(student.status)}">
        ...
        <td><div class="ulim-student-row-actions"><button type="button" class="admin-btn blue" onclick="ulimStudentMasterSaveRow7343('${key}')"${student.firestorePersisted === false ? ' disabled title="먼저 목록 새로고침으로 Firestore에 반영해주세요."' : ''}>${student.firestorePersisted === false ? '시트 가져오기 필요' : '저장'}</button></div></td>
      </tr>`;
    }).join('');
  ...
  async function listFirestoreDirect() {
    ...
    const all = snap.docs.map(function (doc) { return source(Object.assign({ studentUid: doc.id }, doc.data() || {})); });
    hiddenIncompleteCount = all.filter(function (item) { return item.incomplete; }).length;
    sheetOnlyCount = 0;
    sheetMergeError = '서버 병합 목록 호출 실패';
    students = all.filter(function (item) { return !item.incomplete; });
    ...
    dirtyKeys.clear();
    render();
    return { source: 'firestore-direct', count: students.length, hiddenIncomplete: hiddenIncompleteCount };
  }
  async function listFirestore() {
    try {
      const data = await call('listStudentsAdmin7342', { limit: 3000, mergeSheet: true, requestId: requestId('student-list-merged') });
      students = (Array.isArray(data.students) ? data.students : []).map(source).filter(function (item) { return !item.incomplete; });
      hiddenIncompleteCount = Number(data.hiddenIncomplete || 0);
      sheetOnlyCount = Number(data.sheetOnlyCount || 0);
      sheetMergeError = text(data.sheetMergeError);
      filtered = students.slice();
      dirtyKeys.clear();
      render();
  ...
  async function load(importFromSheet) {
    ...
    loadingPromise = (async function () {
      ...
      if (syncResult) {
        ...
        const notes = [];
        if (sheetOnlyCount) notes.push('시트에만 있는 학생 ' + sheetOnlyCount + '명은 목록 새로고침 후 저장할 수 있습니다.');
        if (hiddenIncompleteCount) notes.push('이름·출결번호가 없는 UID 전용 문서 ' + hiddenIncompleteCount + '건은 목록에서 숨겼습니다.');
        if (sheetMergeError) notes.push('시트 보완 조회 실패: ' + sheetMergeError);
        const base = listResult && listResult.source === 'firestore-direct'
          ? '전체 학생 ' + students.length + '명을 표시했습니다. 서버 목록 함수 대신 Firestore 직접 조회를 사용했습니다.'
          : '전체 학생 ' + students.length + '명을 표시했습니다.';
        setStatus(base + (notes.length ? '\n' + notes.join('\n') : ''), (listResult && listResult.source === 'firestore-direct') || notes.length ? 'warn' : 'ok');
      }
      return true;
  ...
  function install() {
    ...
    try { console.info('[ULIM 7.34.3.7] student sheet merge + incomplete UID hiding installed'); } catch (_ignore) {}
  }
