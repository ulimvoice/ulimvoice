(function (global) {
  'use strict';
  if (global.__ULIM_STUDENT_MANAGEMENT_V2_73545__) return;
  global.__ULIM_STUDENT_MANAGEMENT_V2_73545__ = true;

  const VERSION = '2026-08-04.735.04.5.1';
  ...
  function isSuperAdmin() {
    ...
    return role === 'super' || role === 'superadmin' || role === 'fulladmin'
      || role === normalize('전체관리자') || role === normalize('전체관리') || role === normalize('원장');
  }
  ...
  function cardHtml() {
    return `<div id="${CARD_ID}" class="admin-card admin-full-only">
      ...
      <div class="ulim-student-help"><b>필수입력은 학생명과 학생 전화번호뿐입니다.</b> 생년월일·보호자전화·등록일·수강반·메모는 빈칸으로 저장하거나 나중에 수정할 수 있습니다. 수강반 선택을 바꾸고 저장하면 선택된 반목록이 최종 수강반으로 적용됩니다. 기존 반이 빠지거나 다른 반으로 바뀌면 반이동으로 자동 처리됩니다. 신규·보강은 처리구분에서 별도로 선택하세요. 등록 취소 후에는 학생 삭제 버튼으로 최종 삭제합니다.</div>
  ...
  function rowData(key) {
    ...
    let registrationType = text(document.getElementById(key + '_operation') && document.getElementById(key + '_operation').value) || 'existing';
    const classSelectionChanged = !sameTextSet(classIds, originalClassIds);
    if (classSelectionChanged && registrationType === 'existing') registrationType = 'class_move';
    return {
      studentUid: uid,
  ...
  function render() {
    ...
    const rows = filtered.map(function (student) {
      ...
      const actions = cancelled
        ? '<button type="button" class="admin-btn red" onclick="ulimStudentManagementRetire7352(\'' + key + '\',\'delete\')">학생 삭제</button>'
        : '<button type="button" class="admin-btn blue" onclick="ulimStudentManagementSaveRow7352(\'' + key + '\')">저장</button>' + (student.retryable ? '<button type="button" class="admin-btn" onclick="ulimStudentManagementRetry7352(\'' + key + '\')">재시도</button>' : '') + '<button type="button" class="admin-btn" onclick="ulimStudentManagementRetire7352(\'' + key + '\',\'withdraw\')">퇴원 처리</button><button type="button" class="admin-btn" onclick="ulimStudentManagementRetire7352(\'' + key + '\',\'cancel\')">등록 취소</button>';
      return `<tr${rowClass ? ' class="' + rowClass + '"' : ''} data-row-key="${key}">
        <td>${statusSelect}</td>
        ...
        <td><select id="${key}_operation" data-row-key="${key}"${cancelled ? ' disabled' : ''}><option value="existing">기본정보·반 추가</option><option value="new">신규 표시</option><option value="class_move">반이동</option><option value="makeup">보강</option></select><div id="${key}_operation_note" class="ulim-operation-note">반이동: 처리구분 선택 후 이동할 새 반만 선택<br>보강: 선택 반·처리일에만 추가</div></td>
  ...
  function updateRowClassPreview(key) {
    ...
    if (operation) {
      ...
      if (changed && mode === 'existing') {
        operation.value = 'class_move';
        operation.dataset.ulimAutoClassMove = '1';
        if (dateInput && !dateInput.value) dateInput.value = today();
      } else if (!changed && operation.dataset.ulimAutoClassMove === '1') {
        operation.value = 'existing';
        delete operation.dataset.ulimAutoClassMove;
      }
    }
    const effectiveMode = operation ? (text(operation.value) || 'existing') : 'existing';
    if (note) {
      if (effectiveMode === 'class_move') note.innerHTML = '<b>현재 선택된 반목록으로 교체됩니다.</b><br>목록에서 빠진 기존 반은 처리일부터 자동 종료됩니다.';
      else if (effectiveMode === 'makeup') note.innerHTML = '보강: 선택 반·처리일에만 추가';
      else note.innerHTML = '수강반 선택을 바꾸면 반이동으로 자동 처리됩니다.<br>보강은 처리구분에서 별도 선택하세요.';
    }
  ...
  function handleOperationModeChange(key) {
    ...
    const mode = text(operation.value) || 'existing';
    if (mode === 'class_move') {
      if (note) note.innerHTML = '<b>현재 선택된 반목록으로 교체됩니다.</b><br>목록에서 빠진 기존 반은 처리일부터 자동 종료됩니다.';
      const dateInput = document.getElementById(key + '_operation_date');
      if (dateInput && !dateInput.value) dateInput.value = today();
    } else if (note) {
      note.innerHTML = mode === 'makeup'
        ? '보강: 선택 반·처리일에만 추가'
        : '수강반 선택을 바꾸면 반이동으로 자동 처리됩니다.<br>보강은 처리구분에서 별도 선택하세요.';
    }
  ...
  async function saveKeys(keys) {
    ...
    const operationSummary = ['기본정보·반 추가 ' + Number(operationCounts.existing || 0), '신규 ' + Number(operationCounts.new || 0), '반이동 ' + Number(operationCounts.class_move || 0), '보강 ' + Number(operationCounts.makeup || 0)].join(' / ');
    ...
    if (!confirm('학생 ' + edits.length + '명의 변경사항을 저장할까요?\n' + operationSummary + (moveDetails.length ? '\n\n반이동 대상\n' + moveDetails.join('\n') : '') + '\n\n수강반 선택을 변경한 학생은 선택된 반목록으로 완전히 교체되고, 목록에서 빠진 기존 반은 처리일부터 종료됩니다. 보강은 처리일(빈칸이면 오늘)의 해당 반에만 추가됩니다.')) return;
    ...
    try {
      ...
      else setStatus('학생정보 ' + edits.length + '명의 변경사항과 처리구분을 앱 운영자료에 저장했습니다. 시트 기록은 백그라운드에서 진행됩니다.', 'ok');
    } catch (error) {
  ...
  async function retire(key, mode) {
    ...
    if (!studentUid) return;
    const label = mode === 'delete' ? '학생 삭제' : (mode === 'cancel' ? '등록 취소' : '퇴원 처리');
...
/* ULIM 7.35.4.5.1: attach admin-only add/remove controls to the final compact attendance renderer. */
(function (global) {
  'use strict';
  if (global.__ULIM_ATTENDANCE_ADMIN_UI_ATTACH_735451__) return;
  global.__ULIM_ATTENDANCE_ADMIN_UI_ATTACH_735451__ = true;

  var attachTimer = 0;
  var observer = null;

  function normalizedRole735451(value) {
    return String(value == null ? '' : value).normalize('NFKC').toLowerCase().replace(/\s+/g, '');
  }

  function isFullAdmin735451() {
    try {
      if (typeof global.adminIsFullAdmin === 'function') return !!global.adminIsFullAdmin();
    } catch (_ignore) {}
    var info = global.adminInfo || {};
    var role = normalizedRole735451(info.firebaseRole || info.role);
    return role === 'super' || role === 'superadmin' || role === 'fulladmin'
      || role === normalizedRole735451('전체관리자')
      || role === normalizedRole735451('전체관리')
      || role === normalizedRole735451('원장');
  }

  function ensureStyle735451() {
    if (document.getElementById('ulim-attendance-admin-ui-attach-style-735451')) return;
    var style = document.createElement('style');
    style.id = 'ulim-attendance-admin-ui-attach-style-735451';
    style.textContent = [
      '#adminAttendanceTableWrap .ulim-attendance-admin-manage-head-735451{width:84px;text-align:center;white-space:nowrap;}',
      '#adminAttendanceTableWrap .ulim-attendance-admin-manage-cell-735451{text-align:center;white-space:nowrap;}',
      '#adminAttendanceTableWrap .ulim-attendance-admin-delete-btn-735451{border:0;border-radius:10px;padding:8px 12px;background:#ef4444;color:#fff;font-weight:900;cursor:pointer;}',
      '#adminAttendanceTableWrap .ulim-attendance-admin-delete-btn-735451:hover{filter:brightness(.96);}',
      '#adminAttendanceTableWrap .ulim-attendance-admin-add-btn-735451{background:#f97316!important;color:#fff!important;}',
      '@media(max-width:700px){',
      '#adminAttendanceTableWrap .ulim-attendance-admin-manage-cell-735451{display:grid!important;grid-template-columns:104px minmax(0,1fr)!important;}',
      '#adminAttendanceTableWrap .ulim-attendance-admin-delete-btn-735451{justify-self:end;min-width:88px;min-height:42px;}',
      '}'
    ].join('');
    document.head.appendChild(style);
  }

  function removeControls735451(wrap) {
    var nodes = wrap.querySelectorAll('.ulim-attendance-admin-ui-735451');
    Array.prototype.forEach.call(nodes, function (node) { node.remove(); });
  }

  function addEmptyButton735451(wrap) {
    if (wrap.querySelector('.ulim-attendance-admin-add-btn-735451')) return;
    var row = document.createElement('div');
    row.className = 'ulim-attendance-admin-ui-735451';
    row.style.cssText = 'display:flex;justify-content:flex-end;margin-top:12px;';
    var button = document.createElement('button');
    button.type = 'button';
    button.className = 'admin-btn orange ulim-attendance-admin-add-btn-735451';
    button.textContent = '학생추가';
    button.addEventListener('click', function () {
      if (typeof global.ulimAttendanceOpenAddModal73545 === 'function') global.ulimAttendanceOpenAddModal73545();
    });
    row.appendChild(button);
    wrap.appendChild(row);
  }

  function addTableControls735451(wrap, table) {
    var headRow = table.querySelector('thead tr');
    if (headRow && !headRow.querySelector('.ulim-attendance-admin-manage-head-735451')) {
      var th = document.createElement('th');
      th.className = 'ulim-attendance-admin-ui-735451 ulim-attendance-admin-manage-head-735451';
      th.textContent = '관리';
      headRow.appendChild(th);
    }

    var rows = table.querySelectorAll('tbody tr[data-att-index]');
    Array.prototype.forEach.call(rows, function (row) {
      if (row.querySelector('.ulim-attendance-admin-manage-cell-735451')) return;
      var index = Number(row.getAttribute('data-att-index'));
      if (!Number.isFinite(index)) return;
      var td = document.createElement('td');
      td.className = 'ulim-attendance-admin-ui-735451 ulim-attendance-admin-manage-cell-735451';
      td.setAttribute('data-label', '관리');
      var button = document.createElement('button');
      button.type = 'button';
      button.className = 'ulim-attendance-admin-delete-btn-735451';
      button.textContent = '삭제';
      button.addEventListener('click', function () {
        if (typeof global.ulimAttendanceRemoveRow73545 === 'function') global.ulimAttendanceRemoveRow73545(index);
      });
      td.appendChild(button);
      row.appendChild(td);
    });

    var actions = wrap.querySelector('.ulim-attendance-actions-559');
    if (!actions) {
      var candidates = wrap.querySelectorAll('div');
      for (var i = candidates.length - 1; i >= 0; i -= 1) {
        if (candidates[i].querySelector('button[onclick*="adminSaveAttendanceFromTable"]')) {
          actions = candidates[i];
          break;
        }
      }
    }
    if (actions && !actions.querySelector('.ulim-attendance-admin-add-btn-735451')) {
      var addButton = document.createElement('button');
      addButton.type = 'button';
      addButton.className = 'admin-btn orange ulim-attendance-admin-ui-735451 ulim-attendance-admin-add-btn-735451';
      addButton.textContent = '학생추가';
      addButton.addEventListener('click', function () {
        if (typeof global.ulimAttendanceOpenAddModal73545 === 'function') global.ulimAttendanceOpenAddModal73545();
      });
      actions.insertBefore(addButton, actions.firstChild);
    }
  }

  function attachControls735451() {
    var wrap = document.getElementById('adminAttendanceTableWrap');
    if (!wrap) return;
    if (!isFullAdmin735451()) {
      removeControls735451(wrap);
      return;
    }
    ensureStyle735451();
    var table = wrap.querySelector('table');
    if (!table) {
      addEmptyButton735451(wrap);
      return;
    }
    addTableControls735451(wrap, table);
  }

  function scheduleAttach735451() {
    clearTimeout(attachTimer);
    attachTimer = setTimeout(attachControls735451, 0);
  }

  function hookRenderer735451() {
    var original = global.adminRenderAttendanceTable;
    if (typeof original !== 'function' || original.__ulimAttendanceAdminUi735451Wrapped) return;
    var wrapped = function () {
      var result = original.apply(this, arguments);
      scheduleAttach735451();
      return result;
    };
    wrapped.__ulimAttendanceAdminUi735451Wrapped = true;
    wrapped.__ulimAttendanceAdminUi735451Original = original;
    global.adminRenderAttendanceTable = wrapped;
    try { adminRenderAttendanceTable = wrapped; } catch (_ignore) {}
  }

  function install735451() {
    hookRenderer735451();
    var wrap = document.getElementById('adminAttendanceTableWrap');
    if (wrap && !observer) {
      observer = new MutationObserver(function () { scheduleAttach735451(); });
      observer.observe(wrap, { childList: true, subtree: true });
    }
    scheduleAttach735451();
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install735451, { once: true });
  else install735451();
  global.addEventListener('pageshow', function () { setTimeout(install735451, 60); });
  global.addEventListener('ulim-firebase-auth-ready', function () { setTimeout(install735451, 60); });
  setTimeout(install735451, 250);
  setTimeout(install735451, 1200);
})(typeof window !== 'undefined' ? window : globalThis);
