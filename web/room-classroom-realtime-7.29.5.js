(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72905__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72905__ = true;
  ...
  const APP_NAME = "ulim-room-classroom-realtime-7295";
  const SESSION_FINGERPRINT_KEY = "ulimRealtimeLegacySessionFingerprint721";
  const REQUEST_PREFIX = "RT7295";
  ...
  async function subscribeClassroom(date) {
    ...
    state.classroomUnsubscribe = runtime.sdk.onSnapshot(ref, function (snapshot) {
      ...
      if (state.sheetAuthorityLoading[date]) return;
      ...
      try {
        ...
        if (summary && Date.now() >= state.suppressRealtimeSummaryUntil) {
          summary.textContent = "시트에 확정된 다른 강사의 변경사항이 실시간 반영됐습니다.";
        }
        setStatus("ready", "실시간 연결됨");
      } catch (error) {
  ...
  function isClassroomSlotSaving729(date, room, hour) {
    return state.savingClassroomSlots.has(classroomSlotSavingKey729(date, room, hour));
  }
  ...
  function applyAuthoritativeClassroomRows7295(date, records, summaryText) {
    ...
    try {
      adminClassroomUsageRows = normalized.map(function (record) {
        return Object.assign({}, record);
      });
      adminClassroomUsageLoadedDate = date;
  ...
  async function mirrorSheetAuthorityToFirestore7295(date, records, reason) {
    ...
    const promise = (async function () {
      ...
      try {
        if (options.loadOptions !== false && typeof global.adminLoadClassroomUsageOptions === "function") {
          try { await global.adminLoadClassroomUsageOptions(true); } catch (_ignore) {}
        }
  ...
  async function writeClassroomSheetFirst7295(operation, payload, date, reason) {
    const requestId = makeRequestId("SHEET-FIRST");
    const result = await runClassroomSheetWrite7294(
      { jobId: requestId, requestId: requestId },
      operation,
      payload
    );
    const rows = await loadAuthoritativeClassroomRows727(date, result);
    const records = applyAuthoritativeClassroomRows7295(
      date,
      rows,
      classroomSheetConflict7295(result)
        ? "Google Sheets의 기존 사용 기록을 우선 반영했습니다."
        : "Google Sheets 저장 완료 · Firestore 실시간 반영 중"
    );
    ...
    try {
      ...
      mirrorError = error;
      state.classroomBaselineReady[date] = false;
      safeConsole("warn", "[ULIM classroom 7.29.5 write mirror]", error);
      setStatus("error", "시트 저장 완료 · 실시간 반영 재시도");
      setTimeout(function () {
        refreshClassroomFromSheet7295(date, {
          showOverlay: false,
          forceMirror: true,
          reason: "SHEET-WRITE-RETRY"
        });
      }, 2000);
    }

    return { result: result || {}, records: records, mirrorError: mirrorError };
  }
  ...
  async function saveClassroomFirestoreFirst727(silentConfirm, forceOverride) {
    ...
    if (!silentConfirm) {
      ...
      if (!confirm([
        ...
        "",
        "Google Sheets에 먼저 " + actionText + "하고",
        "확정된 시트 현황을 Firestore에 실시간 반영할까요?"
      ].join("\n"))) return;
    }
    ...
    try {
      if (typeof showLoading === "function") {
        showLoading(forceOverride
          ? "Google Sheets에서 배정/용도 변경 중..."
          : "Google Sheets에 강의실 사용 기록 중...");
      }
  ...
  function installWrappers() {
    ...
    if (typeof global.adminLoadClassroomUsage === "function") {
      ...
      global.adminLoadClassroomUsage = async function (showOverlay) {
        const date = dateKeyFromClassroom();
        return refreshClassroomFromSheet7295(date, {
          showOverlay: showOverlay !== false,
          forceMirror: true,
          reason: "SHEET-MANUAL-LOAD"
        });
      };
    }
    ...
    if (typeof global.adminReleaseClassroomUsage === "function") {
      ...
      global.adminReleaseClassroomUsage = async function (recordId) {
        ...
        state.classroomPrimarySaving = true;
        try {
          if (typeof showLoading === "function") showLoading("Google Sheets에서 강의실 사용 해제 중...");
          const output = await writeClassroomSheetFirst7295(
            "classroomReleaseSlot",
            {
  ...
  async function start() {
    ...
    const date = dateKeyFromClassroom();
    if (date) {
      subscribeClassroom(date);
      setTimeout(function () {
        refreshClassroomFromSheet7295(date, {
          showOverlay: false,
          forceMirror: true,
          reason: "SHEET-INITIAL-LOAD"
        }).catch(function (error) {
          safeConsole("warn", "[ULIM classroom 7.29.5 initial sheet load]", error);
        });
      }, 250);
    }
    ...
    if (!state.sheetPollTimer) {
      state.sheetPollTimer = setInterval(function () {
        ...
        refreshClassroomFromSheet7295(currentDate, {
          showOverlay: false,
          forceMirror: false,
          loadOptions: false,
          reason: "SHEET-POLL"
        }).catch(function (error) {
          safeConsole("warn", "[ULIM classroom 7.29.5 sheet poll]", error);
        });
      }, 10000);
    }
  ...
  safeConsole("info", "[ULIM 7.29.5 classroom sheet-direct module]", VERSION);
