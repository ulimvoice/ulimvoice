(function (global) {
  ...
  function render() {
    ...
    const rows = filtered.map(function (student) {
      ...
      return `<tr${rowClass ? ' class="' + rowClass + '"' : ''} data-row-key="${key}" data-student-uid="${escapeHtml(student.studentUid)}">
        <td>${statusSelect}</td><td><input value="${escapeHtml(student.attendanceNo)}" readonly><div class="ulim-student-uid">UID ${escapeHtml(maskedUid(student.studentUid))}</div></td>
  ...
  function handleOperationModeChange(key) {
    ...
    if (note) note.textContent = mode === 'class_move' ? '처리일부터 기존 반을 종료하고 선택 반으로 이동합니다.' : (mode === 'makeup' ? '선택 날짜에만 보강으로 추가합니다.' : (mode === 'new' ? '선택 반을 추가하고 기존 반은 유지합니다.' : '선택한 반목록을 최종 수강반으로 저장합니다.'));
  ...
  async function load(forceDiscardDirty) {
    ...
    loadingPromise = (async function () {
      ...
      const result = await call('listStudentManagementAdmin7352', { requestId: requestId('student-list-7355002') });
      ...
      students = (Array.isArray(result.students) ? result.students : []).map(normalizeStudent); filtered = students.slice(); dirtyKeys.clear(); renderCreateClasses(); render();
    ...
    })().catch(function (error) { setStatus(text(error && error.message) || '학생정보를 불러오지 못했습니다.', 'error'); return false; }).finally(function () { loadingPromise = null; });
    return loadingPromise;
  }
  async function reloadClasses() {
    try {
      showLoading('운영 반 목록을 다시 불러오는 중...');
      const result = await call('getStudentClassCatalogAdmin7352', { requestId: requestId('class-list-7355002') });
      ...
      renderCreateClasses(); if (!dirtyKeys.size) render(); setStatus('운영 반 ' + classes.length + '개를 다시 불러왔습니다.', 'ok');
  ...
  function validateInput(input) {
    ...
    if (input.enrollmentStatus !== 'withdrawn' && (!Array.isArray(input.classIds) || !input.classIds.length)) return '수강반을 하나 이상 선택해주세요.';
    return '';
  }
  async function createStudent() {
    ...
    try {
      ...
      try { global.dispatchEvent(new CustomEvent('ulim-student-roster-updated', { detail:{ studentUids:created.studentUid ? [created.studentUid] : [] } })); } catch (_ignore) {}
  ...
  async function saveKeys(keys) {
    ...
    try {
      ...
      if (edits.length === 1) result={results:[await call('updateStudentAdmin7352', Object.assign({},edits[0],{requestId:requestId('student-update-7355002')}))]};
      else result=await call('updateStudentsBatchAdmin7352',{edits:edits,requestId:requestId('student-batch-7355002')});
      ...
      edits.forEach(function (edit) { if (results.length && !succeeded.has(edit.studentUid)) return; const student=students.find(function(item){return item.studentUid===edit.studentUid;}); if(student){student.name=edit.name;student.birthDate=edit.birthDate;student.studentPhone=edit.studentPhone;student.parentPhone=edit.parentPhone;student.enrollmentStatus=edit.enrollmentStatus;student.initialRegisteredDate=edit.initialRegisteredDate;student.memo=edit.memo;student.selectedClassIds=unique(edit.classIds);student.instructorNames=selectedClassDetails(edit.classIds).map(function(item){return item.instructorName;}).filter(Boolean);student.sheetSyncState='backup_0600';student.sheetSyncMessage='매일 오전 6시 백업';student.retryable=false;} const key=selectedKeys.find(function(candidate){return (rowKeyMap.get(candidate)||'')===edit.studentUid;}); if(key) dirtyKeys.delete(key); });
      applyFilter(); try { global.dispatchEvent(new CustomEvent('ulim-student-roster-updated',{detail:{studentUids:edits.map(function(edit){return edit.studentUid;})}})); } catch(_ignore){}
  ...
  async function retireClassCatalog7354(classId) {
    ...
    try{showLoading('반 사용중지 처리 중...');const result=await call('retireClassCatalogAdmin7354',{classId:classId,requestId:requestId('class-retire-7355002')});await reloadClasses();setStatus(text(result.message)||'반을 사용중지했습니다.','ok');}catch(error){setStatus(text(error&&error.message)||'반을 사용중지하지 못했습니다.','error');}finally{hideLoading();}
  }
