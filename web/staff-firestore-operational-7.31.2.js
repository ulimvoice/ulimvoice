(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73102__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73102__ = true;

  const VERSION = '2026-07-31.731.02';
  const CACHE_PREFIX = 'ulim_staff_fsop_7312_';
  ...
  async function bootstrapDateOnce(rt, date, reason, datasets) {
    ...
    const key = 'ulim_staff_fsop_7312_bootstrap_' + owner() + '_' + text(date) + '_' + datasetKey;
    ...
    const promise = (async function () {
      ...
      try {
        ...
        if (forceSheetSync) {
          ...
          try {
            await syncDateFromSheets(rt, ctx.date, 'attendance_button_refresh', true, ['attendance']);
          } catch (error) {
            sheetSyncError = error;
            safeConsole('warn', '[ULIM 7.31.2 attendance sheet refresh]', error);
          }
        }
        ...
        safeConsole('warn', '[ULIM 7.31.2 attendance Firestore read]', error);
        if (!cached) {
          const summary = document.getElementById('adminAttendanceSummary');
  ...
  global.adminLoadDailyEvalStudents = async function (forceSheetSync) {
    ...
    async function readAndRender(rt, message) {
      const data = callableData(await rt.staffOperational731.getDaily(ctx));
    ...
    try {
      ...
      if (forceSheetSync) {
        ...
        const attendanceSyncPromise = syncDateFromSheets(
          ...
          safeConsole('warn', '[ULIM 7.31.2 daily attendance sheet refresh]', error);
          return null;
        });
        ...
        const dailySyncPromise = syncDateFromSheets(
          ...
          safeConsole('warn', '[ULIM 7.31.2 daily evaluation sheet refresh]', error);
          return null;
        });
      ...
      if (!result.roster.length) {
        ...
        try {
          ...
          attendanceSyncError = error;
          safeConsole('warn', '[ULIM 7.31.2 daily attendance bootstrap]', error);
        }
      }
      ...
      if (!result.savedRows.length) {
        bootstrapDateOnce(rt, ctx.date, 'daily_evaluation_empty_bootstrap', ['dailyEvaluations'])
          .then(async function (refreshed) {
            ...
            catch (error) { safeConsole('warn', '[ULIM 7.31.2 daily background refresh]', error); }
          })
          .catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.2 daily evaluation bootstrap]', error);
          });
      }
      ...
      safeConsole('warn', '[ULIM 7.31.2 daily Firestore read]', error);
  ...
  const CLASS_CACHE_PREFIX = 'ulim_staff_classlist_7312_';
  ...
  function readPersistentClassList(date) {
    try {
      ...
      const last = JSON.parse(localStorage.getItem(CLASS_CACHE_PREFIX + owner() + '_last') || 'null');
  ...
  async function legacyClassListForDate(date, force) {
    ...
    try {
      ...
      safeConsole('warn', '[ULIM 7.31.2 legacy class list]', error);
      return [];
    }
  }
  ...
  global.adminLoadClassList = async function (dateOverride, force) {
    ...
    const promise = (async function () {
      ...
      try {
        ...
        safeConsole('warn', '[ULIM 7.31.2 Firestore class list]', error);
      }
      ...
      if (force || !firestoreClasses.length) {
        try {
          ...
          safeConsole('warn', '[ULIM 7.31.2 class list sheet sync]', error);
        }
      }
  ...
  async function autoBootstrapTodayIfNeeded() {
    ...
    try {
      ...
      safeConsole('warn', '[ULIM 7.31.2 login auto bootstrap]', error);
      return false;
    }
  }
  ...
  function prewarm() {
    ...
    }).catch(function (error) { safeConsole('warn', '[ULIM 7.31.2 Firebase prewarm]', error); });
  }
  ...
  style.id = 'ulim-staff-firestore-operational-7312-style';
