(function (global) {
  ...
  if (global.__ULIM_STAFF_LOGIN_FAST_7319__) return;
  global.__ULIM_STAFF_LOGIN_FAST_7319__ = true;

  const VERSION = '2026-07-31.731.09';
  const WARM_KEY = 'ulimStaffLoginWarm7319At';
    ...
    try {
      ...
      if (room && typeof room.preloadRuntime === 'function') {
        return Promise.resolve(room.preloadRuntime()).catch(function (error) {
          safeConsole('warn', '[ULIM 7.31.9 Firebase preload delayed]', error);
          return null;
        });
      }
    } catch (_ignore) {}
  ...
  function prewarmLoginServer(force) {
    ...
    warmPromise = Promise.resolve()
      ...
      .then(function (data) {
        ...
        safeConsole('info', '[ULIM 7.31.9 login server warmed]', data && data.elapsedMs);
        return true;
      })
      .catch(function (error) {
        safeConsole('warn', '[ULIM 7.31.9 login prewarm skipped]', error);
        return false;
      })
    ...
    setTimeout(function () {
      try {
        ...
        if (room && typeof room.forceReauthenticate === 'function') {
          room.forceReauthenticate('staff-login-bundled-proof').catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.9 Firebase login background]', error);
          });
        }
      } catch (_ignore) {}
      ...
      try {
        if (typeof global.adminLoadInitialData === 'function') {
          Promise.resolve(global.adminLoadInitialData()).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.9 staff initial background]', error);
          });
        }
      } catch (_ignore) {}
      ...
      safeConsole('info', '[ULIM 7.31.9 staff login shell ready]', {
  ...
  async function fastAdminLogin() {
    ...
    loginPromise = (async function () {
      ...
      try {
        if (typeof global.clearAdminSensitiveScreen_ === 'function') global.clearAdminSensitiveScreen_('login-start-7319');
      } catch (_ignore) {}
  ...
  function install() {
    ...
    safeConsole('info', '[ULIM 7.31.9 staff login final acceleration installed]', VERSION);
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install, { once: true });
  else install();

  global.__ULIM_STAFF_LOGIN_FAST_7319_API__ = Object.freeze({
    version: VERSION,
