(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735422__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-08.735.04.22-schedule-change-server-class-owner';

  var VERSION = '2026-08-08.735.04.22-schedule-change-server-class-owner';
  var loadSequence = 0;
  ...
  function renderAllClassesBoardLocal735410() {
    ...
    directory.students.forEach(function (student) {
      if (student.registrationCancelled === true || student.enrollmentStatus === 'leave' || student.enrollmentStatus === 'withdrawn') return;
      ...
      return '<section data-all-class-drop="' + escapeHtml(cls.classId) + '" style="border:1px solid #cbd5e1;border-radius:14px;background:#f8fafc;min-height:160px;overflow:hidden">'
        ...
        + rows.map(function (entry) {
          ...
          return '<div draggable="' + (draggable ? 'true' : 'false') + '" data-all-student="' + escapeHtml(student.studentUid || '') + '" data-source-class="' + escapeHtml(entry.sourceClassId || cls.classId) + '" data-temporary-index="' + temporaryIndex + '" data-temporary-card="' + (isTemporaryMove ? '1' : '0') + '" style="padding:9px 10px;border:1px solid ' + (entry.temporary || entry.specialStatus ? '#f59e0b' : '#e2e8f0') + ';border-radius:10px;background:' + (entry.temporary || entry.specialStatus ? '#fffbeb' : '#fff') + ';cursor:' + (draggable ? 'grab' : 'default') + ';font-size:13px">'
            ...
            + '<div style="min-width:0;flex:1"><div style="display:flex;align-items:center;gap:7px"><b style="color:#166534;font-size:13px">' + escapeHtml(student.name || '') + '</b>'
            + (student.studentUid ? '<button type="button" class="ulim-att-student-settings735421" data-whole-detail-student="' + escapeHtml(student.studentUid || '') + '" title="학생정보 확인·수정" aria-label="학생정보 확인·수정">⚙</button>' : '') + '</div>'
  ...
  function install() {
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355016__ = 'attendance-admin-integrated-7.35.4.22';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355015__ = 'attendance-admin-integrated-7.35.4.22';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355014__ = 'attendance-admin-integrated-7.35.4.22';
