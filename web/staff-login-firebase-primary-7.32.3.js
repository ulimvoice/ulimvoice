(function (global) {
  ...
  if (global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7323__) return;
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7323__ = true;

  const VERSION = '2026-07-31.732.03';
  ...
  async function readCurrentStaffProfile(rt, fallbackId) {
    ...
    const tokenResult = await rt.sdk.getIdTokenResult(user, true);
  ...
  function showStaffShell() {
    ...
    /*
     * 7.32.3: 기존 UI 패치들은 adminToken이 생길 때까지 교직원 탭을 잠급니다.
  ...
  async function migratePasswordCredential(rt, loginId, password, migrationProof) {
    ...
    await rt.sdk.getIdToken(credential.user, true);
    primaryCredentialReady = true;
    safeConsole('info', '[ULIM 7.32.3 password credential migrated]', {
    ...
    legacyCompatibilityPromise = Promise.resolve()
      ...
      .then(async function (data) {
        ...
        try {
          ...
          safeConsole('warn', '[ULIM 7.32.3 legacy initial data]', error);
        }
        try {
        ...
        safeConsole('info', '[ULIM 7.32.3 GAS compatibility ready]', {
      ...
      .catch(function (error) {
        ...
        safeConsole('warn', '[ULIM 7.32.3 GAS compatibility failed]', error);
        throw error;
      })
  ...
  async function directFirebaseLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.3 Firebase-primary shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      uid: credential.user.uid,
  ...
  async function legacyFirstLogin(rt, loginId, password, startedAt) {
    ...
    safeConsole('info', '[ULIM 7.32.3 legacy first shell displayed]', {
      elapsedMs: Date.now() - startedAt,
      serverTiming: data && data.loginTiming || null
    });
      ...
      setTimeout(function () {
        migratePasswordCredential(authenticated, loginId, password, text(data && data.firebasePasswordMigrationProof))
          ...
          .catch(function (error) {
            ...
            safeConsole('warn', '[ULIM 7.32.3 credential migration deferred]', error);
          });
      }, 0);
    }
  ...
  async function firebasePrimaryAdminLogin() {
    ...
    loginPromise = (async function () {
      ...
      try {
        ...
        safeConsole('info', '[ULIM 7.32.3 legacy migration fallback]', text(firebaseError && firebaseError.code));
  ...
  function installLogoutWrapper() {
    ...
    const wrapped = async function () {
      ...
      try {
        ...
        safeConsole('warn', '[ULIM 7.32.3 Firebase signOut delayed]', error);
      }
      try {
        return await original.apply(this, arguments);
      } finally {
        /* signOut가 네트워크 오류로 지연돼도 자동복원은 로그아웃 플래그로 계속 차단됩니다. */
        safeConsole('info', '[ULIM 7.32.3 explicit staff logout]', { signOutCompleted: signOutCompleted });
      }
    };
  ...
  async function validateStoredLegacySessionQuietly() {
    ...
    try {
      ...
      if (!(error && error.silent)) safeConsole('warn', '[ULIM 7.32.3 stored GAS session check delayed]', error);
    }
    return false;
  }
    ...
    try {
      ...
      rt.sdk.onAuthStateChanged(rt.auth, async function (user) {
        ...
        try {
          ...
          safeConsole('info', '[ULIM 7.32.3 Firebase-primary restored]', {
            uid: user.uid,
            role: profile.firebaseRole,
          ...
          safeConsole('warn', '[ULIM 7.32.3 stored Firebase session rejected]', error);
          try { await rt.sdk.signOut(rt.auth); } catch (_ignore) {}
        }
      });
    } catch (error) {
      restoreListenerInstalled = false;
      safeConsole('warn', '[ULIM 7.32.3 restore unavailable]', error);
    }
  }
  ...
  function install() {
    ...
    if (!global.__ULIM_LEGACY_SESSION_STATUS_7323__) {
      global.__ULIM_LEGACY_SESSION_STATUS_7323__ = true;
    ...
    safeConsole('info', '[ULIM 7.32.3 Firebase-primary staff login installed]', VERSION);
  }
  ...
  global.__ULIM_STAFF_LOGIN_FIREBASE_PRIMARY_7322_API__ = Object.freeze({
    version: VERSION,
