(function (global) {
  'use strict';
  if (global.__ULIM_STUDENT_MANAGEMENT_V2_73541__) return;
  global.__ULIM_STUDENT_MANAGEMENT_V2_73541__ = true;

  const VERSION = '2026-08-03.735.04.1';
  ...
  function cardHtml() {
    return `<div id="${CARD_ID}" class="admin-card admin-full-only">
      ...
      <div class="ulim-student-help"><b>필수입력은 학생명과 학생 전화번호뿐입니다.</b> 생년월일·보호자전화·등록일·수강반·메모는 빈칸으로 저장하거나 나중에 수정할 수 있습니다. 처리구분에서 신규·반이동·보강을 선택하면 학생-반 관계와 해당 수업일 운영자료에 반영됩니다.</div>
  ...
  function render() {
    ...
    const rows = filtered.map(function (student) {
      ...
      return `<tr${dirty} data-row-key="${key}">
        ...
        <td><select id="${key}_operation" data-row-key="${key}"><option value="existing">기본정보·반 추가</option><option value="new">신규 표시</option><option value="class_move">반이동</option><option value="makeup">보강</option></select><div class="ulim-operation-note">반이동: 선택 반으로 교체<br>보강: 선택 반·처리일에만 추가</div></td>
        <td><input id="${key}_operation_date" type="date" data-row-key="${key}" value=""></td>
  ...
  async function saveKeys(keys) {
    ...
    if (!confirm('학생 ' + edits.length + '명의 변경사항을 저장할까요?\n' + operationSummary + '\n\n반이동은 선택한 반목록으로 교체되고, 보강은 처리일(빈칸이면 오늘)의 해당 반에만 추가됩니다.')) return;
  ...
  async function importSheets() {
    ...
    try {
      ...
      const message = '시트 ' + Number(counts.total || 0) + '건\n자동 연결 ' + Number(counts.matched || 0) + '건\n변경 확인 ' + Number(counts.changed || 0) + '건\n미연결 ' + Number(counts.unmatched || 0) + '건\n중복후보 ' + Number(counts.ambiguous || 0) + '건\n\n자동 연결된 기본정보만 앱에 반영할까요?\n반목록은 이 기능에서 임의로 덮어쓰지 않습니다.';
      hideLoading();
  ...
  async function retire(key, mode) {
    ...
    if (!studentUid) return;
    const label = mode === 'cancel' ? '신규 등록 취소' : '퇴원 처리';
    if (!confirm(text(student.name) + ' 학생을 ' + label + '할까요?\n과거 출석·평가 기록은 삭제하지 않습니다.')) return;
    try {
      showLoading(label + ' 중...');
