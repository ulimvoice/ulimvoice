(function (global) {
  'use strict';
  if (global.__ULIM_DATA_AUTHORITY_735414__) return;
  global.__ULIM_DATA_AUTHORITY_735414__ = true;
  global.ULIM_DATA_AUTHORITY_VERSION = '2026-08-04.735.04.14-six-stage-firestore-authority';
  ...
  function updateButtons() {
    ...
    var apply = document.getElementById(ids.apply); if (apply) apply.disabled = state !== 'approval_complete';
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '#' + CARD_ID + ' input{width:100%;box-sizing:border-box;padding:9px 10px;border:1px solid #94a3b8;border-radius:9px;background:#fff}',
  ...
  function cardHtml() {
    return '<section id="' + CARD_ID + '">' +
      ...
      '<div class="ulim-da-runbar"><input id="ulimDataAuthorityRunIdInput735414" placeholder="실행번호는 분석 후 자동 입력됩니다"><button type="button" class="admin-btn" onclick="ulimDataAuthorityLoadRun735414()">실행내역 불러오기</button></div>' +
  ...
  function install() {
    if (installing || !isSuperAdmin()) return;
    var panel = document.getElementById('adminPanelStudentManagement7352');
    var studentCard = document.getElementById('ulimStudentManagementCard7352');
    if (!panel || !studentCard) return;
    installing = true;
    injectStyles();
    var existing = document.getElementById(CARD_ID);
    if (!existing) studentCard.insertAdjacentHTML('afterbegin', cardHtml());
    rememberRunId(storedRunId());
  ...
  async function loadRun() {
    var input = document.getElementById('ulimDataAuthorityRunIdInput735414');
    var runId = text(input && input.value) || current.runId || storedRunId();
  ...
  async function applyApproved() {
    ...
    if (!confirm('승인된 항목만 Firestore에 적용할까요?\n각 항목의 기존 값은 복구용으로 먼저 보관됩니다.')) return;
    showLoading('승인항목을 적용하고 태블릿 자료를 갱신하는 중...');
    try {
      var result = await call('applyApprovedDataAuthorityPlanAdmin735414', { runId: current.runId, planDigest: current.run.planDigest });
      await loadRun();
      setStatus('4단계 적용 완료: 성공 ' + Number(result.applied || 0) + '건 · 실패/변경감지 ' + Number(result.failed || 0) + '건', Number(result.failed || 0) ? 'warn' : 'ok');
  ...
  async function backup() {
    var full = confirm('전체 Firestore 운영자료를 Google Sheets 백업 탭에 기록할까요?\n취소를 누르면 마지막 성공 이후 변경분만 백업합니다.');
    showLoading(full ? '전체 백업을 실행하는 중...' : '변경분 백업을 실행하는 중...');
    try {
      var result = await call('runFirestoreBackupAdmin735414', { full: full, requestId: requestId('data-authority-backup-735414') });
      var total = (Array.isArray(result.results) ? result.results : []).reduce(function (sum, item) { return sum + Number(item.sent || 0); }, 0);
      setStatus('6단계 Sheets 단방향 백업 완료: ' + total + '건 · ' + (full ? '전체' : '변경분') + ' 백업', 'ok');
    } catch (error) {
      setStatus((text(error && error.message) || '백업에 실패했습니다.') + '\nFirestore 운영자료는 이미 저장되어 있으며, 백업 실패가 앱 저장을 취소하지 않습니다.', 'error');
    } finally { hideLoading(); }
  }
  ...
  document.addEventListener('click', function (event) {
    ...
    if (panel === 'adminPanelStudentManagement7352') scheduleInstall(30);
  }, true);
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function () { scheduleInstall(100); }, { once: true });
  else scheduleInstall(100);
  if (typeof MutationObserver === 'function') {
    new MutationObserver(function () { if (!document.getElementById(CARD_ID)) scheduleInstall(60); else overrideLegacyButtons(); })
