(function (global) {
  'use strict';
  if (global.__ULIM_FIRESTORE_ONLY_ADMIN_735410__) return;
  global.__ULIM_FIRESTORE_ONLY_ADMIN_735410__ = true;
  global.ULIM_FIRESTORE_ONLY_ADMIN_VERSION = '2026-08-04.735.04.10';
  ...
  async function runtime() {
    ...
    if (typeof room.getStableIdToken === 'function') await room.getStableIdToken(rt, false, 'firestore-only-admin-735410');
  ...
  var dailyLoadSequence735410 = 0;
  var dailyLoadingKey735410 = '';
  var dailyLoadingPromise735410 = null;
  var dailyLastLoadedKey735410 = '';
  var dailyDirty735410 = false;

  function currentDailyRows735410() {
    try { return Array.isArray(adminDailyEvalRows) ? adminDailyEvalRows : []; } catch (_ignore) { return Array.isArray(global.adminDailyEvalRows) ? global.adminDailyEvalRows : []; }
  }

  function dailyLoadKey735410(context) {
    return [text(context && context.date), text(context && context.className)].join('|');
  }

  function markDailyDirty735410() {
    dailyDirty735410 = true;
    global.__ULIM_DAILY_EVAL_DIRTY_735410__ = true;
  }

  function clearDailyDirty735410() {
    dailyDirty735410 = false;
    global.__ULIM_DAILY_EVAL_DIRTY_735410__ = false;
  }
  ...
  async function saveAttendanceRecordsFirestore735410(records, silent) {
    ...
    try {
      ...
      var result = await call('saveStaffAttendanceOperational', {
        requestId: requestId('attendance-save-735410'),
        rows: rows
      });
  ...
  async function saveAttendanceFromTable735410(silent) {
    ...
    return saveAttendanceRecordsFirestore735410(rows, silent === true);
  }

  function dailyContext735410() {
    return {
  ...
  async function loadDailyEvaluationsFirestore735410(showAlert) {
    var explicitReload = showAlert === true;
    var context = dailyContext735410();
    if (!context.className) {
      assignDailyRows735410([]);
      renderDaily735410('반을 선택해주세요.');
      dailyLastLoadedKey735410 = '';
      return { status: 'empty-class' };
    }
    var key = dailyLoadKey735410(context);
    var currentRows = currentDailyRows735410();

    if (!explicitReload && key === dailyLastLoadedKey735410 && currentRows.length) {
      renderDaily735410('현재 화면의 일일평가를 유지했습니다.');
      return { status: 'cached', rows: currentRows };
    }
    if (!explicitReload && dailyDirty735410 && key === dailyLastLoadedKey735410) {
      renderDaily735410('작성 중인 일일평가를 유지했습니다.');
      return { status: 'dirty-preserved', rows: currentRows };
    }
    if (explicitReload && dailyDirty735410 && key === dailyLastLoadedKey735410) {
      if (!confirm('작성 중인 변경사항이 있습니다. 현재 반 학생목록을 다시 불러올까요?\n저장하지 않은 화면 내용은 바뀔 수 있습니다.')) return { status: 'cancelled' };
    }
    if (dailyLoadingPromise735410 && dailyLoadingKey735410 === key) return dailyLoadingPromise735410;

    var sequence = ++dailyLoadSequence735410;
    dailyLoadingKey735410 = key;
    if (key !== dailyLastLoadedKey735410) {
      assignDailyRows735410([]);
      renderDaily735410('선택한 반의 학생목록을 불러오는 중...');
    }

    dailyLoadingPromise735410 = (async function () {
      try {
        ...
        var data = await call('getStaffDailyEvaluationOperationalSnapshot', {
          ...
          keyword: '',
          requestId: requestId('daily-list-735410')
        });
        if (sequence !== dailyLoadSequence735410 || dailyLoadKey735410(dailyContext735410()) !== key) return { stale: true };
        var roster = Array.isArray(data.roster) ? data.roster : [];
        var saved = Array.isArray(data.rows) ? data.rows : [];
        var savedMap = new Map(saved.map(function (row) { return [dailyKey735410(row), row]; }));
        var merged = roster.map(function (row) {
          var prior = savedMap.get(dailyKey735410(row)) || {};
        ...
        saved.forEach(function (row) {
          if (!merged.some(function (item) { return dailyKey735410(item) === dailyKey735410(row); })) merged.push(row);
        });
        assignDailyRows735410(merged);
        dailyLastLoadedKey735410 = key;
        clearDailyDirty735410();
        renderDaily735410(text(data.message) || ('울림앱 학생 ' + merged.length + '명'));
        if (explicitReload && !merged.length) alert('조건에 맞는 학생이 없습니다. 날짜와 반을 확인해주세요.');
        return data;
      } catch (error) {
        if (sequence !== dailyLoadSequence735410 || dailyLoadKey735410(dailyContext735410()) !== key) return { stale: true };
        if (key !== dailyLastLoadedKey735410) assignDailyRows735410([]);
        renderDaily735410('일일평가 학생목록을 불러오지 못했습니다.');
        if (explicitReload) alert(text(error && error.message) || '일일평가 학생목록을 불러오지 못했습니다.');
        return { status: 'error', message: text(error && error.message) };
      } finally {
        if (sequence === dailyLoadSequence735410) {
          dailyLoadingPromise735410 = null;
          dailyLoadingKey735410 = '';
          hideLoading();
        }
      }
    })();
    return dailyLoadingPromise735410;
  }

  function selectedDailyRows735410() {
    try {
  ...
  async function saveDailyEvaluationsFirestore735410(sendRequested) {
    var rows = selectedDailyRows735410();
    if (!rows.length) return alert('저장할 일일평가 내용이 없습니다.');
    var recipients = recipientTypes735410();
    var channel = sendChannel735410();
    ...
    try {
      ...
      var result = await call('saveStaffDailyEvaluationsOperational', {
        requestId: requestId('daily-save-735410'),
        rows: compact,
      ...
      var savedMap = new Map(compact.map(function (row) { return [dailyKey735410(row), row]; }));
      var next = current.map(function (row) {
        var saved = savedMap.get(dailyKey735410(row));
        return saved ? Object.assign({}, row, saved, { savedAt: '방금 저장', savedBy: nowName }) : row;
      });
      assignDailyRows735410(next);
      dailyLastLoadedKey735410 = dailyLoadKey735410(dailyContext735410());
      clearDailyDirty735410();
      renderDaily735410('울림앱에 저장했습니다.');
  ...
  function disableSheetAutoSync735410() {
    global.__ULIM_DAILY_AUTOSAVE_SHEET_MODE_704__ = 'DISABLED_FIRESTORE_ONLY_735410';
  ...
  function bindDailyDirtyTracker735410() {
    if (global.__ULIM_DAILY_DIRTY_TRACKER_BOUND_735410__) return;
    global.__ULIM_DAILY_DIRTY_TRACKER_BOUND_735410__ = true;
    document.addEventListener('input', function (event) {
      if (event.target && event.target.closest && event.target.closest('#adminDailyEvalTableWrap')) markDailyDirty735410();
    }, true);
  ...
  function install735410() {
    global.adminSaveAttendanceRecords_ = saveAttendanceRecordsFirestore735410;
    global.adminSaveAttendanceFromTable = saveAttendanceFromTable735410;
    global.adminSaveAttendance = saveAttendanceFromTable735410;
    global.adminLoadDailyEvalStudents = loadDailyEvaluationsFirestore735410;
    global.adminSaveDailyEvaluations = saveDailyEvaluationsFirestore735410;
    try { adminSaveAttendanceRecords_ = saveAttendanceRecordsFirestore735410; } catch (_ignore1) {}
    try { adminSaveAttendanceFromTable = saveAttendanceFromTable735410; } catch (_ignore2) {}
    try { adminSaveAttendance = saveAttendanceFromTable735410; } catch (_ignore3) {}
    try { adminLoadDailyEvalStudents = loadDailyEvaluationsFirestore735410; } catch (_ignore4) {}
    try { adminSaveDailyEvaluations = saveDailyEvaluationsFirestore735410; } catch (_ignore5) {}
    disableSheetAutoSync735410();
    bindDailyDirtyTracker735410();
  }

  global.ULIM_FIRESTORE_ONLY_ADMIN_API_735410 = {
    install: install735410,
    loadDaily: loadDailyEvaluationsFirestore735410,
    saveDaily: saveDailyEvaluationsFirestore735410,
    saveAttendance: saveAttendanceFromTable735410
  };

  global.addEventListener('ulim-firebase-auth-ready', function () { setTimeout(install735410, 40); });
  global.addEventListener('pageshow', function () { setTimeout(install735410, 40); });
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', install735410, { once: true });
  else install735410();
  setTimeout(install735410, 180);
