(function (global) {
  'use strict';
  if (global.__ULIM_STAFF_ACCOUNT_ADMIN_7331__) return;
  global.__ULIM_STAFF_ACCOUNT_ADMIN_7331__ = true;

  const VERSION = '2026-07-31.733.01';
  let accounts = [];
  ...
  async function runtime() {
    ...
    if (room && typeof room.getStableIdToken === 'function') {
      await room.getStableIdToken(rt, false, 'staff-account-admin');
    } else {
  ...
  function status(message, state) {
    const el = document.getElementById('ulimStaffAccountStatus7331');
    if (!el) return;
  ...
  function injectStyles() {
    if (document.getElementById('ulimStaffAccountStyle7331')) return;
    const style = document.createElement('style');
    style.id = 'ulimStaffAccountStyle7331';
    style.textContent = `
      ...
      #ulimStaffAccountStatus7331{display:none;margin:10px 0;padding:10px 12px;border-radius:9px;font-size:13px}
      #ulimStaffAccountStatus7331[data-state="ok"]{display:block;background:#ecfdf5;color:#166534}
      #ulimStaffAccountStatus7331[data-state="error"]{display:block;background:#fff7ed;color:#9a3412}
      #ulimStaffAccountStatus7331[data-state="loading"]{display:block;background:#eff6ff;color:#1d4ed8}
  ...
  function injectPanel() {
    ...
    if (subtabs) {
      ...
      button.onclick = function () {
        ...
        else global.ulimStaffAccountLoad7331(false);
      };
    ...
    panel.innerHTML = `
      <div class="admin-card admin-full-only">
        ...
        <div id="ulimStaffAccountStatus7331"></div>
        <div class="ulim-staff-account-grid">
          <div class="admin-field"><label>로그인 ID</label><input id="ulimStaffNewLoginId7331" autocomplete="off" placeholder="예: kimcs"></div>
          <div class="admin-field"><label>이름</label><input id="ulimStaffNewName7331" placeholder="강사명"></div>
          <div class="admin-field"><label>전화번호</label><input id="ulimStaffNewPhone7331" placeholder="선택 입력"></div>
          <div class="admin-field"><label>역할</label><select id="ulimStaffNewRole7331"><option value="teacher">강사</option><option value="admin">관리자</option><option value="superAdmin">전체관리자</option></select></div>
          <div class="admin-field"><label>임시 비밀번호</label><input id="ulimStaffNewPassword7331" type="password" autocomplete="new-password" placeholder="6자 이상"></div>
          <div class="admin-field"><label>최초 로그인</label><select id="ulimStaffNewMustChange7331"><option value="true">비밀번호 변경 필요</option><option value="false">변경 없이 사용</option></select></div>
        </div>
        <div class="admin-btn-row" style="margin-top:10px;">
          <button type="button" class="admin-btn" onclick="ulimStaffAccountCreate7331()">교직원 추가</button>
          <button type="button" class="admin-btn blue" onclick="ulimStaffAccountLoad7331(true)">목록 새로고침</button>
        </div>
        <div id="ulimStaffAccountSummary7331" style="font-size:12px;color:#64748b;margin:10px 0;"></div>
        <div class="admin-table-wrap"><div id="ulimStaffAccountTable7331"></div></div>
      </div>`;
    dashboard.appendChild(panel);
  }
  function render() {
    const wrap = document.getElementById('ulimStaffAccountTable7331');
    const summary = document.getElementById('ulimStaffAccountSummary7331');
    if (!wrap) return;
    if (!accounts.length) {
    ...
    const rows = accounts.map(function (account) {
      ...
      return `<tr${disabledClass}>
        ...
        <td><div class="ulim-staff-account-actions">
          <button type="button" class="admin-btn blue" onclick="ulimStaffAccountSave7331('${key}')">저장</button>
          <button type="button" class="admin-btn orange" onclick="ulimStaffAccountResetPassword7331('${key}')">암호 초기화</button>
          <button type="button" class="admin-btn red" onclick="ulimStaffAccountRetire7331('${key}')">퇴사처리</button>
        </div></td>
      </tr>`;
    }).join('');
  ...
  async function create() {
    ...
    const loginId = text(document.getElementById('ulimStaffNewLoginId7331')?.value);
    const name = text(document.getElementById('ulimStaffNewName7331')?.value);
    const phone = text(document.getElementById('ulimStaffNewPhone7331')?.value);
    const role = text(document.getElementById('ulimStaffNewRole7331')?.value) || 'teacher';
    const password = String(document.getElementById('ulimStaffNewPassword7331')?.value || '');
    const mustChangePassword = text(document.getElementById('ulimStaffNewMustChange7331')?.value) !== 'false';
    ...
    try {
      ...
      ['ulimStaffNewLoginId7331','ulimStaffNewName7331','ulimStaffNewPhone7331','ulimStaffNewPassword7331'].forEach(function (id) { const el = document.getElementById(id); if (el) el.value = ''; });
      await load(true);
  ...
  function install() {
    ...
    injectPanel();
    installPanelHook();
    global.ulimStaffAccountLoad7331 = load;
    global.ulimStaffAccountCreate7331 = create;
    global.ulimStaffAccountSave7331 = save;
    global.ulimStaffAccountResetPassword7331 = resetPassword;
    global.ulimStaffAccountRetire7331 = retire;
    ...
    try {
      if (!global.__ULIM_CONSOLE_CLEANED_7331__) {
        global.__ULIM_CONSOLE_CLEANED_7331__ = true;
        console.clear();
      }
      console.info('[ULIM 7.33.1 CLEAN START] stable Firebase token guard + staff account management', VERSION);
    } catch (_ignore) {}
  }
