(function (global) {
  'use strict';
  if (global.__ULIM_STAFF_ACCOUNT_ADMIN_73435__) return;
  global.__ULIM_STAFF_ACCOUNT_ADMIN_73435__ = true;

  const VERSION = '2026-08-01.734.03.5';
  let accounts = [];
  ...
  async function refreshFirestoreOnly(options) {
    ...
    const run = (async function () {
      ...
      try { console.info('[ULIM 7.34.3.5 staff list] Firestore refresh start'); } catch (_ignore) {}
      ...
      try { console.info('[ULIM 7.34.3.5 staff list] Firestore refresh complete', { count: accounts.length }); } catch (_ignore) {}
      return listResult;
    })().catch(function (error) {
      if (opts.keepStatus !== true) status(text(error && error.message) || '교직원 계정 목록 조회에 실패했습니다.', 'error');
      try { console.error('[ULIM 7.34.3.5 staff list] failed', error); } catch (_ignore) {}
      throw error;
    }).finally(function () {
  ...
  function install() {
    ...
    try {
      ...
      console.info('[ULIM 7.34.3.5 CLEAN START] incomplete staff repair + guarded purge');
    } catch (_ignore) {}
  }
