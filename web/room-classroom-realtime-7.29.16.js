(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72916__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72916__ = true;

  const VERSION = "2026-08-01.729.16";
  ...
  function resetStableTokenGuard72916(uid) {
    tokenGuard72916.uid = String(uid || "");
    tokenGuard72916.promise = null;
    tokenGuard72916.lastError = null;
    tokenGuard72916.lastErrorAt = 0;
    tokenGuard72916.lastForceAt = 0;
    tokenGuard72916.invalidEventUid = "";
  }

  function terminalTokenError72916(error) {
  ...
  async function getStableIdToken72916(runtime, forceRefresh, reason) {
    ...
    if (tokenGuard72916.uid !== uid) resetStableTokenGuard72916(uid);

    const now = Date.now();
    const cooldownMs = terminalTokenError72916(tokenGuard72916.lastError) ? 60000 : 12000;
    if (tokenGuard72916.lastError && now - tokenGuard72916.lastErrorAt < cooldownMs) {
      throw tokenGuard72916.lastError;
    }
    if (tokenGuard72916.promise) return tokenGuard72916.promise;

    let force = forceRefresh === true;
    if (force && now - tokenGuard72916.lastForceAt < 15000) force = false;
    if (force) tokenGuard72916.lastForceAt = now;

    tokenGuard72916.promise = Promise.resolve()
      ...
      .catch(function (error) {
        tokenGuard72916.lastError = error;
        tokenGuard72916.lastErrorAt = Date.now();
        if (terminalTokenError72916(error) && tokenGuard72916.invalidEventUid !== uid) {
          tokenGuard72916.invalidEventUid = uid;
          try {
    ...
    return tokenGuard72916.promise;
  }

  async function getStableIdTokenResult72916(runtime, forceRefresh, reason) {
    await getStableIdToken72916(runtime, forceRefresh === true, reason || "token-result");
  ...
  async function signInLegacySessionFresh(reason) {
    ...
    await getStableIdToken72916(runtime, false, "custom-token-signin");
    ...
    safeConsole("info", "[ULIM realtime auth refreshed]", reason || "manual");
  ...
  async function acceptExistingFirebaseSession72912(runtime, reason, forceRefresh) {
    ...
    const user = runtime.auth.currentUser;
    const tokenResult = await getStableIdTokenResult72916(runtime, forceRefresh === true, reason || "accept-existing");
    ...
    resetStableTokenGuard72916(user.uid);
    state.ready = true;
  ...
  async function ensureAuthenticated() {
    ...
    state.signingIn = (async function () {
      ...
      if (runtime.auth.currentUser && storedFingerprint === fingerprint) {
        try {
          await getStableIdToken72916(runtime, false, "legacy-existing");
          state.ready = true;
          state.sessionFingerprint = fingerprint;
          setStatus("ready", "실시간 연결됨");
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (lastRuntime && lastRuntime.auth && lastRuntime.auth.currentUser) {
          await getStableIdToken72916(lastRuntime, false, "wait-until-authenticated");
          return lastRuntime;
        }
      } catch (_ignore) {}
  ...
  const publicApi728 = Object.freeze({
    ...
    getStableIdToken: getStableIdToken72916,
    resetStableTokenGuard: resetStableTokenGuard72916,
  ...
  safeConsole("info", "[ULIM 7.29.16 Firebase-primary-preserving sheet-canonical realtime module]", VERSION);
