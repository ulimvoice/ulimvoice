(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73114__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73114__ = true;

  const VERSION = '2026-08-01.731.14';
  const CACHE_PREFIX = 'ulim_staff_fsop_73114_';
  const inflight = new Map();
  let runtimePromise = null;
  let revisionEnsurePromise73112 = null;
  const revisionListeners7318 = {
    attendance: { date: '', unsubscribe: null, initialized: false, lastRevision: 0, reloadTimer: null, blockedUntil: 0 },
    daily: { date: '', unsubscribe: null, initialized: false, lastRevision: 0, reloadTimer: null, blockedUntil: 0 }
  };
  ...
  function attendanceMinimal(row) {
    ...
    return {
      ...
      currentStatus: r.currentStatus || r.remarkText || r.sheetRemark || '',
      remarkText: r.remarkText || r.currentStatus || r.sheetRemark || '',
      sheetRemark: r.sheetRemark || r.currentStatus || r.remarkText || '',
      currentStatusDirty: r.currentStatusDirty === true || r.remarkDirty === true || r.sheetRemarkDirty === true || r.__currentStatusDirty73439 === true,
      remarkDirty: r.currentStatusDirty === true || r.remarkDirty === true || r.sheetRemarkDirty === true || r.__currentStatusDirty73439 === true,
      sheetRemarkDirty: r.currentStatusDirty === true || r.remarkDirty === true || r.sheetRemarkDirty === true || r.__currentStatusDirty73439 === true,
      memo: r.memo || '',
      ...
      if (kind === 'attendance') {
        ...
        if (select && text(select.value) && typeof global.adminLoadAttendanceSnapshot === 'function') {
          global.adminLoadAttendanceSnapshot(false, false).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.12 attendance realtime reload]', error);
          });
        }
      } else {
        ...
        if (select && text(select.value) && typeof global.adminLoadDailyEvalStudents === 'function') {
          global.adminLoadDailyEvalStudents(false).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.12 daily realtime reload]', error);
          });
        }
      }
    }, 180);
  ...
  async function bindRevisionListener7318(kind, date) {
    ...
    try {
      ...
      state.unsubscribe = rt.sdk.onSnapshot(ref, function (snapshot) {
        ...
        state.lastRevision = revision;
        safeConsole('info', '[ULIM 7.31.12 ' + kind + ' revision]', targetDate, revision);
        ...
        if (code.indexOf('permission-denied') >= 0) {
          ...
          safeConsole('info', '[ULIM 7.31.12 ' + kind + ' revision waits for Firebase claims]');
          return;
        }
        safeConsole('warn', '[ULIM 7.31.12 ' + kind + ' revision listener]', error);
      });
    } catch (error) {
      state.blockedUntil = Date.now() + 1500;
      safeConsole('warn', '[ULIM 7.31.12 revision bind]', kind, error);
    }
  }
