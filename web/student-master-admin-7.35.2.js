(function (global) {
  'use strict';
  if (global.__ULIM_STUDENT_MANAGEMENT_V2_7352__) return;
  global.__ULIM_STUDENT_MANAGEMENT_V2_7352__ = true;

  const VERSION = '2026-08-02.735.02.0';
  ...
  function cardHtml() {
    return `<div id="${CARD_ID}" class="admin-card admin-full-only">
      ...
      <div class="ulim-student-help">학생 기본정보와 수강반을 함께 관리합니다. 출결번호와 최초 비밀번호는 학생 전화번호의 마지막 네 자리로 자동 생성됩니다. 학생정보와 앱 출석부·태블릿 운영자료는 즉시 저장됩니다. Google Sheets 명단·로그인 기록은 백그라운드에서 후속 처리됩니다.</div>
  ...
  async function createStudent() {
    ...
    try {
      ...
      if (form) Array.from(form.querySelectorAll('input')).forEach(function (el) { if (el.type !== 'date' || el.id !== 'ulimNewStudentStart7352') el.value = ''; });
      ...
      document.getElementById('ulimNewStudentStart7352').value = today();
      updateCreateClassPreview();
  ...
  async function saveKeys(keys) {
    ...
    try {
      ...
      else setStatus('학생정보 ' + edits.length + '명의 변경사항을 앱 운영자료에 저장했습니다. 시트 기록은 백그라운드에서 진행됩니다.', 'ok');
    } catch (error) {
