(function (global) {
  ...
  if (global.__ULIM_ROOM_CLASSROOM_REALTIME_72912__) return;
  global.__ULIM_ROOM_CLASSROOM_REALTIME_72912__ = true;

  const VERSION = "2026-07-31.729.12";
  ...
  async function forceReauthenticate(reason) {
    ...
    if (runtime.auth.currentUser) {
      try {
        ...
        safeConsole("warn", "[ULIM 7.29.12 primary token refresh failed]", primaryError);
      }
    }
  ...
  async function ensureAuthenticated() {
    ...
    state.signingIn = (async function () {
      /*
       * 7.29.12 Firebase-primary login:
      ...
      if (runtime.auth.currentUser) {
        try {
          ...
          safeConsole("warn", "[ULIM 7.29.12 primary session rejected]", primaryError);
      ...
      if (!session) {
        if (state.runtime && state.runtime.auth.currentUser) {
          try {
            ...
            safeConsole("warn", "[ULIM 7.29.12 auth poll primary]", primaryError);
            return;
          }
        }
        state.ready = false;
  ...
  safeConsole("info", "[ULIM 7.29.12 Firebase-primary-preserving sheet-canonical realtime module]", VERSION);
