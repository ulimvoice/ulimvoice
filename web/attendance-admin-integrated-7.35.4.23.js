(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735423__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-09.735.04.23-dual-month-realtime-ledger';

  var VERSION = '2026-08-09.735.04.23-dual-month-realtime-ledger';
  var loadSequence = 0;
  ...
  function ensureAllClassesModal735410() {
    ...
    modal.addEventListener('click', function (event) { if (event.target && event.target.closest('[data-close-all-classes="1"]')) modal.style.display = 'none'; });
    document.body.appendChild(modal);
    document.getElementById('ulimAllClassesReload735423').addEventListener('click', function () { loadAllClassesData735410(true); });
    var removeButton=document.getElementById('ulimAllClassesRemoveSelected735423'); if(removeButton){removeButton.style.display=isFullAdmin()?'inline-flex':'none';removeButton.addEventListener('click',removeSelectedAllClassStudents735413);}
    return modal;
  }
  ...
  function renderTeacherTabs735423() {
    ...
    wrap.innerHTML = items.map(function (item) { var active = normalize(allClassesState735410.teacher) === normalize(item.value); return '<button type="button" data-ledger-teacher="' + escapeHtml(item.value) + '" style="flex:0 0 auto;border:1px solid ' + (active ? '#2563eb' : '#cbd5e1') + ';background:' + (active ? '#2563eb' : '#fff') + ';color:' + (active ? '#fff' : '#334155') + ';border-radius:999px;padding:7px 12px;font-size:12px;font-weight:900;cursor:pointer">' + escapeHtml(item.label) + '</button>'; }).join('');
    wrap.querySelectorAll('[data-ledger-teacher]').forEach(function (button) { button.onclick = function () { allClassesState735410.teacher = text(button.getAttribute('data-ledger-teacher')) || 'all'; renderAllClassesBoardLocal735410(); }; });
  }
  ...
  async function saveLedgerCell735423(group, student, session, patch) {
    ...
    var payload = { date: session.date, classId: group.classId, className: group.className, studentUid: student.studentUid, studentName: student.studentName, name: student.studentName, attendanceNo: student.attendanceNo, studentNo: student.attendanceNo, status: text(patch.status || cell.status || '미체크'), attendanceStatus: text(patch.status || cell.attendanceStatus || '미체크'), specialStatus: text(patch.specialStatus != null ? patch.specialStatus : cell.specialStatus), currentStatus: text(patch.currentStatus != null ? patch.currentStatus : cell.currentStatus), memo: text(patch.memo != null ? patch.memo : cell.memo) };
    allClassesState735410.actionInFlight = true;
    try { await call('saveAttendanceRowsAdmin73550', { rows: [payload], requestId: requestId('ledger-attendance-save-735423') }); Object.assign(cell, payload, {eligible:true}); student.cells[session.date] = cell; renderAllClassesBoardLocal735410(); }
  ...
  function bindLedgerEvents735423() {
    var board=document.getElementById('ulimAllClassesBoard735410'); if(!board)return;
    board.querySelectorAll('[data-ledger-header]').forEach(function(th){th.onclick=function(){var group=groupById735423(th.dataset.classId);var session=group&&(group.sessions||[]).find(function(s){return text(s.date)===text(th.dataset.date);});if(group&&session)scheduleOperationPrompt735423(group,session);};});
    board.querySelectorAll('[data-ledger-cell]').forEach(function(td){
      var group=groupById735423(td.dataset.classId);var student=group&&studentByLedger735423(group,td.dataset.studentUid);var session=group&&(group.sessions||[]).find(function(s){return text(s.date)===text(td.dataset.date);});if(!group||!student||!session)return;
      td.querySelectorAll('[data-ledger-status]').forEach(function(btn){btn.onclick=function(e){e.stopPropagation();var desired=text(btn.dataset.ledgerStatus);var cell=student.cells&&student.cells[session.date]||{};saveLedgerCell735423(group,student,session,{status:cleanAttendanceStatus7355014(cell.status)===desired?'미체크':desired});};});
      var detail=td.querySelector('[data-ledger-detail]');if(detail)detail.onclick=function(e){e.stopPropagation();openLedgerDetail735423(group,student,session);};
      var add=td.querySelector('[data-ledger-add]');if(add&&isFullAdmin())add.onclick=function(e){e.stopPropagation();openAttendanceAddModal735410({date:session.date,classId:group.classId,className:group.className});};
      td.addEventListener('dragover',function(e){if(isFullAdmin()){e.preventDefault();td.classList.add('drag-over');}});td.addEventListener('dragleave',function(){td.classList.remove('drag-over');});td.addEventListener('drop',function(e){e.preventDefault();td.classList.remove('drag-over');if(allClassesDragData735410)applyAllClassesDrop735410(allClassesDragData735410,group.classId,session.date);});
    });
    board.querySelectorAll('tr[data-ledger-student]').forEach(function(tr){var select=tr.querySelector('[data-ledger-select]');if(select)select.onchange=function(e){e.stopPropagation();var key=text(tr.dataset.sourceClass)+'|'+text(tr.dataset.ledgerStudent);if(select.checked)allClassesState735410.selectedCards.set(key,{classId:text(tr.dataset.sourceClass),studentUid:text(tr.dataset.ledgerStudent)});else allClassesState735410.selectedCards.delete(key);var removeButton=document.getElementById('ulimAllClassesRemoveSelected735423');if(removeButton)removeButton.textContent=allClassesState735410.selectedCards.size?'선택 학생 제거 ('+allClassesState735410.selectedCards.size+')':'선택 학생 제거';};if(isFullAdmin()){tr.addEventListener('dragstart',function(){allClassesDragData735410={studentUid:text(tr.dataset.ledgerStudent),sourceClassId:text(tr.dataset.sourceClass)};});tr.addEventListener('dragend',function(){allClassesDragData735410=null;});}var gear=tr.querySelector('[data-ledger-gear]');if(gear)gear.onclick=function(e){e.stopPropagation();openWholeClassStudentDetail7355016(tr.dataset.ledgerStudent);};});
    board.querySelectorAll('[data-ledger-add-class]').forEach(function(btn){btn.onclick=function(){var group=groupById735423(btn.dataset.classId);var session=group&&(group.sessions||[]).filter(function(s){return text(s.date).slice(0,7)===text(allClassesState735410.ledger.currentMonth)&&s.state!=='cancelled'&&s.state!=='moved';})[0];if(group&&session)openAttendanceAddModal735410({date:session.date,classId:group.classId,className:group.className});};});
  }
  async function openWholeClassStudentDetail7355016(studentUid) {
    if(!isFullAdmin())return alert('전체관리자 권한이 필요합니다.');
    try { var directory=allClassesState735410.directory||await loadDirectory(false); var student=(directory.students||[]).find(function(s){return text(s.studentUid)===text(studentUid);}); if(!student)return alert('학생정보를 찾지 못했습니다.'); detailRecordIndex=-1;detailCandidates=[student];detailStudent=student;ensureStyles();ensureLedgerStyles735423();var modal=ensureDetailModal();modal.classList.add('open');renderDetailStudent(student,directory,'전체반 학생정보입니다. 수정하면 학생명단과 출석부에 즉시 반영됩니다.'); } catch(error){alert(text(error&&error.message)||'학생정보를 열지 못했습니다.');}
  }
  ...
  async function loadAllClassesData735410(force) {
    if(allClassesState735410.loadingPromise){allClassesState735410.reloadPending=true;return allClassesState735410.loadingPromise;}
    var board=document.getElementById('ulimAllClassesBoard735410');if(board&&!allClassesState735410.ledger)board.innerHTML='<div style="padding:30px;text-align:center;color:#64748b">전월·현재월 출석부를 불러오는 중...</div>';setAllClassesStatus735410(force?'최신 출석부를 갱신하는 중...':'출석부를 불러오는 중...');
    allClassesState735410.loadingPromise=(async function(){try{var values=await Promise.all([loadDirectory(force===true),call('getAttendanceRosterAdmin73550',{ledger:true,view:'ledger',anchorDate:today(),requestId:requestId('attendance-ledger-735423')})]);if(text(values[1]&&values[1].source)!=='firestore_attendance_ledger_7355033')throw new Error('최신 출석부 서버가 아직 반영되지 않았습니다.');allClassesState735410.directory=values[0];allClassesState735410.ledger=values[1];if(!isFullAdmin()&&values[1].teachers&&values[1].teachers.length)allClassesState735410.teacher=values[1].teachers[0];renderAllClassesBoardLocal735410();return values[1];}catch(error){setAllClassesStatus735410(text(error&&error.message)||'전체 출석부를 불러오지 못했습니다.',true);throw error;}finally{allClassesState735410.loadingPromise=null;if(allClassesState735410.reloadPending){allClassesState735410.reloadPending=false;setTimeout(function(){loadAllClassesData735410(false).catch(function(){});},0);}}})();return allClassesState735410.loadingPromise;
  }
  function openAllClassesModal735410() {
    if(!fullAdminOrTeacher7355033())return alert('교직원 로그인이 필요합니다.');var modal=ensureAllClassesModal735410();modal.style.display='block';loadAllClassesData735410(!allClassesState735410.ledger).catch(function(){});
  }
  function scheduleRealtimeAttendanceRefresh735423(detail) {
    ...
    clearTimeout(allClassesState735410.reloadTimer);allClassesState735410.reloadTimer=setTimeout(function(){
      var modal=document.getElementById('ulimAllClassesAttendanceModal735410');if(modal&&modal.style.display==='block'){loadAllClassesData735410(false).catch(function(){});return;}
  ...
  function ensureLedgerStyles735423() {
    if (document.getElementById('ulim-attendance-ledger-style-735423')) return;
    var style=document.createElement('style');style.id='ulim-attendance-ledger-style-735423';style.textContent=''
      +'.ulim-ledger-class735423{margin:0 0 20px;background:#fff;border:1px solid #cbd5e1;border-radius:14px;overflow:hidden;box-shadow:0 2px 8px rgba(15,23,42,.05)}'
      +'.ulim-ledger-class-title735423{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:11px 13px;background:#e0f2fe;border-bottom:1px solid #bae6fd;color:#075985}.ulim-ledger-class-title735423 div{display:flex;gap:10px;align-items:baseline;flex-wrap:wrap}.ulim-ledger-class-title735423 b{font-size:14px}.ulim-ledger-class-title735423 span{font-size:11px;font-weight:800}'
      +'.ulim-ledger-scroll735423{overflow:auto;max-width:100%}.ulim-ledger-table735423{border-collapse:separate;border-spacing:0;min-width:1100px;width:100%;font-size:12px}.ulim-ledger-table735423 th,.ulim-ledger-table735423 td{border-right:1px solid #dbe3ec;border-bottom:1px solid #dbe3ec;padding:6px;text-align:center;background:#fff;min-width:76px;height:42px}.ulim-ledger-table735423 thead th{position:sticky;top:0;z-index:3;background:#f8fafc;font-weight:900}'
      +'.ulim-ledger-month735423{font-size:13px!important;height:30px!important}.ulim-ledger-month735423.prev{background:#f1f5f9!important;color:#475569}.ulim-ledger-month735423.current{background:#dbeafe!important;color:#1e3a8a;border-left:5px solid #0f172a!important}'
      +'.ulim-ledger-current-start735423{border-left:5px solid #0f172a!important}.ulim-ledger-date735423{cursor:pointer;user-select:none}.ulim-ledger-date735423:hover{background:#e0f2fe!important}.ulim-ledger-date735423 b{display:block;font-size:12px}.ulim-ledger-date735423 small{display:block;margin-top:2px;color:#64748b}.ulim-ledger-date735423 span{display:block;margin-top:3px;color:#b45309;font-size:10px}'
      +'.ulim-ledger-sticky-no735423{position:sticky!important;left:0!important;z-index:5!important;min-width:42px!important;width:42px!important;background:#f8fafc!important}.ulim-ledger-sticky-name735423{position:sticky!important;left:55px!important;z-index:5!important;min-width:160px!important;text-align:left!important;background:#fff!important;white-space:nowrap}.ulim-ledger-sticky-name735423 button{border:0;background:transparent;cursor:pointer}.ulim-ledger-drag735423{cursor:grab;color:#94a3b8;margin-right:5px}.ulim-ledger-note735423{min-width:130px!important;max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'
      +'.ulim-ledger-cell-actions735423{display:flex;align-items:center;justify-content:center;gap:3px}.ulim-ledger-ox735423,.ulim-ledger-more735423{border:1px solid #cbd5e1;border-radius:7px;background:#fff;padding:4px 7px;font-weight:900;cursor:pointer}.ulim-ledger-ox735423.on-o{background:#16a34a;color:#fff;border-color:#16a34a}.ulim-ledger-ox735423.on-x{background:#dc2626;color:#fff;border-color:#dc2626}.ulim-ledger-more735423{color:#475569}.ulim-ledger-special735423{margin-top:3px;font-size:9px;font-weight:900;color:#92400e;background:#fef3c7;border-radius:5px;padding:2px 3px}.ulim-ledger-session-off735423{font-weight:900;color:#b91c1c;background:#fee2e2;border-radius:7px;padding:5px}.ulim-ledger-empty735423{border:1px dashed #cbd5e1;background:#f8fafc;color:#64748b;border-radius:7px;width:30px;height:28px;cursor:pointer}.ulim-ledger-table735423 td.drag-over{outline:3px solid #22c55e;outline-offset:-3px;background:#f0fdf4!important}.ulim-ledger-add-row735423 button{border:0;background:transparent;color:#2563eb;font-weight:900;cursor:pointer}'
      +'@media(max-width:900px){.ulim-ledger-sticky-name735423{left:48px!important;min-width:130px!important}.ulim-ledger-table735423{font-size:11px}}';document.head.appendChild(style);
  }

  function install() {
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355016__ = 'attendance-admin-integrated-7.35.4.23';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355015__ = 'attendance-admin-integrated-7.35.4.23';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355014__ = 'attendance-admin-integrated-7.35.4.23';
    global.__ULIM_ATTENDANCE_NO_AUTO_REFRESH_7355016__ = false;
    global.__ULIM_ATTENDANCE_NO_AUTO_REFRESH_7355015__ = false;
    global.__ULIM_ATTENDANCE_NO_AUTO_REFRESH_7355014__ = false;
