(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73110__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73110__ = true;

  const VERSION = '2026-07-31.731.10';
  const CACHE_PREFIX = 'ulim_staff_fsop_73110_';
      ...
      if (kind === 'attendance') {
        ...
        if (select && text(select.value) && typeof global.adminLoadAttendanceSnapshot === 'function') {
          global.adminLoadAttendanceSnapshot(false, false).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.10 attendance realtime reload]', error);
          });
        }
      } else {
        ...
        if (select && text(select.value) && typeof global.adminLoadDailyEvalStudents === 'function') {
          global.adminLoadDailyEvalStudents(false).catch(function (error) {
            safeConsole('warn', '[ULIM 7.31.10 daily realtime reload]', error);
          });
        }
      }
    }, 180);
  ...
  async function bindRevisionListener7318(kind, date) {
    ...
    try {
      ...
      state.unsubscribe = rt.sdk.onSnapshot(ref, function (snapshot) {
        ...
        state.lastRevision = revision;
        safeConsole('info', '[ULIM 7.31.10 ' + kind + ' revision]', targetDate, revision);
        scheduleRevisionReload7318(kind, targetDate, revision);
      }, function (error) {
        state.unsubscribe = null;
        safeConsole('warn', '[ULIM 7.31.10 ' + kind + ' revision listener]', error);
      });
    } catch (error) {
      safeConsole('warn', '[ULIM 7.31.10 revision bind]', kind, error);
    }
  }
    ...
    try {
      ...
      if (room && typeof room.ensureAuthenticated === 'function') {
        room.ensureAuthenticated().catch(function (error) {
          safeConsole('warn', '[ULIM 7.31.10 auth prewarm]', error);
        });
      }
    } catch (ignore) {}
  }
