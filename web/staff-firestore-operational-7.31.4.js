(function (global) {
  ...
  if (global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73104__) return;
  global.__ULIM_STAFF_FIRESTORE_OPERATIONAL_73104__ = true;

  const VERSION = '2026-07-31.731.04';
  ...
  async function runtime() {
    ...
    runtimePromise = (async function () {
      ...
      let rt = await room.ensureAuthenticated();
      ...
      if (!rt || !rt.sdk || !rt.functions || !rt.auth || !rt.auth.currentUser) {
        throw new Error('Firebase 교직원 인증을 완료하지 못했습니다.');
      }
  ...
  async function invokeStaff(rt, fn, payload, label) {
    try {
      ...
      safeConsole('warn', '[ULIM 7.31.4 staff auth retry]', label || '', error);
      const fresh = await room.forceReauthenticate('staff-call-' + (label || 'unknown'));
