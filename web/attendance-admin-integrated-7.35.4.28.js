(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735428__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-10.735.04.28-equal-month-width-persistent-note';

  var VERSION = '2026-08-10.735.04.28-equal-month-width-persistent-note';
  var loadSequence = 0;
  ...
  function ledgerMonthBlockHtml735427(group, monthKey, sessions, kind) {
    ...
    header += '<th class="ulim-ledger-note735423">비고</th>';
    header += '</tr>';
    ...
    var rows = students.map(function (student, rowIndex) {
      ...
      row += '<td class="ulim-ledger-note735423" title="' + escapeHtml(lastMemo) + '">' + escapeHtml(lastMemo) + '</td>';
      return row + '</tr>';
    }).join('');

    if (isFullAdmin()) {
      var addDate = safeSessions.filter(function (session) { return session.state !== 'cancelled' && session.state !== 'moved'; })[0];
      rows += '<tr class="ulim-ledger-add-row735423"><td></td><td><button type="button" data-ledger-add-class="1" data-class-id="' + escapeHtml(group.classId) + '" data-date="' + escapeHtml(addDate && addDate.date || '') + '">＋ 학생 추가</button></td><td colspan="' + (Math.max(1, safeSessions.length) + 1) + '"></td></tr>';
    }
  ...
  function install() {
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355016__ = 'attendance-admin-integrated-7.35.4.28';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355015__ = 'attendance-admin-integrated-7.35.4.28';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355014__ = 'attendance-admin-integrated-7.35.4.28';
