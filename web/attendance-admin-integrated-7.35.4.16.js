(function (global) {
  ...
  if (global.__ULIM_ATTENDANCE_ADMIN_INTEGRATED_735416__) return;
  ...
  global.ULIM_ATTENDANCE_ADMIN_INTEGRATED_VERSION = '2026-08-08.735.04.16-no-mixed-roster-strict-date';

  var VERSION = '2026-08-08.735.04.16-no-mixed-roster-strict-date';
  var loadSequence = 0;
  ...
  async function loadClassListFirebaseFirst(dateOverride, force) {
    ...
    try {
      var data = await call('getStaffClassListOperationalSnapshot', {
        date: date,
        force: force === true,
        requestId: requestId('attendance-class-list-7355015')
      });
      if (sequence !== classListSequence) return { stale: true };
      if (dateEl && text(dateEl.value) !== date) return { stale: true };
      if (text(data && data.source) !== 'firestore_canonical_class_catalog_7355015') {
  ...
  function renderAttendanceOwned7355014() {
    ...
    var html = '<table class="admin-table ulim-attendance-owned-7355014"><thead><tr>'
    ...
    html += '</tbody></table><div class="ulim-attendance-actions-7355014" style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:12px">'
    ...
    wrap.querySelectorAll('tr[data-att-index]').forEach(function (row) {
      ...
      row.querySelectorAll('[data-att-quick]').forEach(function (button) {
        button.addEventListener('click', async function () {
          var value = text(button.getAttribute('data-att-quick'));
          try {
            if (typeof global.adminSetAttendanceRowStatus === 'function') await global.adminSetAttendanceRowStatus(index, value);
            else if (typeof adminSetAttendanceRowStatus === 'function') await adminSetAttendanceRowStatus(index, value);
          } catch (_ignore) {}
        });
      });
      ...
      if (statusSelect) statusSelect.addEventListener('change', async function () {
        try {
          if (typeof global.adminSyncAttendanceRowStatus === 'function') await global.adminSyncAttendanceRowStatus(index, statusSelect.value);
          else if (typeof adminSyncAttendanceRowStatus === 'function') await adminSyncAttendanceRowStatus(index, statusSelect.value);
        } catch (_ignore) {}
      });
    ...
    if (saveButton) saveButton.addEventListener('click', async function () {
      try {
        var fn = global.adminSaveAttendanceFromTable;
        if (typeof fn !== 'function' && typeof adminSaveAttendanceFromTable === 'function') fn = adminSaveAttendanceFromTable;
        if (typeof fn !== 'function') return alert('출석 저장 기능을 찾지 못했습니다.');
        var result = await fn(false);
        if (result !== false) attendanceDraftDirty7355014 = false;
  ...
  async function loadFromFirebase(context) {
    return call('getAttendanceRosterAdmin73550', {
      ...
      requestId: requestId('attendance-snapshot-7355015')
    });
  }
  ...
  async function safeLoadAttendanceSnapshot(showAlert) {
    ...
    try {
      ...
      if (text(data && data.source) !== 'firestore_canonical_roster_7355015') {
    ...
    directoryLoadingPromise735414 = (async function () {
      try {
        ...
        directoryCache = {
          ...
          teachers: (Array.isArray(result.teachers) ? result.teachers : []).map(function (teacher) {
    ...
    try {
      ...
      document.getElementById('ulimScheduleSave7355014').onclick = async function () {
        ...
        try {
          ...
          if (hidden) hidden.value = classItem.className;
          await safeLoadAttendanceSnapshot(false);
    ...
    allClassesState735410.loadingPromise = (async function () {
      try {
        ...
        var tempPromise = (!dateValue || (!force && allClassesState735410.temporaryDate === dateValue))
          ...
          : call('getAttendanceRosterAdmin73550', { date: dateValue, className: '전체반', keyword: '', statusFilter: '', requestId: requestId('all-classes-attendance-7355015') });
        ...
        if (dateValue && text(values[1] && values[1].source) !== 'firestore_canonical_roster_7355015') {
        ...
        allClassesState735410.temporaryRecords = (Array.isArray(values[1] && values[1].records) ? values[1].records : [])
          .filter(isWholeClassTemporaryRecord7355015);
  ...
  function install() {
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355015__ = 'attendance-admin-integrated-7.35.4.16';
    global.__ULIM_ATTENDANCE_SINGLE_OWNER_7355014__ = 'attendance-admin-integrated-7.35.4.16';
