(function (global) {
  'use strict';
  if (global.__ULIM_DATA_AUTHORITY_7354150__) return;
  global.__ULIM_DATA_AUTHORITY_7354150__ = true;
  ...
  global.ULIM_DATA_AUTHORITY_VERSION = '2026-08-05.735.04.15.0-bounded-backup-continuation';
  ...
  async function backup() {
    ...
    showLoading(full ? '전체 백업을 중단 지점부터 자동으로 이어서 처리하는 중...' : '변경분 백업을 실행하는 중...');
    try {
      var clientRequestId = requestId('data-authority-backup-7354150');
      var continuationCount = 0;
      var result = null;
      while (continuationCount < 500) {
        result = await call('runFirestoreBackupAdmin735414', {
          full: full,
          requestId: clientRequestId
        }, 1500000);
        if (!result || result.continuationRequired !== true) break;
        continuationCount += 1;
        var progressSent = Number(result.totalSent || 0);
        var currentSent = Number(result.currentSent || 0);
        var currentFound = Number(result.currentFound || 0);
        setStatus(
          '6단계 백업 자동 이어서 처리 중' +
          '\n백업ID: ' + text(result.backupId || '') +
          '\n현재 컬렉션: ' + text(result.currentCollection || 'attendance') +
          '\n누적 전송 ' + progressSent + '건' +
          (currentFound ? ' · 현재 컬렉션 ' + currentSent + '/' + currentFound + '건' : '') +
          '\n서버 제한시간 전에 안전하게 구간 저장을 완료했으며 다음 구간을 자동 실행합니다.',
          'warn'
        );
        showLoading('백업 구간 ' + (continuationCount + 1) + '을 자동으로 이어서 처리하는 중...');
        await new Promise(function (resolve) { setTimeout(resolve, 800); });
      }
      if (!result) throw new Error('백업 서버 응답이 없습니다.');
      if (result.continuationRequired === true) {
        setStatus(
          '6단계 백업이 안전한 구간 단위로 저장되었습니다.' +
          '\n백업ID: ' + text(result.backupId || '') +
          '\n누적 전송 ' + Number(result.totalSent || 0) + '건' +
          '\n자동 연속 실행 한도에 도달했습니다. 같은 버튼을 다시 누르면 저장된 지점부터 이어집니다.',
          'warn'
        );
        return;
      }
      ...
      var failures = Array.isArray(result.failedCollections) ? result.failedCollections : results.filter(function (item) { return item && item.ok === false && item.pending !== true; });
      if (result.ok === false || failures.length) {
        ...
        setStatus(
          ...
          '\n백업ID: ' + text(result.backupId || '') +
          (details ? '\n' + details : '') +
          '\nFirestore 운영자료는 유지됩니다. 같은 버튼을 다시 누르면 저장된 지점부터 이어집니다.',
          'error'
        );
        return;
      }
      ...
      if (/deadline-exceeded|504|internal/i.test(errorText)) {
        errorText = '백업 호출이 서버 제한시간에 도달했습니다. 완료된 구간은 저장되어 있습니다.';
      }
      setStatus(errorText + '\nFirestore 운영자료와 완료된 백업 구간은 유지됩니다. 같은 버튼을 다시 누르면 저장된 지점부터 이어집니다.', 'error');
    } finally { hideLoading(); }
  }
