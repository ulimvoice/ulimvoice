...
} from "../src/mirror/attendancePhase3bAhnSuhyeonControlledRollout.js";
import {
  handleAttendancePhase3b2AhnSuhyeonControlledRollout,
  type AttendancePhase3b2AhnSuhyeonApproval,
  type AttendancePhase3b2AhnSuhyeonApprovalStore,
  type AttendancePhase3b2AhnSuhyeonAssignmentWriter,
  type AttendancePhase3b2AhnSuhyeonAuditWriter
} from "../src/mirror/syncAttendancePhase3bAhnSuhyeonControlledRolloutCallable.js";
...
function targetRow(
  ...
  return {
    ...
    studentName:
      `테스트학생${studentIndex}`,
    studentNo:
      String(9200 + studentIndex),
    studentUid:
      `STU-20260716-AS${studentIndex}`,
    studentIdentityKey:
      `UID_TEST_AS_${studentIndex}`,
    instructor:
      ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET
        .instructor,
    teacherName:
...
test("fixed five-record Ahn Suhyeon target produces safe initial plan", async () => {
  const testDeps = deps();
...
test("source cell outside V21-V25 allowlist is blocked", async () => {
  const testDeps = deps();
...
test("dry run is read-only", async () => {
  ...
  const result =
    await handleAttendancePhase3b2AhnSuhyeonControlledRollout(
      {
        requestId:
          "phase3b2as-dry-run-test-0001",
        mode: "dry_run",
        records: targetRows()
      },
      auth,
...
test("same successful commit requestId is idempotent after approval is consumed", async () => {
  ...
  const commitInput = {
    requestId:
      "phase3b2as-commit-test-duplicate-0001",
