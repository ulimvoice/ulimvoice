...
export const UID_V2_REBASE_CELL_PATCH_PLAN_PHASE4C13R_VERSION =
  "2026-07-28.716.59-phase4c13r-cell-level-sheet-patch-plan-rollback-simulation-only";
...
const CONTRACT_DIGEST =
  "541de0633426132095cbf272a01e2dd9785744e9f9448b9144b14d3841f6e920";
...
const EXPECTED_COUNTS = Object.freeze(
  {"studentCellPatches":156,"principalCellPatches":12,"identityCellPatches":168,"schemaHeaderPatches":0,"totalCellPatches":168,"principalAuthColumn9Anchors":12,"targetSheets":2}
);
...
async function buildAuthoritativePlan(
  ...
  for (
    ...
    if (
      ...
      throw new HttpsError(
        "failed-precondition",
        "Duplicate target cell or UID exists."
      );
    }
  ...
  if (
    ...
    throw new HttpsError(
      "failed-precondition",
      "Phase 4C-13R patch counts are incorrect."
    );
  }
  ...
  const planCore = {
    version:
      UID_V2_REBASE_CELL_PATCH_PLAN_PHASE4C13R_VERSION,
    phase:
      "Phase 4C-13R",
    mode:
      "rebase156_cell_level_sheet_patch_plan_and_rollback_simulation_only",
    requestId:
      REQUEST_ID,
...
function publicResult(
  ...
  return {
    ...
    nextGate: {
      phase:
        "Phase 4C-14R isolated Sheet-copy cell-patch rehearsal only",
      allowed:
        true,
      actualUidCutoverAllowed:
        false
...
export const stageUidV2RebaseCellPatchPlanPhase4c13r =
  onCall(
    {
      region:
        REGION,
        ...
        async (transaction) => {
          ...
          if (existing.exists) {
            ...
            if (
              ...
              throw new HttpsError(
                "already-exists",
                "A conflicting Phase 4C-13R plan exists."
              );
            }
...
export const inspectUidV2RebaseCellPatchPlanPhase4c13r =
  onCall(
    {
      region:
        REGION,
    ...
    async (request) => {
      ...
      if (!snapshot.exists) {
        throw new HttpsError(
          "not-found",
          "Phase 4C-13R plan was not found."
        );
      }
      ...
      if (!allTrue(checks)) {
        ...
        throw new HttpsError(
          "data-loss",
          `Phase 4C-13R stored plan verification failed: ${failed.join(", ")}`
        );
      }
