...
import {
  ...
  attendancePhase3b2ParkSoyeonApprovalTokenHash,
  buildAttendancePhase3b2ParkSoyeonControlledPlan,
  publicAttendancePhase3b2ParkSoyeonPlan,
  type AttendancePhase3b2ParkSoyeonControlledPlan,
  type AttendancePhase3b2ParkSoyeonControlledRepository,
  type AttendancePhase3b2ParkSoyeonTeacherCandidate
} from "./attendancePhase3bParkSoyeonControlledRollout.js";

export type AttendancePhase3b2ParkSoyeonMode =
  | "dry_run"
  | "arm"
  | "commit"
  | "verify";

export interface AttendancePhase3b2ParkSoyeonInput {
  readonly requestId: string;
  readonly mode: AttendancePhase3b2ParkSoyeonMode;
...
export interface AttendancePhase3b2ParkSoyeonAuth {
  readonly uid: string;
  readonly role: string;
}

export interface AttendancePhase3b2ParkSoyeonApproval {
  readonly approvalId: string;
...
export interface AttendancePhase3b2ParkSoyeonApprovalStore {
  create(
    approval: AttendancePhase3b2ParkSoyeonApproval
  ): Promise<void>;
  ...
  ): Promise<AttendancePhase3b2ParkSoyeonApproval>;
...
export interface AttendancePhase3b2ParkSoyeonAssignmentWriter {
  ensureAssignments(
    operatorUid: string,
    teacherUid: string,
    classId: string,
...
export interface AttendancePhase3b2ParkSoyeonAuditWriter {
  writeAudit(
    requestId: string,
...
export interface AttendancePhase3b2ParkSoyeonDeps {
  readonly repository:
    AttendancePhase3b2ParkSoyeonControlledRepository &
    AttendanceMirrorRepository;
  readonly approvalStore:
    AttendancePhase3b2ParkSoyeonApprovalStore;
  readonly assignmentWriter:
    AttendancePhase3b2ParkSoyeonAssignmentWriter;
  readonly auditWriter:
    AttendancePhase3b2ParkSoyeonAuditWriter;
...
export interface AttendancePhase3b2ParkSoyeonResult {
  readonly ok: true;
  readonly version: string;
  readonly rolloutId: string;
  readonly mode: AttendancePhase3b2ParkSoyeonMode;
  ...
  readonly plan:
    ReturnType<
      typeof publicAttendancePhase3b2ParkSoyeonPlan
    >;
...
function parseInput(
  data: unknown
): AttendancePhase3b2ParkSoyeonInput {
  if (
    !data ||
    typeof data !== "object" ||
    Array.isArray(data)
  ...
  const mode =
    text(input.mode) as
      AttendancePhase3b2ParkSoyeonMode;
...
function requireSuperAdmin(
  auth:
    AttendancePhase3b2ParkSoyeonAuth |
    undefined
): void {
  if (!auth?.uid) {
    throw new HttpsError(
  ...
  if (auth.role !== "superAdmin") {
    throw new HttpsError(
      "permission-denied",
      "Phase 3B-2 박소연 통제 동기화 requires superAdmin"
    );
  }
}
...
function operationName(): string {
  return [
    "attendance_phase3b2_park_soyeon_controlled",
    ATTENDANCE_PHASE3B2_PARK_SOYEON_TARGET
      .rolloutId,
...
function approvalValid(
  approval:
    AttendancePhase3b2ParkSoyeonApproval |
    undefined,
  auth: AttendancePhase3b2ParkSoyeonAuth,
  input: AttendancePhase3b2ParkSoyeonInput,
  plan:
...
function duplicateResult(
  ...
  plan:
    AttendancePhase3b2ParkSoyeonControlledPlan
): AttendancePhase3b2ParkSoyeonResult {
  return {
    ok: true,
    version:
      ATTENDANCE_PHASE3B2_PARK_SOYEON_CONTROLLED_VERSION,
...
export async function handleAttendancePhase3b2ParkSoyeonControlledRollout(
  data: unknown,
  auth:
    AttendancePhase3b2ParkSoyeonAuth |
    undefined,
  deps: AttendancePhase3b2ParkSoyeonDeps
): Promise<AttendancePhase3b2ParkSoyeonResult> {
  ...
  try {
    ...
    if (claimed) {
      try {
        ...
        console.error(
          "[phase3b2-park-soyeon] approval failure recording failed",
          approvalError
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[phase3b2-park-soyeon] idempotency failure recording failed",
        idempotencyError
      );
    }
    ...
    console.error(
      "[phase3b2-park-soyeon] controlled rollout failed",
      error
    );

    throw new HttpsError(
      "internal",
      "Phase 3B-2 박소연 통제 동기화 failed"
    );
  }
}
...
export class FirestoreAttendancePhase3b2ParkSoyeonRepository
implements AttendancePhase3b2ParkSoyeonControlledRepository,
AttendanceMirrorRepository {
...
export class FirestoreAttendancePhase3b2ParkSoyeonApprovalStore
...
export class FirestoreAttendancePhase3b2ParkSoyeonAssignmentWriter
...
export class FirestoreAttendancePhase3b2ParkSoyeonAuditWriter
...
export const syncAttendancePhase3bParkSoyeonControlledRolloutCallable =
  onCall(
    {
      region:
        ULIM_FUNCTION_REGION,
    ...
    async (
      ...
      return handleAttendancePhase3b2ParkSoyeonControlledRollout(
        request.data,
        request.auth
          ? {
              uid:
        ...
        {
          repository:
            new FirestoreAttendancePhase3b2ParkSoyeonRepository(
              db
            ),
          approvalStore:
            new FirestoreAttendancePhase3b2ParkSoyeonApprovalStore(
              db
            ),
          assignmentWriter:
            new FirestoreAttendancePhase3b2ParkSoyeonAssignmentWriter(
              db
            ),
          auditWriter:
            new FirestoreAttendancePhase3b2ParkSoyeonAuditWriter(
              db
            ),
          idempotencyStore:
