...
export const ATTENDANCE_PHASE3B2_KIM_HAEUN_CONTROLLED_VERSION =
  "20260716.3-phase3b2-kim-haeun-youth-intermediate-controlled-rollout-fixed-teacher-uid";

export const ATTENDANCE_PHASE3B2_KIM_HAEUN_TARGET = Object.freeze({
  rolloutId:
    "phase3b2-kim-haeun-saturday-youth-intermediate-2026-07-18",
  date: "2026-07-18",
  className:
    "[김하은T] - 토요일 청소년 초중급 12:30 ~ 14:30",
  instructor: "김하은T",
  classId:
...
export interface AttendancePhase3b2KimHaeunTeacherCandidate {
...
export interface AttendancePhase3b2KimHaeunAssignmentPresence {
  readonly admin: boolean;
  readonly teacher: boolean;
}

export interface AttendancePhase3b2KimHaeunControlledRepository
extends AttendancePhase3bPreflightRepository {
  ...
  ): Promise<AttendancePhase3b2KimHaeunTeacherCandidate[]>;

  detailedAssignmentPresence(
    operatorUid: string,
    teacherUid: string,
    classId: string
  ): Promise<AttendancePhase3b2KimHaeunAssignmentPresence>;
}

export interface AttendancePhase3b2KimHaeunControlledPlan {
  ...
  readonly assignmentPresence:
    AttendancePhase3b2KimHaeunAssignmentPresence;
...
export function attendancePhase3b2KimHaeunApprovalTokenHash(
  token: string
): string {
...
export function publicAttendancePhase3b2KimHaeunPlan(
  plan: AttendancePhase3b2KimHaeunControlledPlan
): Omit<
...
export async function buildAttendancePhase3b2KimHaeunControlledPlan(
  records: readonly LegacyAttendanceRecord[],
  operatorUid: string,
  repository:
    AttendancePhase3b2KimHaeunControlledRepository,
  now = new Date()
): Promise<AttendancePhase3b2KimHaeunControlledPlan> {
  const errors: string[] = [];
  ...
  if (
    ...
    ATTENDANCE_PHASE3B2_KIM_HAEUN_TARGET.classId
  ) {
    errors.push(
  ...
  const mirrorPlan =
    buildAttendanceMirrorPlan(
      ...
      {
        runId:
          `phase3b2_kim_haeun_plan_${now.getTime()}`,
        now,
  ...
  let status:
    AttendancePhase3b2KimHaeunControlledPlan["status"];
...
export function attendancePhase3b2KimHaeunDigestForApproval(
  value: {
...
export function attendancePhase3b2KimHaeunStoredAttendanceIds(
  documents:
