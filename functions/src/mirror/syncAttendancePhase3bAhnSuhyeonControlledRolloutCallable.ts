...
import {
  ATTENDANCE_PHASE3B2_AHN_SUHYEON_CONTROLLED_VERSION,
  ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET,
  attendancePhase3b2AhnSuhyeonApprovalTokenHash,
  buildAttendancePhase3b2AhnSuhyeonControlledPlan,
  publicAttendancePhase3b2AhnSuhyeonPlan,
  type AttendancePhase3b2AhnSuhyeonControlledPlan,
  type AttendancePhase3b2AhnSuhyeonControlledRepository,
  type AttendancePhase3b2AhnSuhyeonTeacherCandidate
} from "./attendancePhase3bAhnSuhyeonControlledRollout.js";

export type AttendancePhase3b2AhnSuhyeonMode =
  | "dry_run"
  | "arm"
  | "commit"
  | "verify";

export interface AttendancePhase3b2AhnSuhyeonInput {
  readonly requestId: string;
  readonly mode: AttendancePhase3b2AhnSuhyeonMode;
...
export interface AttendancePhase3b2AhnSuhyeonAuth {
  readonly uid: string;
  readonly role: string;
}

export interface AttendancePhase3b2AhnSuhyeonApproval {
  readonly approvalId: string;
...
export interface AttendancePhase3b2AhnSuhyeonApprovalStore {
  create(
    approval: AttendancePhase3b2AhnSuhyeonApproval
  ): Promise<void>;

  get(
    approvalId: string
  ): Promise<
    AttendancePhase3b2AhnSuhyeonApproval |
    undefined
  >;
  ...
  ): Promise<AttendancePhase3b2AhnSuhyeonApproval>;
...
export interface AttendancePhase3b2AhnSuhyeonAssignmentWriter {
  ensureAssignments(
    operatorUid: string,
    teacherUid: string,
    classId: string,
...
export interface AttendancePhase3b2AhnSuhyeonAuditWriter {
  writeAudit(
    requestId: string,
...
export interface AttendancePhase3b2AhnSuhyeonDeps {
  readonly repository:
    AttendancePhase3b2AhnSuhyeonControlledRepository &
    AttendanceMirrorRepository;
  readonly approvalStore:
    AttendancePhase3b2AhnSuhyeonApprovalStore;
  readonly assignmentWriter:
    AttendancePhase3b2AhnSuhyeonAssignmentWriter;
  readonly auditWriter:
    AttendancePhase3b2AhnSuhyeonAuditWriter;
...
export interface AttendancePhase3b2AhnSuhyeonResult {
  readonly ok: true;
  readonly version: string;
  readonly rolloutId: string;
  readonly mode: AttendancePhase3b2AhnSuhyeonMode;
  ...
  readonly plan:
    ReturnType<
      typeof publicAttendancePhase3b2AhnSuhyeonPlan
    >;
...
function parseInput(
  data: unknown
): AttendancePhase3b2AhnSuhyeonInput {
  if (
    !data ||
    typeof data !== "object" ||
    Array.isArray(data)
  ...
  const mode =
    text(input.mode) as
      AttendancePhase3b2AhnSuhyeonMode;
...
function requireSuperAdmin(
  auth:
    AttendancePhase3b2AhnSuhyeonAuth |
    undefined
): void {
  if (!auth?.uid) {
    throw new HttpsError(
  ...
  if (auth.role !== "superAdmin") {
    throw new HttpsError(
      "permission-denied",
      "Phase 3B-2 안수현 A반 통제 동기화 requires superAdmin"
    );
  }
}
...
function operationName(): string {
  return [
    "attendance_phase3b2_ahn_suhyeon_controlled",
    ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET
      .rolloutId,
    ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET.date,
    ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET
      .classId,
    "commit"
  ].join(":");
...
function approvalValid(
  approval:
    AttendancePhase3b2AhnSuhyeonApproval |
    undefined,
  auth: AttendancePhase3b2AhnSuhyeonAuth,
  input: AttendancePhase3b2AhnSuhyeonInput,
  plan:
    AttendancePhase3b2AhnSuhyeonControlledPlan,
  now: Date
): void {
  if (!approval) {
    throw new HttpsError(
...
function duplicateResult(
  ...
  plan:
    AttendancePhase3b2AhnSuhyeonControlledPlan
): AttendancePhase3b2AhnSuhyeonResult {
  return {
    ok: true,
    version:
      ATTENDANCE_PHASE3B2_AHN_SUHYEON_CONTROLLED_VERSION,
...
export async function handleAttendancePhase3b2AhnSuhyeonControlledRollout(
  data: unknown,
  auth:
    AttendancePhase3b2AhnSuhyeonAuth |
    undefined,
  deps: AttendancePhase3b2AhnSuhyeonDeps
): Promise<AttendancePhase3b2AhnSuhyeonResult> {
  ...
  const plan =
    await buildAttendancePhase3b2AhnSuhyeonControlledPlan(
      input.records,
      auth!.uid,
      deps.repository,
      now
  ...
  try {
    ...
    if (claimed) {
      try {
        ...
        console.error(
          "[phase3b2-ahn-suhyeon] approval failure recording failed",
          approvalError
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[phase3b2-ahn-suhyeon] idempotency failure recording failed",
        idempotencyError
      );
    }
    ...
    console.error(
      "[phase3b2-ahn-suhyeon] controlled rollout failed",
      error
    );

    throw new HttpsError(
      "internal",
      "Phase 3B-2 안수현 A반 통제 동기화 failed"
    );
  }
}
...
export class FirestoreAttendancePhase3b2AhnSuhyeonRepository
implements AttendancePhase3b2AhnSuhyeonControlledRepository,
AttendanceMirrorRepository {
  ...
  async resolveTeacherCandidates(
    ...
    AttendancePhase3b2AhnSuhyeonTeacherCandidate[]
  > {
    const byUid = new Map<
      string,
      AttendancePhase3b2AhnSuhyeonTeacherCandidate
...
export class FirestoreAttendancePhase3b2AhnSuhyeonApprovalStore
implements AttendancePhase3b2AhnSuhyeonApprovalStore {
  constructor(
    private readonly db: Firestore
  ) {}
...
export class FirestoreAttendancePhase3b2AhnSuhyeonAssignmentWriter
implements AttendancePhase3b2AhnSuhyeonAssignmentWriter {
  constructor(
    private readonly db: Firestore
  ) {}
...
export class FirestoreAttendancePhase3b2AhnSuhyeonAuditWriter
implements AttendancePhase3b2AhnSuhyeonAuditWriter {
  constructor(
    private readonly db: Firestore
  ) {}
...
export const syncAttendancePhase3bAhnSuhyeonControlledRolloutCallable =
  onCall(
    {
      region:
        ULIM_FUNCTION_REGION,
    ...
    async (
      ...
      return handleAttendancePhase3b2AhnSuhyeonControlledRollout(
        request.data,
        request.auth
          ? {
              uid:
        ...
        {
          repository:
            new FirestoreAttendancePhase3b2AhnSuhyeonRepository(
              db
            ),
          approvalStore:
            new FirestoreAttendancePhase3b2AhnSuhyeonApprovalStore(
              db
            ),
          assignmentWriter:
            new FirestoreAttendancePhase3b2AhnSuhyeonAssignmentWriter(
              db
            ),
          auditWriter:
            new FirestoreAttendancePhase3b2AhnSuhyeonAuditWriter(
              db
            ),
          idempotencyStore:
