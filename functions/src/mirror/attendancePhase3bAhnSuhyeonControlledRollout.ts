...
export const ATTENDANCE_PHASE3B2_AHN_SUHYEON_CONTROLLED_VERSION =
  "20260716.2-phase3b2-ahn-suhyeon-a-controlled-rollout-fixed-teacher-uid";

export const ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET = Object.freeze({
  rolloutId:
    "phase3b2-ahn-suhyeon-saturday-acting-basic-a-2026-07-18",
  date: "2026-07-18",
  className:
    "[안수현T] - 토요일 연기 기초 A반 13:00 ~ 16:00",
  instructor: "안수현T",
  classId:
...
export interface AttendancePhase3b2AhnSuhyeonTeacherCandidate {
...
export interface AttendancePhase3b2AhnSuhyeonAssignmentPresence {
  readonly admin: boolean;
  readonly teacher: boolean;
}

export interface AttendancePhase3b2AhnSuhyeonControlledRepository
extends AttendancePhase3bPreflightRepository {
  ...
  ): Promise<AttendancePhase3b2AhnSuhyeonTeacherCandidate[]>;

  detailedAssignmentPresence(
    operatorUid: string,
    teacherUid: string,
    classId: string
  ): Promise<AttendancePhase3b2AhnSuhyeonAssignmentPresence>;
}

export interface AttendancePhase3b2AhnSuhyeonControlledPlan {
  ...
  readonly assignmentPresence:
    AttendancePhase3b2AhnSuhyeonAssignmentPresence;
...
export function attendancePhase3b2AhnSuhyeonApprovalTokenHash(
  token: string
): string {
...
export function publicAttendancePhase3b2AhnSuhyeonPlan(
  plan: AttendancePhase3b2AhnSuhyeonControlledPlan
): Omit<
...
export async function buildAttendancePhase3b2AhnSuhyeonControlledPlan(
  records: readonly LegacyAttendanceRecord[],
  operatorUid: string,
  repository:
    AttendancePhase3b2AhnSuhyeonControlledRepository,
  now = new Date()
): Promise<AttendancePhase3b2AhnSuhyeonControlledPlan> {
  const errors: string[] = [];
  ...
  if (
    ...
    ATTENDANCE_PHASE3B2_AHN_SUHYEON_TARGET.classId
  ) {
    errors.push(
  ...
  const mirrorPlan =
    buildAttendanceMirrorPlan(
      ...
      {
        runId:
          `phase3b2_ahn_suhyeon_plan_${now.getTime()}`,
        now,
  ...
  let status:
    AttendancePhase3b2AhnSuhyeonControlledPlan["status"];
...
export function attendancePhase3b2AhnSuhyeonDigestForApproval(
  value: {
...
export function attendancePhase3b2AhnSuhyeonStoredAttendanceIds(
  documents:
