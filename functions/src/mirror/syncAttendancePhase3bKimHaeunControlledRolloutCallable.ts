...
import {
  ATTENDANCE_PHASE3B2_KIM_HAEUN_CONTROLLED_VERSION,
  ATTENDANCE_PHASE3B2_KIM_HAEUN_TARGET,
  attendancePhase3b2KimHaeunApprovalTokenHash,
  buildAttendancePhase3b2KimHaeunControlledPlan,
  publicAttendancePhase3b2KimHaeunPlan,
  type AttendancePhase3b2KimHaeunControlledPlan,
  type AttendancePhase3b2KimHaeunControlledRepository,
  type AttendancePhase3b2KimHaeunTeacherCandidate
} from "./attendancePhase3bKimHaeunControlledRollout.js";

export type AttendancePhase3b2KimHaeunMode =
  | "dry_run"
  | "arm"
  | "commit"
  | "verify";

export interface AttendancePhase3b2KimHaeunInput {
  readonly requestId: string;
  readonly mode: AttendancePhase3b2KimHaeunMode;
...
export interface AttendancePhase3b2KimHaeunAuth {
  readonly uid: string;
  readonly role: string;
}

export interface AttendancePhase3b2KimHaeunApproval {
  readonly approvalId: string;
...
export interface AttendancePhase3b2KimHaeunApprovalStore {
  create(
    approval: AttendancePhase3b2KimHaeunApproval
  ): Promise<void>;
  ...
  ): Promise<AttendancePhase3b2KimHaeunApproval>;
...
export interface AttendancePhase3b2KimHaeunAssignmentWriter {
  ensureAssignments(
    operatorUid: string,
    teacherUid: string,
    classId: string,
...
export interface AttendancePhase3b2KimHaeunAuditWriter {
  writeAudit(
    requestId: string,
...
export interface AttendancePhase3b2KimHaeunDeps {
  readonly repository:
    AttendancePhase3b2KimHaeunControlledRepository &
    AttendanceMirrorRepository;
  readonly approvalStore:
    AttendancePhase3b2KimHaeunApprovalStore;
  readonly assignmentWriter:
    AttendancePhase3b2KimHaeunAssignmentWriter;
  readonly auditWriter:
    AttendancePhase3b2KimHaeunAuditWriter;
...
export interface AttendancePhase3b2KimHaeunResult {
  readonly ok: true;
  readonly version: string;
  readonly rolloutId: string;
  readonly mode: AttendancePhase3b2KimHaeunMode;
  ...
  readonly plan:
    ReturnType<
      typeof publicAttendancePhase3b2KimHaeunPlan
    >;
...
function parseInput(
  data: unknown
): AttendancePhase3b2KimHaeunInput {
  if (
    !data ||
    typeof data !== "object" ||
    Array.isArray(data)
  ...
  const mode =
    text(input.mode) as
      AttendancePhase3b2KimHaeunMode;
...
function requireSuperAdmin(
  auth:
    AttendancePhase3b2KimHaeunAuth |
    undefined
): void {
  if (!auth?.uid) {
    throw new HttpsError(
  ...
  if (auth.role !== "superAdmin") {
    throw new HttpsError(
      "permission-denied",
      "Phase 3B-2 김하은 청소년 초중급 통제 동기화 requires superAdmin"
    );
  }
}
...
function operationName(): string {
  return [
    "attendance_phase3b2_kim_haeun_controlled",
    ATTENDANCE_PHASE3B2_KIM_HAEUN_TARGET
      .rolloutId,
...
function approvalValid(
  approval:
    AttendancePhase3b2KimHaeunApproval |
    undefined,
  auth: AttendancePhase3b2KimHaeunAuth,
  input: AttendancePhase3b2KimHaeunInput,
  plan:
    AttendancePhase3b2KimHaeunControlledPlan,
  now: Date
): void {
  if (!approval) {
    throw new HttpsError(
...
function duplicateResult(
  ...
  plan:
    AttendancePhase3b2KimHaeunControlledPlan
): AttendancePhase3b2KimHaeunResult {
  return {
    ok: true,
    version:
      ATTENDANCE_PHASE3B2_KIM_HAEUN_CONTROLLED_VERSION,
...
export async function handleAttendancePhase3b2KimHaeunControlledRollout(
  data: unknown,
  auth:
    AttendancePhase3b2KimHaeunAuth |
    undefined,
  deps: AttendancePhase3b2KimHaeunDeps
): Promise<AttendancePhase3b2KimHaeunResult> {
  ...
  try {
    ...
    if (claimed) {
      try {
        ...
        console.error(
          "[phase3b2-kim-haeun] approval failure recording failed",
          approvalError
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[phase3b2-kim-haeun] idempotency failure recording failed",
        idempotencyError
      );
    }
    ...
    console.error(
      "[phase3b2-kim-haeun] controlled rollout failed",
      error
    );

    throw new HttpsError(
      "internal",
      "Phase 3B-2 김하은 청소년 초중급 통제 동기화 failed"
    );
  }
}
...
export class FirestoreAttendancePhase3b2KimHaeunRepository
implements AttendancePhase3b2KimHaeunControlledRepository,
AttendanceMirrorRepository {
  ...
  async resolveTeacherCandidates(
    ...
    AttendancePhase3b2KimHaeunTeacherCandidate[]
  > {
    const byUid = new Map<
      string,
      AttendancePhase3b2KimHaeunTeacherCandidate
...
export class FirestoreAttendancePhase3b2KimHaeunApprovalStore
...
export class FirestoreAttendancePhase3b2KimHaeunAssignmentWriter
...
export class FirestoreAttendancePhase3b2KimHaeunAuditWriter
...
export const syncAttendancePhase3bKimHaeunControlledRolloutCallable =
  onCall(
    {
      region:
        ULIM_FUNCTION_REGION,
    ...
    async (
      ...
      return handleAttendancePhase3b2KimHaeunControlledRollout(
        request.data,
        request.auth
          ? {
              uid:
        ...
        {
          repository:
            new FirestoreAttendancePhase3b2KimHaeunRepository(
              db
            ),
          approvalStore:
            new FirestoreAttendancePhase3b2KimHaeunApprovalStore(
              db
            ),
          assignmentWriter:
            new FirestoreAttendancePhase3b2KimHaeunAssignmentWriter(
              db
            ),
          auditWriter:
            new FirestoreAttendancePhase3b2KimHaeunAuditWriter(
              db
            ),
          idempotencyStore:
